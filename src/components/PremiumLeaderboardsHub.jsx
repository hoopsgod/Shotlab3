...
export default function PremiumLeaderboardsHub({ viewerRole, leaderboardRows = [], leaderboardStatus = 'idle', userEmail = '', currentUser = {}, programScores = [], programDrills = [], players = [], teamId = '', homeScores = [], shotLogs = [], testId = 'premium-leaderboards-hub' }) {
  ...
  const coachRosterIdentitySet = useMemo(() => new Set((Array.isArray(players) ? players : []).filter((player) => player?.role !== 'coach' && player?.archived !== true && player?.hideFromLeaderboards !== true && !['archived', 'removed', 'team_local_data_deleted'].includes(String(player?.rosterStatus || player?.roster_status || '').toLowerCase())).flatMap((player) => [player?.email, player?.player_email, player?.playerId, player?.player_id, player?.userId, player?.user_id, player?.profileId, player?.profile_id, player?.id].map((value) => String(value || '').trim().toLowerCase()).filter(Boolean))), [players]);
  const rowMatchesCoachRoster = (row = {}) => [row?.email, row?.player_email, row?.playerId, row?.player_id, row?.userId, row?.user_id, row?.profileId, row?.profile_id, row?.id].map((value) => String(value || '').trim().toLowerCase()).some((key) => key && coachRosterIdentitySet.has(key));
  ...
  const programDrillLeaderboardRows = useMemo(() => viewerRole === 'coach' ? rawProgramDrillLeaderboardRows.filter(rowMatchesCoachRoster) : rawProgramDrillLeaderboardRows, [viewerRole, rawProgramDrillLeaderboardRows, coachRosterIdentitySet]);
  useEffect(() => {
    if (viewerRole !== 'coach' || !isShotLabDebugMode()) return;
    rawProgramDrillLeaderboardRows.forEach((row) => {
      if (!rowMatchesCoachRoster(row)) console.warn('[coach-leaderboard] filtered non-roster program row', { normalizedRowEmail: String(row?.email || row?.player_email || '').trim().toLowerCase(), rowIdentity: { playerId: row?.playerId, player_id: row?.player_id, userId: row?.userId, user_id: row?.user_id, profileId: row?.profileId, profile_id: row?.profile_id, id: row?.id }, coachTeamId: teamId, activeRosterCount: coachRosterIdentitySet.size, rosterMatchFound: false, reason: 'excluded_not_active_roster_member' });
    });
  }, [viewerRole, rawProgramDrillLeaderboardRows, coachRosterIdentitySet, teamId]);
  useEffect(() => {
