      ...
      <section style={{padding:"8px 12px 12px"}}>
        ...
        <div style={{minHeight:62,border:"1px solid var(--stroke-1)",borderRadius:14,background:"linear-gradient(180deg, rgba(255,255,255,0.02), rgba(0,0,0,0.22))",padding:"8px 10px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
          <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}>
            ...
            <h2 className="u-allcaps-long" style={{fontFamily:FD,fontSize:15,letterSpacing:"0.05em",color:"var(--text-1)",margin:0,whiteSpace:"nowrap"}}>Coach Tools</h2>
          </div>
  ...
  return (
    <section style={{padding:"12px 12px 14px"}}>
      <div style={{marginTop:2,marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <h2 className="u-allcaps-long" style={{fontFamily:FD,fontSize:17,letterSpacing:"0.05em",color:"var(--text-1)",margin:0}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3, minmax(0, 1fr))",gap:8}}>
        <button type="button" onClick={onPlayersClick} style={{...metricBase,border:highlightPlayersAttention?"1px solid rgba(255,69,69,0.45)":metricBase.border,boxShadow:"none"}}>
          <div style={{fontFamily:FB,fontSize:11,fontWeight:700,letterSpacing:"0.04em",color:"var(--text-2)",textTransform:"uppercase",overflow:"hidden",textOverflow:"ellipsis"}}>Players</div>
          <div style={{marginTop:5,fontFamily:FD,fontSize:26,fontWeight:900,lineHeight:1,color:"var(--accent)",letterSpacing:"0.03em"}}>{totalPlayers}</div>
        </button>

        <button type="button" onClick={onActiveTodayClick} style={metricBase}>
          <div style={{fontFamily:FB,fontSize:11,fontWeight:700,letterSpacing:"0.04em",color:"var(--text-2)",textTransform:"uppercase",overflow:"hidden",textOverflow:"ellipsis"}}>Active Today</div>
          <div style={{marginTop:5,fontFamily:FD,fontSize:26,fontWeight:900,lineHeight:1,color:"var(--text-1)",letterSpacing:"0.03em"}}>{activeTodayCount}</div>
        </button>

        <button type="button" onClick={onNextEventClick} style={metricBase}>
          <div style={{fontFamily:FB,fontSize:11,fontWeight:700,letterSpacing:"0.04em",color:"var(--text-2)",textTransform:"uppercase",overflow:"hidden",textOverflow:"ellipsis"}}>Next Event</div>
          <div style={{marginTop:6,fontFamily:FD,fontSize:18,fontWeight:900,lineHeight:1,color:"var(--text-1)",letterSpacing:"0.02em"}}>{nextEventDateFormatted}</div>
        </button>
      </div>
      ...
      <div style={{margin:"10px 0 4px",padding:"14px 14px",border:"1px solid var(--stroke-1)",borderRadius:14,background:"var(--surface-2)"}}>
        <div className="u-meta-label" style={{fontFamily:FB,fontSize:11,color:"var(--text-2)",letterSpacing:"0.05em"}}>TEAM CODE</div>
        <div style={{display:"flex",alignItems:"center",gap:8,marginTop:8,flexWrap:"wrap"}}>
          <div style={{fontFamily:FD,fontSize:27,color:"var(--text-1)",letterSpacing:3,minWidth:114,lineHeight:1}}>{joinCode||"—"}</div>
          <button onClick={onCopyJoinCode} style={{padding:"8px 12px",fontSize:10,border:"1px solid var(--stroke-1)",background:"var(--surface-1)",color:"var(--text-1)",borderRadius:10,cursor:"pointer",fontWeight:800,letterSpacing:"0.04em"}}>COPY</button>
          <button onClick={onRegenerateJoinCode} style={{padding:"8px 12px",fontSize:10,border:"1px solid var(--stroke-1)",background:"var(--surface-1)",color:"var(--text-1)",borderRadius:10,cursor:"pointer",fontWeight:800,letterSpacing:"0.03em"}}>REGENERATE</button>
        </div>
