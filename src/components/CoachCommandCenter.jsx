...
function TeamActivityPanel({ activeCount, inactiveCount, rosterSize, activeRate, onOpen }) {
  return <article className="mcSection mcTeamHealth" aria-labelledby="mc-health-heading" data-testid="coach-program-pulse"><div className="mcSectionHead"><span><small>Program pulse</small><h2 id="mc-health-heading">Program Pulse</h2></span><strong className="mcHealthScore">{activeRate}%</strong></div><div className="mcHealthBar"><span style={{ width: `${activeRate}%` }} /></div><div className="mcHealthFacts"><div><strong>{activeCount}</strong><small>Active</small></div><div><strong>{inactiveCount}</strong><small>Not active</small></div><div><strong>{rosterSize}</strong><small>Rostered</small></div></div><button type="button" className="mcTextLink" onClick={onOpen}>Review activity <Icon name="arrow" size={15} /></button></article>;
}
...
function NextSessionPanel({ date, onOpen }) {
  return <article className="mcSection mcNextSession" aria-labelledby="mc-session-heading" data-testid="coach-upcoming-event"><div className="mcSectionHead"><span><small>Upcoming event</small><h2 id="mc-session-heading">Upcoming Event</h2></span></div><div className="mcSessionSummary"><span className="mcSessionIcon is-ready"><Icon name="calendar" /></span><div><strong>Team session ready</strong><small>{String(date)}</small></div></div><button type="button" className="mcSecondaryAction" onClick={onOpen}>Open session<Icon name="arrow" size={17} /></button></article>;
}
