...
export default function CoachCommandCenter({
  ...
  return (
    <section style={{padding:"8px 12px 10px"}}>
      <div style={{marginTop:0,marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3, minmax(0, 1fr))",gap:8}}>
        <button type="button" onClick={onPlayersClick} style={{...metricBase,border:highlightPlayersAttention?"1px solid rgba(255,69,69,0.45)":metricBase.border,boxShadow:"none"}}>
          ...
          <div style={{marginTop:2,fontFamily:FD,fontSize:23,fontWeight:900,lineHeight:1,color:"var(--accent)"}}>{totalPlayers}</div>
          <div style={{marginTop:1,fontFamily:FB,fontSize:9,color:"var(--text-3)",textTransform:"uppercase",letterSpacing:"var(--tracking-tight)"}}>{highlightPlayersAttention?"Needs check-ins":"On track"}</div>
        </button>
        ...
        <button type="button" onClick={onActiveTodayClick} style={metricBase}>
          ...
          <div style={{marginTop:2,fontFamily:FD,fontSize:23,fontWeight:900,lineHeight:1,color:"var(--text-1)"}}>{activeTodayCount}</div>
          <div style={{marginTop:1,fontFamily:FB,fontSize:9,color:"var(--text-3)",textTransform:"uppercase",letterSpacing:"var(--tracking-tight)"}}>Session logs</div>
        </button>
        ...
        <button type="button" onClick={onNextEventClick} style={metricBase}>
          ...
          <div style={{marginTop:3,fontFamily:FD,fontSize:16,fontWeight:900,lineHeight:1,color:"var(--text-1)"}}>{nextEventDateFormatted}</div>
          <div style={{marginTop:1,fontFamily:FB,fontSize:9,color:"var(--text-3)",textTransform:"uppercase",letterSpacing:"var(--tracking-tight)"}}>Timeline</div>
        </button>
      </div>

      <div style={{marginTop:7,overflowX:"auto",whiteSpace:"nowrap",paddingBottom:2}}>
      ...
      <div style={{margin:"8px 0 2px",padding:"10px 12px",border:"1px solid var(--stroke-1)",borderRadius:14,background:"var(--surface-2)"}}>
