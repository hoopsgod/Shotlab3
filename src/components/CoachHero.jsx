...
export default function CoachHero({ heroRef, userName, wordmark, teamName, onOpenTeamBranding, onLogout }) {
  ...
  const logoExists = Boolean(branding?.logoUrl || branding?.logoMarkUrl);
  const coachLabel = userName || "Demo Coach";
  const programLabel = branding?.teamName || teamName || "";
  ...
  return (
    <section ref={heroRef} className="coach-premium-header" aria-label="Coach dashboard header">
      <style>{`
        ...
        .coach-premium-brand-btn{margin-top:11px;display:inline-flex;align-items:center;gap:8px;padding:11px 14px;border-radius:12px;border:1px solid color-mix(in srgb,var(--accent) 45%,var(--stroke-1));background:linear-gradient(160deg,color-mix(in srgb,var(--team-brand-badge-bg) 65%,transparent),rgba(200,255,0,.09));color:var(--text-1);font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;cursor:pointer}
        .coach-premium-logo-zone{position:relative;min-height:175px;display:flex;align-items:center;justify-content:center;padding:10px 28px 8px}
        .coach-premium-logo-shell{width:100%;height:100%;display:flex;align-items:center;justify-content:center}
        .coach-premium-logo-shell.has-logo{border:none}
        .coach-premium-logo-shell.no-logo{min-height:150px;border:1px dashed color-mix(in srgb,var(--accent) 40%,var(--stroke-1));border-radius:16px;background:color-mix(in srgb,var(--surface-1) 30%,transparent)}
        ...
        @media (max-width:700px){.coach-premium-grid{grid-template-columns:1fr;gap:8px}.coach-premium-logo-zone{min-height:120px;padding:4px 22px 0;justify-content:flex-end}.coach-premium-logo-shell.no-logo{min-height:100px}.coach-premium-name{font-size:clamp(30px,10vw,44px)}}
      `}</style>
      <div className="coach-premium-grid">
        ...
        <div className="coach-premium-logo-zone">
          ...
          <div className={`coach-premium-logo-shell ${logoExists ? "has-logo" : "no-logo"}`}>
            <div style={{ maxWidth: "100%", maxHeight: 170, filter: "drop-shadow(0 10px 24px rgba(0,0,0,0.35))" }}>{wordmark}</div>
          </div>
        </div>
      </div>
    </section>
