...
const improvementLabel = (improvement) => {
  if (!improvement) return "Archive a completed season to unlock season-over-season progress.";
  ...
  return `${direction}${number(improvement.delta)} vs ${improvement.comparedTo}${pct}`;
};
...
export default function PlayerCareerHistory({
  ...
  const metrics = [
    ["Career Training", number(history.career.trainingTotal)],
    ...
    ["Program Score", number(history.career.totalProgramScore)],
  ...
  return <section data-testid="player-career-history" data-viewer-role={viewerRole} style={styles.shell}>
    ...
    <div style={styles.copy}>Current activity is combined with immutable season archives. Archived results are read-only and are never rewritten by this view.</div>
    ...
    <div style={{ marginTop: 14, padding: 11, borderRadius: 11, border: "1px solid rgba(200,255,0,.22)", background: "rgba(200,255,0,.055)" }}>
      <div style={{ color: "var(--accent, #c8ff00)", fontSize: 9, fontWeight: 900, letterSpacing: ".08em", textTransform: "uppercase" }}>Season-over-season</div>
    ...
    <div style={{ marginTop: 15 }}>
      ...
      <div style={{ display: "grid", gap: 6 }}>
        <div style={{ color: "var(--text-3, #aaa)", fontSize: 11 }}>Best training season: <strong style={{ color: "var(--text-1, #fff)" }}>{history.records.bestTrainingSeason?.seasonName || "No season yet"}{history.records.bestTrainingSeason ? ` · ${number(history.records.bestTrainingSeason.trainingTotal)}` : ""}</strong></div>
        ...
        <div style={{ color: "var(--text-3, #aaa)", fontSize: 11 }}>Best program score: <strong style={{ color: "var(--text-1, #fff)" }}>{number(history.records.bestProgramScore)}</strong></div>
      </div>
    </div>
      ...
      <div data-testid="career-season-list" style={{ display: "grid", gap: 8 }}>
        {history.seasons.map((season) => <article key={`${season.isCurrent ? "current" : season.archiveId}-${season.seasonName}`} style={styles.season}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 6, marginTop: 9 }}>
            {[["Training", season.trainingTotal], ["Home", season.totalHomeMakes], ["Program", season.totalProgramScore]].map(([label, value]) => <div key={label} style={{ minWidth: 0 }}><div style={{ color: "var(--text-1, #fff)", fontSize: 14, fontWeight: 900 }}>{number(value)}</div><div style={{ color: "var(--text-3, #aaa)", fontSize: 8, textTransform: "uppercase" }}>{label}</div></div>)}
          </div>
