...
export default function PlayerProgressStory({
  ...
  const story = useMemo(() => derivePlayerProgressStory({ userEmail, teamId, shotLogs, scores, programScores, drills, programDrills, streak, coachPriorities, today }), [userEmail, teamId, shotLogs, scores, programScores, drills, programDrills, streak, coachPriorities, today]);
  ...
  return <>
    <TeamIdentityTitleStage variant="editorial" brandTreatment="signature" surface="light" role="Development" title="Progress" status={`${trendWord} · ${story.activeDays7}/7 active days`} testId="player-progress-team-title" className="playerProgressTeamTitle" dataLayoutRole="editorial-header" dataVisualRole="page-intro" dataPageKind="progress" dataMobileStage="editorial" ariaLabel="Team development and Progress" />
      ...
      <div className={styles.hero} data-testid="player-progress-story-hero" data-layout-role="command-story-header">
        ...
        <div className={styles.heroGrid} data-testid="player-progress-story-hero-grid">
          ...
          <div className={styles.targetPanel} data-testid="player-progress-target-court">
            <div className={styles.targetPanelCopy} data-testid="player-progress-target-summary"><span>TODAY’S STANDARD</span><strong>{todayMakes}<small>/{PLAYER_DAILY_SHOT_TARGET}</small></strong></div>
            <ShotLabPerformanceCourt value={todayMakes} max={PLAYER_DAILY_SHOT_TARGET} size={76} label="Target path" testId="player-progress-target-visual" />
          </div>
        </div>
        <div className={styles.metricStrip} data-testid="player-progress-metrics">
          <div className={styles.metricMark}><ShotLabPerformanceMark kind="milestone" value={`${story.activeDays7}/7`} label="Active days" detail="Last 7 days" compact testId="player-progress-active-days-mark" /></div>
          <div className={styles.metricMark}><ShotLabPerformanceMark kind="streak" value={`${story.currentStreak}D`} label="Live rhythm" detail="Current streak" compact testId="player-progress-streak-mark" /></div>
          <div className={styles.metricMark}><ShotLabPerformanceMark kind="pb" value={story.pbCount30} label="Recent PBs" detail="Last 30 days" compact testId="player-progress-pb-mark" /></div>
        </div>
      </div>
