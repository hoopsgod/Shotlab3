    ...
    <div style={{ display: "grid", gap: 14 }} data-testid="branding-live-preview">
      <div style={{ overflow: "hidden", borderRadius: 20, boxShadow: "0 16px 34px rgba(4,18,25,.18)" }}>
        <TeamIdentityTitleStage
          ...
          personName="Coach preview"
          summary="The program owns the first impression."
          iconName="coach"
          ariaLabel="Coach title stage preview"
        />
      </div>
      <div style={{ overflow: "hidden", border: "1px solid rgba(24,32,31,.1)", borderRadius: 20, background: "#f8f6ee", padding: "0 16px" }}>
        <TeamIdentityTitleStage
          ...
          summary="The same team identity carries into the athlete experience."
          status="Coach plan active"
          iconName="training"
          ariaLabel="Player title stage preview"
        />
      </div>
      <section style={{ padding: 16, border: "1px solid rgba(24,32,31,.1)", borderRadius: 18, background: "#f8f7f3" }}>
        ...
        <div style={{ marginTop: 7, color: "#68716c", fontSize: "calc(12px * var(--coach-text-scale-medium))", lineHeight: 1.55 }}>Balanced team colors personalize identity accents while ShotLab keeps body text, actions, status semantics, and navigation readable.</div>
      </section>
    </div>
  );
}
