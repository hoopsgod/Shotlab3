...
function Field({ field, value, onChange }) {
  return (
    <label className="team-branding-form__field">
      ...
      <input type={field.type} value={value || ""} placeholder={field.placeholder || ""} onChange={(event) => onChange(field.name, event.target.value)} />
    </label>
  );
}
  ...
  return (
    <div className="team-branding-form__logo-preview">
      ...
      <div className="team-branding-form__logo-preview-grid">
        <div className="team-branding-form__logo-canvas team-branding-form__logo-canvas--dark"><img src={src} alt={`${label} on dark background`} /></div>
        <div className="team-branding-form__logo-canvas team-branding-form__logo-canvas--transparent"><img src={src} alt={`${label} transparency preview`} /></div>
      </div>
    </div>
  );
}
...
function SectionHeading({ title, description }) {
  return <div className="team-branding-form__section-heading"><h3>{title}</h3><p>{description}</p></div>;
}
...
export default function TeamBrandingForm({ branding, onSave, onCancel, onChange, saving = false }) {
  ...
  const handleChange = (name, value) => setValues((previous) => ({ ...previous, [name]: value || DEFAULT_BRANDING[name] || "" }));
  const handlePaletteSelect = (palette) => setValues((previous) => ({ ...previous, primaryColor: palette.primaryColor, secondaryColor: palette.secondaryColor, accentColor: palette.accentColor, textOnPrimary: palette.textOnPrimary }));
  ...
  const handleLogoUpload = async (kind, file) => {
    if (!file) return;
    if (!file.type?.startsWith("image/")) return setUploadError("Please upload a PNG, JPG, WebP, or SVG image.");
    if (file.size > 6 * 1024 * 1024) return setUploadError("Logo files must be smaller than 6 MB.");
    try {
      setCleaning(true);
  ...
  const cleanCurrentLogos = async () => {
    try {
      setCleaning(true);
      const [full, mark] = await Promise.all([cleanTeamLogoSource(values.logoUrl || FALLBACK_LOGO), cleanTeamLogoSource(values.logoMarkUrl || FALLBACK_MARK)]);
      ...
      <section className="team-branding-form__section">
        ...
        <div className="team-branding-form__palette-grid" aria-label="Approved team color palettes">
          {APPROVED_BRAND_PALETTES.map((palette) => {
            ...
            return (
              <button key={palette.key} type="button" className="team-branding-form__palette" onClick={() => handlePaletteSelect(palette)} aria-pressed={selected}>
                <span className="team-branding-form__palette-label"><span>{palette.label}</span>{selected ? <span className="team-branding-form__selected-label">Selected</span> : null}</span>
                <span className="team-branding-form__swatches" aria-hidden="true">
                  {[palette.primaryColor, palette.secondaryColor, palette.accentColor, palette.textOnPrimary].map((swatch) => <span key={swatch} className="team-branding-form__swatch" style={{ background: swatch }} />)}
                </span>
              </button>
            );
          })}
      ...
      <section className="team-branding-form__section">
        ...
        <div role="radiogroup" aria-label="Text size" className="team-branding-form__option-list">
          {TEXT_SCALE_OPTIONS.map((option) => {
            ...
            return <button key={option.key} type="button" role="radio" aria-checked={selected} className="team-branding-form__text-option" onClick={() => handleChange("textScale", option.key)}><span className="team-branding-form__text-option-label">{option.label}</span><span className="team-branding-form__text-option-hint">{option.hint}</span></button>;
          })}
        </div>
      </section>

      <section className="team-branding-form__section">
        <SectionHeading title="Team logos" description="Upload PNG or SVG when possible. ShotLab removes simple flat backgrounds and saves a transparent PNG." />
        <div className="team-branding-form__field-list">{LOGO_FIELDS.map((field) => <Field key={field.name} field={field} value={values[field.name]} onChange={handleChange} />)}</div>
        <div className="team-branding-form__upload-grid">
          <button type="button" onClick={() => fullLogoInputRef.current?.click()} disabled={cleaning} className="team-branding-form__secondary-action">Upload full logo</button>
          <button type="button" onClick={() => markLogoInputRef.current?.click()} disabled={cleaning} className="team-branding-form__secondary-action">Upload logo mark</button>
          <input ref={fullLogoInputRef} type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" hidden onChange={(event) => { handleLogoUpload("full", event.target.files?.[0]); event.target.value = ""; }} />
          <input ref={markLogoInputRef} type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" hidden onChange={(event) => { handleLogoUpload("mark", event.target.files?.[0]); event.target.value = ""; }} />
        </div>
        <button type="button" onClick={cleanCurrentLogos} disabled={cleaning} className="team-branding-form__clean-action">{cleaning ? "Preparing transparent logos…" : "Clean logo backgrounds"}</button>
        {uploadError ? <div role="alert" className="team-branding-form__alert">{uploadError}</div> : null}
        <div className="team-branding-form__previews"><LogoPreview src={cleanFullLogo} label="Full logo" /><LogoPreview src={cleanMarkLogo} label="Logo mark" /></div>
      </section>

      <div className="team-branding-form__actions">
        <button type="submit" disabled={isBusy} className="cta-primary team-branding-form__save-action">{saving || submitting ? "Saving..." : "Save team branding"}</button>
        <button type="button" onClick={onCancel} disabled={isBusy} className="team-branding-form__cancel-action">Cancel</button>
      </div>
    </form>
  );
}
