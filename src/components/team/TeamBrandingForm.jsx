...
const TEXT_SCALE_OPTIONS = [
  { key: "standard", label: "Default", hint: "Balanced ShotLab sizing" },
...
function LogoPreview({ src, label }) {
  return (
    <div className="branding-logo-preview" style={{ display: "grid", gap: 7 }}>
      <div style={{ color: "#9CA3AF", fontSize: 11, fontWeight: 700 }}>{label}</div>
      <div className="branding-logo-preview__surfaces" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <div style={{ minHeight: 92, borderRadius: 14, border: "1px solid rgba(255,255,255,0.12)", background: "#101310", display: "grid", placeItems: "center", padding: 10, overflow: "hidden" }}>
          <img src={src} alt={`${label} on dark background`} style={{ width: "100%", height: 66, objectFit: "contain", filter: "drop-shadow(0 7px 12px rgba(0,0,0,.35))" }} />
        </div>
        <div style={{ minHeight: 92, borderRadius: 14, border: "1px solid rgba(24,32,31,.12)", background: "linear-gradient(45deg,#e5e7eb 25%,#fff 25%,#fff 50%,#e5e7eb 50%,#e5e7eb 75%,#fff 75%) 0 0/18px 18px", display: "grid", placeItems: "center", padding: 10, overflow: "hidden" }}>
          <img src={src} alt={`${label} transparency preview`} style={{ width: "100%", height: 66, objectFit: "contain" }} />
        </div>
      </div>
    </div>
  );
...
export default function TeamBrandingForm({ branding, onSave, onCancel, onChange, saving = false }) {
  ...
  return (
    <form noValidate onSubmit={submit} style={{ display: "grid", gap: 18 }} data-testid="program-identity-form">
      <section style={{ display: "grid", gap: 11 }} data-testid="branding-palette-controls">
        {sectionTitle("Brand colors", "Choose one controlled palette. It will accent the product without overwhelming the content.")}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>
          {APPROVED_BRAND_PALETTES.map((palette) => {
            const selected = selectedPaletteKey === palette.key;
            return (
              <button key={palette.key} type="button" aria-label={`Use ${palette.label} palette`} onClick={() => handlePaletteSelect(palette)} aria-pressed={selected} style={{ display: "grid", gap: 10, minHeight: 68, padding: "10px 12px", borderRadius: 10, border: selected ? "1px solid rgba(157,255,122,0.9)" : "1px solid rgba(255,255,255,0.14)", background: selected ? "rgba(157,255,122,0.1)" : "#111620", color: "#E5E7EB", textAlign: "left", cursor: "pointer" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}><span style={{ fontSize: 13, fontWeight: 700 }}>{palette.label}</span>{selected ? <span style={{ fontSize: 10, color: "#9DFF7A", fontWeight: 800, letterSpacing: ".05em", textTransform: "uppercase" }}>Selected</span> : null}</div>
      ...
      <section style={{ display: "grid", gap: 12, paddingTop: 15, borderTop: "1px solid rgba(255,255,255,0.08)" }} data-testid="branding-logo-controls">
        {sectionTitle("Team logos", "Upload the assets coaches already use. ShotLab prepares simple backgrounds automatically and previews both dark and transparent placement.")}
        <div className="branding-logo-upload-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8 }}>
          <button type="button" onClick={() => fullLogoInputRef.current?.click()} disabled={cleaning} style={{ minHeight: 46, borderRadius: 10, border: "1px solid rgba(255,255,255,0.18)", background: "rgba(255,255,255,0.04)", color: "#E5E7EB" }}>Upload full logo</button>
          <button type="button" onClick={() => markLogoInputRef.current?.click()} disabled={cleaning} style={{ minHeight: 46, borderRadius: 10, border: "1px solid rgba(255,255,255,0.18)", background: "rgba(255,255,255,0.04)", color: "#E5E7EB" }}>Upload logo mark</button>
        ...
        <button type="button" onClick={cleanCurrentLogos} disabled={cleaning} style={{ minHeight: 42, borderRadius: 10, border: "1px solid rgba(200,255,26,0.34)", background: "rgba(200,255,26,0.06)", color: "#DFFF75", fontWeight: 700 }}>{cleaning ? "Preparing transparent logos…" : "Clean logo backgrounds"}</button>
        {uploadError ? <div style={{ color: "#B4232A", fontSize: 12, lineHeight: 1.4 }} role="alert">{uploadError}</div> : null}
        <LogoPreview src={cleanFullLogo} label="Full logo" />
        <LogoPreview src={cleanMarkLogo} label="Logo mark" />
        <details className="branding-advanced-sources" data-testid="branding-advanced-logo-sources">
          <summary>Advanced logo sources</summary>
          <p>Use direct URLs only when the logo already lives on a stable public image host.</p>
      ...
      <section style={{ display: "grid", gap: 10, paddingTop: 15, borderTop: "1px solid rgba(255,255,255,0.08)" }} data-testid="branding-text-controls">
        {sectionTitle("Text size", "Choose one shared readability level for body copy, controls, inputs, and navigation labels.")}
      ...
      <div className="branding-form-actions" style={{ display: "flex", gap: 8, marginTop: 4 }}>
        <button type="submit" disabled={saving || cleaning} className="cta-primary" style={{ width: "auto", margin: 0, minHeight: 44, borderRadius: 10, padding: "0 16px", boxShadow: "0 6px 16px rgba(0,0,0,0.34)" }}>{saving ? "Saving..." : "Save team identity"}</button>
        <button type="button" onClick={onCancel} style={{ minHeight: 44, borderRadius: 10, padding: "0 14px", border: "1px solid rgba(255,255,255,0.18)", background: "rgba(255,255,255,0.02)", color: "#E5E7EB" }}>Cancel</button>
      </div>
    </form>
  );
}
