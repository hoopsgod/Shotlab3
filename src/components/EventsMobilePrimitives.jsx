  ...
  return (
    <header className="eventsTitleStage" data-testid={`${role}-events-title-stage`} data-role={role}>
      ...
      <div className="eventsTitleStage__main">
        <h1>Events</h1>
        {onCreate ? <button type="button" className="eventsTitleStage__action" onClick={onCreate} aria-label="Create new event">+ New</button> : null}
      </div>
...
export function EventTimeLocation({ time, location, compact = false }) {
  return (
    <div className={`eventsTimeLocation${compact ? " eventsTimeLocation--compact" : ""}`}>
      <span>{cleanEventValue(time) || "Time TBD"}</span>
      <span>{cleanEventValue(location) || "Location TBD"}</span>
    </div>
  );
}
    ...
    <section className="eventsWeekRail" data-testid="events-week-rail">
      ...
      <div className="eventsWeekRail__days" role="group" aria-label="Select schedule date">
        {DAYS.map((weekday, index) => {
          ...
          return (
            <button key={key} type="button" className="eventsWeekRail__day" data-selected={selected} data-today={key === today} data-has-event={hasEvent} onClick={() => onSelectDate?.(key)} aria-pressed={selected} aria-label={`${date.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" })}${hasEvent ? ", event scheduled" : ""}`}>
              <span>{weekday}</span><strong>{date.getDate()}</strong><i aria-hidden="true" />
            </button>
          );
        })}
      </div>
...
export function EventsMonthPanel({ rows = [], anchorDate, selectedDate, onSelectDate }) {
  ...
  return (
    <details className="eventsMonthPanel" data-testid="events-month-panel">
      <summary><div><span>MONTH</span><strong>{formatMonthLabel(month)}</strong></div><span className="eventsMonthPanel__control">View month <b aria-hidden="true">+</b></span></summary>
      <div className="eventsMonthPanel__body">
        ...
        <div className="eventsMonthPanel__grid">
          {Array.from({ length: 42 }, (_, index) => {
            ...
            const hasEvent = eventDates.has(key);
            const selected = key === selectedDate;
            return <button key={key} type="button" className="eventsMonthPanel__day" data-selected={selected} data-today={key === today} data-has-event={hasEvent} onClick={() => onSelectDate?.(key)} disabled={date.getMonth() !== month.getMonth()} aria-pressed={selected} aria-label={`${date.toLocaleDateString(undefined, { month: "long", day: "numeric" })}${hasEvent ? ", event scheduled" : ""}`}><span>{date.getDate()}</span><i aria-hidden="true" /></button>;
          })}
        </div>
      </div>
    </details>
