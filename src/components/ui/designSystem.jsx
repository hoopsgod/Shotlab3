...
export const DS_TOKENS = {
  ...
  spacing: { xxs: 4, xs: 6, sm: 8, md: 10, lg: 12, xl: 14, xxl: 16, section: 20 },
  shadow: {
...
export function DSMetricCard({ label, value, onClick, style, valueStyle }) {
  return <button type="button" className="ds-metric-card" onClick={onClick} style={{ minHeight: 72, borderRadius: DS_TOKENS.radius.lg, border: "1px solid color-mix(in srgb, var(--accent) 12%, var(--stroke-1))", background: "linear-gradient(165deg, rgba(255,255,255,0.06), rgba(0,0,0,0.32) 62%)", padding: "12px", display: "flex", flexDirection: "column", justifyContent: "center", textAlign: "left", cursor: "pointer", transition: `transform ${DS_TOKENS.motion.fast} ${DS_TOKENS.motion.easeStandard}, border-color ${DS_TOKENS.motion.base} ${DS_TOKENS.motion.easeStandard}, box-shadow ${DS_TOKENS.motion.base} ${DS_TOKENS.motion.easeStandard}`, willChange: "transform", ...style }}><div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.05em", color: "var(--text-tertiary)", textTransform: "uppercase" }}>{label}</div><div style={{ marginTop: 6, fontSize: 24, lineHeight: 1, color: "var(--text-1)", ...valueStyle }}>{value}</div></button>;
}
