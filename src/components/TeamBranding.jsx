  ...
  return (
    <div
      style={{
        ...
        border:`1px solid ${alphaFromHex(primaryColor,0.5)}`,
        boxShadow:`0 0 0 1px ${alphaFromHex(primaryColor,0.08)}, 0 7px 14px rgba(0,0,0,0.3)`,
        display:"grid",
        placeItems:"center",
        overflow:"hidden",
        flexShrink:0,
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:compact?10:14,minWidth:0,position:"relative"}}>
      {showLogo ? <div style={{position:"relative"}}>
        <div style={{position:"absolute",inset:"-8px",borderRadius:"50%",background:`radial-gradient(circle, ${alphaFromHex(branding.primaryColor,0.08)} 0%, transparent 70%)`,filter:"blur(3px)"}}/>
...
export function TeamWatermark({ logoUrl, primaryColor="#C8FF1A", opacity=0.07, size=220 }) {
  ...
  return (
    <div aria-hidden style={{position:"absolute",right:-28,top:-28,width:size,height:size,opacity:Math.min(opacity,0.03),pointerEvents:"none",filter:"grayscale(0.55)"}}>
      <img src={logoUrl} alt="" style={{width:"100%",height:"100%",objectFit:"contain",mixBlendMode:"screen",filter:`drop-shadow(0 0 8px ${alphaFromHex(primaryColor,0.12)})`}}/>
    </div>
  );
}
