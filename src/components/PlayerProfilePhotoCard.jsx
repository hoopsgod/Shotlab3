...
export default function PlayerProfilePhotoCard({player={}}){
 ...
 return <label data-testid="player-profile-photo-card" data-player-photo-state={photo?"set":"empty"} className="playerProfilePhotoCard"><span>{photo?<img src={photo} alt=""/>:name[0]}</span><strong data-testid="player-profile-photo-action">{photo?"Change photo":"Add photo"}</strong><input data-testid="player-profile-photo-input" type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} hidden/>{saved===false&&<small role="alert">Upload failed. Try another image.</small>}</label>
}
