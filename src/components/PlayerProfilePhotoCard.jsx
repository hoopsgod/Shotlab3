...
export default function PlayerProfilePhotoCard({player={}}){
 ...
 return <label data-testid="player-profile-photo-card" className="p7"><span>{photo?<img src={photo} alt=""/>:name[0]}</span><strong>{photo?"Change photo":"Add photo"}</strong><input data-testid="player-profile-photo-input" type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} hidden/>{saved===false&&<small role="alert">Upload failed.</small>}</label>
}
