...
export default function InSeasonPerformanceHub({
  ...
  const visibleDrills = useMemo(() => (Array.isArray(programDrills) ? programDrills : []).filter((drill) => drill?.inSeason !== false && drill?.in_season !== false), [programDrills]);
  ...
  const customCount = visibleDrills.filter((drill) => !isDefaultDrill(drill)).length;
