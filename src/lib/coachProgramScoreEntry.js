...
export function buildCoachVerifiedProgramScoreRow({id,player={},drill={},score,date,teamId,now=Date.now()}={}){const validation=validateCoachProgramScoreEntry({player,drill,score,date});if(!validation.ok)return null;const playerEmail=email(player),programDrillId=drillId(drill),normalizedTeamId=clean(teamId||player?.teamId||player?.team_id);if(!normalizedTeamId)return null;return{id:clean(id)||`coach_program_score_${now}`,email:playerEmail,playerId:playerEmail,player_id:playerEmail,teamId:normalizedTeamId,team_id:normalizedTeamId,name:name(player)||playerEmail,drillId:programDrillId,drill_id:programDrillId,drillName:drillName(drill),score:validation.score,date:clean(date),ts:now,src:"program"}}
