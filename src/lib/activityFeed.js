...
export function deriveActivityFeedItems({ view = "player", user = null, events = [], rsvps = [], shotLogs = [], players = [], scores = [], today = "" }) {
  ...
  scopedShots.forEach((log) => {
    const isSelf = user && log.email === user.email;
    const actor = isSelf ? "You" : formatActor(log);
    items.push({ text: `${actor} logged ${Number(log.made) || 0} At Home Shots`, date: log.date || today, ts: Number(log.ts) || normalizeDateValue(log.date) || 0, type: "shots" });
  });
