...
export const normalizeShotLogRowForApp = (row = {}, options = {}) => {
  ...
  const allowedSyncStates = new Set(["remote_saved", "local_pending", "failed_sync"]);
  const syncState = allowedSyncStates.has(explicitSyncState)
    ? explicitSyncState
    ...
    hideFromLeaderboards:
      typeof row.hideFromLeaderboards === "boolean"
        ...
        : typeof row.hide_from_leaderboards === "boolean"
          ...
          : undefined,
    syncState,
    syncError: cleanText(row.syncError || row.sync_error) || (syncState === "local_pending" && !allowedSyncStates.has(explicitSyncState) ? "legacy_missing_sync_state" : ""),
  };
