...
export const shouldThrowOnEmptyStrictRemotePayload = ({ key, rows, remoteRows, strictRemote, table }) => {
  if (!strictRemote) return false;
  if (!table) return false;
  if (!Array.isArray(rows) || rows.length === 0) return false;
  if (key !== "sl:scores" && key !== "sl:shotlogs") return false;
  return Array.isArray(remoteRows) && remoteRows.length === 0;
};
