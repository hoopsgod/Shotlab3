...
const rowPlayerKey = (row = {}) => lower(row?.playerId ?? row?.player_id ?? row?.id ?? row?.userId ?? row?.user_id);
...
const buildPlayerLookup = (players = [], profiles = []) => {
  ...
  const remember = (record = {}) => {
    ...
    [email, playerId].filter(Boolean).forEach((key) => {
...
export const buildAtHomeLeaderboardRows = ({ scores = [], shotLogs = [], programDrills = [], players = [], profiles = [], limit } = {}) => {
  ...
  const add = (row = {}, amount = 0) => {
    ...
    if (!key) return;
    const matchedPlayer = playerLookup.get(email) || playerLookup.get(playerId) || {};
