...
const isActiveSeasonRosterRow = (row = {}) => {
  ...
  if (status === "inactive" || status === "removed_from_team") return false;
...
export function createSeasonArchive({
  ...
  const rosterSnapshot = deepClone(toArray(activeRosterSource).filter((row) => inTeam(row) && isActiveSeasonRosterRow(row)));
