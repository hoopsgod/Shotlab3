...
export const getProgramLeaderboardRows = (programScores = [], programDrillId, players = [], limit) => {
  ...
  drillScores.forEach((score) => {
    ...
    const existing = byPlayer.get(key) || { playerId: key, player_id: key, email, name: displayName, displayName, player_display_name: displayName, total: 0, score: 0, total_home_shots: 0, drillId: selectedDrillId || score.drillId, drillName: selectedDrillName || score.drillName };
