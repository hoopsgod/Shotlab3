...
function mount(host,ctx){
  ...
  const save=async(state,send=false)=>{
    ...
    if(busy)return;busy=true;paint(send?"Saving private context and delivering assignment…":"Saving…");
    try{
  ...
  paint("Loading follow-up record…");
  Promise.all([loadCoachCoreLoopPlayer(ctx),loadPlayerAssignment(ctx)]).then(([follow,a])=>{
    record=follow.record||null;delivery=a.ok?a.assignment||null:null;
    const parsed=parseCoachResponseNote(follow.record?.note||"");assignment.value=a.assignment?.assignmentText||parsed.assignment||(response?buildNextAssignmentSuggestion(response):"");note.value=parsed.privateNote;
