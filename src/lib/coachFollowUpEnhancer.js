...
const CONTEXT_KEY = "__shotlabCoachFollowUpContext";
...
function CoachFollowUpPanel({ context }) {
  ...
  useEffect(() => {
    ...
    loadCoachFollowUp(context).then((result) => {
      ...
      setNote(result.record?.note || "");
      setError(!result.ok && Boolean(result.error));
      setStatus(result.record ? "Existing follow-up record loaded." : "No follow-up has been recorded.");
    });
    return () => { cancelled = true; };
  }, [context.teamId, context.playerIdentity]);

  const save = async (nextState) => {
    setSaving(true);
    setError(false);
    setStatus("Saving…");
    const result = await saveCoachFollowUp({ ...context, state: nextState, note });
    setSaving(false);
    setRecord(result.record || record);
    setNote(result.record?.note ?? note);
    setError(!result.ok);
  ...
  return React.createElement(
    ...
    React.createElement("div", { className: "coachFollowUpHead" },
      React.createElement("span", null,
        React.createElement("div", { className: "coachFollowUpEyebrow" }, "Coach workflow"),
        React.createElement("h2", { className: "coachFollowUpTitle" }, "Follow-up record")),
    ...
    React.createElement("p", { className: "coachFollowUpCopy" }, state === "completed"
      ...
      : state === "planned"
        ? "This player is on your follow-up list. Add a private coaching note, then confirm completion after you act."
    ...
    React.createElement("label", { className: "coachFollowUpField" },
      ...
      React.createElement("textarea", { value: note, maxLength: 4000, placeholder: "Example: Check in after practice about completing the priority drill.", onChange: (event) => setNote(event.target.value), disabled: saving })),
