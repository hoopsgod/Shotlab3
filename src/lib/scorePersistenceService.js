...
export const pendingId=(row)=>String(row?.id||"").trim(),pendingTeam=(row)=>String(row?.team_id||row?.teamId||"").trim();
...
export function reconcilePendingIds({storage=globalThis?.localStorage,key,requester="",teamId="",localRows=[],remoteRows=[],own=false}={}){
  const remote=Array.isArray(remoteRows)?remoteRows:[],parts=pendingOwner(storage,key,requester);
  if(!parts||(teamId&&parts[1]!==String(teamId||"").trim()))return remote;
  const local=(Array.isArray(localRows)?localRows:[]).filter((row)=>{const rowId=pendingId(row);return parts.includes(rowId,2)&&!remote.some((item)=>pendingId(item)===rowId)&&pendingTeam(row)===parts[1]&&(!own||normalizeIdentity(row?.email||row?.player_email)===parts[0])});
  savePending(storage,key,[parts[0],parts[1],...local.map(pendingId)]);
...
export function createScorePersistenceService({fetchImpl=globalThis?.fetch,storage=globalThis?.localStorage}={}){
  ...
  const upsertScores=async(scores=[])=>{const rows=Array.isArray(scores)?scores:[scores];if(!rows.length)return{ok:true,scores:[],storageMode:"local_only"};const identity=readRequester(storage),teamId=pendingTeam(rows[0]),parts=pendingOwner(storage,KEY,identity);if(identity&&teamId)savePending(storage,KEY,[...new Set([...(parts?.[1]===teamId?parts:[identity,teamId]),...rows.map(pendingId).filter(Boolean)])]);const body=await request("POST",{scores:rows}),confirmed=Array.isArray(body?.scores)?body.scores:[];reconcilePendingScoreRows({storage,requester:identity,localRows:parseStored(storage,"sl:scores",rows),remoteRows:confirmed});return{ok:true,storageMode:signedStorageMode(body),scores:confirmed}};
