...
const identitySetHasAny = (identitySet, values = []) => values.some((value) => identitySet.has(normalizePlayerDataKey(value)));
...
const playerMatchesIdentity = (player = {}, identity = "") => {
  ...
  return Boolean(target && identitySetHasAny(new Set(getRowIdentityKeys(player, ["email", "player_id", "playerId", "id", "user_id", "userId"])), [target]));
};
