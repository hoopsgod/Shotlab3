...
export const getActiveTeamPlayerEmails = (players = [], teamId = "") => (Array.isArray(players) ? players : [])
  .filter((player) => isActiveRosterPlayer(player, teamId))
  .map((player) => normalizePlayerDataEmail(player?.email))
  .filter(Boolean);


export const getActiveTeamPlayerNames = (players = [], teamId = "") => (Array.isArray(players) ? players : [])
  .filter((player) => isActiveRosterPlayer(player, teamId))
