...
export const buildCoachPlayerDevelopmentProfile = ({ player = {}, programDrills = [], programScores = [], scores = [], shotLogs = [], rsvps = [], events = [], scRsvps = [], scLogs = [], teamId = "", today = new Date().toISOString().slice(0, 10) } = {}) => {
  ...
  const homeLeaderboardTotal = buildAtHomeLeaderboardRows({ scores: homeScores, shotLogs: homeShotLogs, programDrills, players: [player], profiles: [player] })
    .filter((row) => [row?.email, row?.player_email, row?.playerId, row?.player_id, row?.id, row?.userId, row?.user_id, row?.profileId, row?.profile_id].map(key).some((candidate) => candidate && homeLeaderboardMatchKeys.has(candidate)))
