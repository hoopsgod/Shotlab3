...
:root{
  ...
  --text-1:rgba(241,245,255,.94);
  --text-2:rgba(223,231,242,.86);
  --text-3:rgba(183,196,214,.82);
...
<p style={{fontFamily:FB,color:MUTED,textAlign:"center",fontSize:14,lineHeight:1.45,margin:"10px auto 0",maxWidth:440}}>Track daily reps, share results with your team, and keep training organized in one place.</p>
...
{["login","register"].map(m=><button key={m} role="tab" aria-selected={mode===m} onClick={()=>{setMode(m);setErr("")}} style={{flex:1,height:40,borderRadius:8,border:"none",cursor:"pointer",fontFamily:FB,fontSize:12,letterSpacing:"0.08em",textTransform:"uppercase",transition:"all .15s ease",background:mode===m?"#F1F5FF":"transparent",color:mode===m?"#0E1420":"#8E95A3",fontWeight:mode===m?700:600}}>{m==="login"?"Sign in":"Register"}</button>)}
    ...
    {mode==="login"&&<>
      ...
      <p style={{fontFamily:FB,color:"#A0A0A0",textAlign:"center",fontSize:13,fontWeight:400,margin:"0 0 18px"}}>Sign in to view your latest sessions, scores, and team updates.</p>
    </>}
...
{resolved.length===0&&pending.length===0&&<Empty t="No duels yet" action="Post a drill score and send your first challenge when you're ready."/>}
{resolved.map(ch=>{
...
{upcoming.length===0&&<Empty variant="lifting" t="No upcoming sessions" action="New strength sessions will appear here as soon as they're scheduled."/>}
...
{upcoming.length===0&&<Empty variant="events" t="No events scheduled yet" subtitle="Team workouts and meetings will appear here once they are posted." cta="LOG SHOTS" ctaVariant="primary" secondaryCta="VIEW PROGRAM DRILLS" secondaryCtaVariant="tertiary" onSecondaryTap={()=>setTab("program")} onTap={()=>setTab("log-drill")}/>} 
...
{isDesktop&&<aside className="insights-panel"><div className="panel-title">COACH INSIGHTS</div><div className="placeholder">Insights modules will appear here as your team logs more activity.</div></aside>}
  </div>;
}
