...
<div style={{flex:1,padding:`${isOverviewTab&&showMiniHeader?"74px":"12px"} var(--shotlab-coach-route-wrapper-gutter, 16px) 104px`,overflowY:"auto",position:"relative",zIndex:showAdd?40:1}}>
  ...
  {tab==="leaderboards"&&<div className="page pageShell fade-up" data-accent="feed" style={shellVars("feed")}><DashboardReturnButton onClick={()=>setTab("feed")} /><CoachPageDashboardHeader eyebrow="Competitive intelligence" title="Leaderboards" summary="See the team standard, the next gap to close, and the category that needs attention." status={`${coachDashboardLeaderboardRows.length} current ranked`} metrics={[{key:"leader",label:"Current Leader",value:coachDashboardLeaderboardRows[0]?.total_home_shots||coachDashboardLeaderboardRows[0]?.total||0,detail:coachDashboardLeaderboardRows[0]?.player_display_name||"No leader yet"},{key:"archives",label:"Archived Seasons",value:seasonArchives.length,detail:"Historical record book"},{key:"view",label:"View",value:"Current",detail:"Switch to all-time below"}]} activeMetric={coachPageMetric} onMetricSelect={setCoachPageMetric} testId="coach-page-dashboard-leaderboards" presentation="context"/><PremiumLeaderboardsHub viewerRole="coach" leaderboardRows={canonicalCoachHomeLeaderboardRows} leaderboardStatus={coachDashboardLeaderboardStatus} leaderboardError={homeShotsLeaderboard?.error||""} leaderboardMode={homeShotsLeaderboard?.mode||"unknown"} onRetryHomeShots={refreshHomeShotsLeaderboard} programScores={safeProgramScores} programDrills={programDrills} players={leaderboardPlayers} teamId={u?.teamId||""} events={safeEvents} rsvps={safeRsvps} scSessions={scSessions} scLogs={safeScLogs} homeScores={scores} shotLogs={shotLogs} seasonArchives={seasonArchives} showHeader={false}/>
  </div>}
