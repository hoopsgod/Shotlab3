...
.pageHeaderPillBrand{border:1px solid transparent;background:var(--page-accent,var(--color-primary,#B6AA94));color:#0B0D10;box-shadow:0 4px 16px rgba(122,145,186,.28);}
...
.lbRow--leader{border-color:rgba(184,255,0,.26)!important;box-shadow:0 10px 24px rgba(0,0,0,.42),0 0 0 1px rgba(63,95,151,.2) inset!important;}
...
<div style={{position:"sticky",top:0,zIndex:10,paddingTop:"max(0px,env(safe-area-inset-top))",height:"calc(72px + max(0px,env(safe-area-inset-top)))",background:TOKENS.BG_BASE,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
  <div style={{height:72,padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
    ...
    <button type="button" className="btn btn--secondary" role="switch" aria-checked={theme==="light"} aria-label="Toggle light theme" onClick={()=>setTheme(v=>v==="dark"?"light":"dark")} style={{minHeight:44,height:44,padding:"0 10px",fontFamily:FB,fontSize:10,letterSpacing:"0.06em",textTransform:"uppercase",marginRight:8}}>{theme==="light"?"Light":"Dark"}</button>
...
<div style={{flex:1,padding:isNarrow?"12px 12px 148px":"14px 16px 148px",overflowY:"auto",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s"}} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
  ...
  {tab==="settings"&&<div className={slideClass} key="settings"><PlayerSettingsPage u={u} onDeleteAccount={deleteAccount} onResetPassword={onResetPassword} theme={theme} setTheme={setTheme}/></div>}
</div>
...
function PlayerSettingsPage({u,onDeleteAccount,onResetPassword,theme,setTheme}){
...
return <SectionContainer className="fade-up">
  ...
  <Card style={{marginBottom:16}}>
    <div style={{fontFamily:FB,color:T.SUB,fontSize:10,letterSpacing:2,fontWeight:700,marginBottom:8}}>APPEARANCE</div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginBottom:12}}>
      <div style={{fontFamily:FB,color:MUTED,fontSize:12}}>Use the premium light palette for brighter surfaces and clearer separation.</div>
      <button type="button" className="btn btn--secondary" role="switch" aria-checked={theme==="light"} onClick={()=>setTheme(v=>v==="dark"?"light":"dark")} style={{minHeight:40,height:40,padding:"0 12px",fontSize:11}}>{theme==="light"?"Light":"Dark"}</button>
    </div>
  </Card>
  <Card style={{marginBottom:16}}>
