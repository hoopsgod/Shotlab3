...
const playerLeaderboardPlayers=useMemo(()=>[...(Array.isArray(players)?players:[]),{email:u?.email||"",name:u?.name||u?.email||"Player",teamId:u?.teamId||"",playerId:u?.playerId||u?.player_id||u?.id||u?.userId||u?.user_id||"",userId:u?.userId||u?.user_id||"",profileId:u?.profileId||u?.profile_id||""}],[players,u]);
const playerDashboardHomeLeaderboardRows=useMemo(()=>buildAtHomeLeaderboardRows({scores,shotLogs,programDrills,players:playerLeaderboardPlayers,limit:3}),[scores,shotLogs,programDrills,playerLeaderboardPlayers]);
...
<div ref={playerScrollRef} className="player-scroll-container" style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px var(--player-scroll-bottom-padding)",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {tab==="leaderboards"&&!active&&<div className={slideClass} key="leaderboards">
    <PremiumLeaderboardsHub viewerRole="player" leaderboardRows={homeShotsLeaderboard?.rows||[]} leaderboardStatus={homeShotsLeaderboard?.status||"idle"} userEmail={u?.email||""} currentUser={u} programScores={teamProgramScores} programDrills={programDrills} players={playerLeaderboardPlayers} teamId={u?.teamId||""} homeScores={homeScores} shotLogs={shotLogs} />
  </div>}
...
const coachRosterPlayers=useMemo(()=>getCoachRosterPlayers({players,playerProfiles,scores,shotLogs,programScores,teamId:u?.teamId}),[players,playerProfiles,scores,shotLogs,programScores,u?.teamId]);
