...
<div style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px 124px",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {tab==="home"&&!active&&<div className={slideClass} key="home">
    ...
    {(()=>{
      ...
      const completionRatio=drills.length>0?Math.round((todayS.length/drills.length)*100):0;
      const streakStatus=streak>=7?"Locked-in consistency":streak>=3?"Rhythm building":"Start your streak";
      const nextMilestoneGap=streak<3?3-streak:streak<7?7-streak:streak<14?14-streak:0;
      const nextMilestoneLabel=nextMilestoneGap>0?`${nextMilestoneGap} day${nextMilestoneGap===1?"":"s"} to next streak tier`:"Streak elite tier reached";
      ...
      return <div style={{marginBottom:24,display:"grid",gap:14}}>
        ...
        <section aria-label="Performance snapshot" style={{display:"grid",gridTemplateColumns:isNarrow?"repeat(2,minmax(0,1fr))":"1.2fr repeat(3,minmax(0,1fr))",gap:8}}>
          <div style={{padding:"10px 12px",borderRadius:12,background:"linear-gradient(150deg, rgba(200,255,0,0.13), rgba(200,255,0,0.02) 72%)",border:"1px solid rgba(200,255,0,0.2)"}}>
            <div style={{fontFamily:FB,color:VOLT,fontSize:10,fontWeight:700,letterSpacing:"0.08em"}}>TODAY COMPLETION</div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginTop:5}}>
              <div style={{fontFamily:FD,color:LIGHT,fontSize:25,lineHeight:1}}>{completionRatio}%</div>
              <div style={{width:42,height:42,borderRadius:"50%",background:`conic-gradient(${VOLT} ${Math.max(2,completionRatio)}%, rgba(255,255,255,0.12) 0)`,display:"grid",placeItems:"center",boxShadow:"inset 0 0 0 1px rgba(255,255,255,0.15)"}}>
                <div style={{width:30,height:30,borderRadius:"50%",background:T.BG,display:"grid",placeItems:"center",fontFamily:FB,color:T.SUB,fontSize:9,fontWeight:700}}>{todayS.length}/{drills.length}</div>
              </div>
            </div>
            <div style={{fontFamily:FB,color:T.SUB,fontSize:10,marginTop:4}}>{todayS.length>=drills.length?"All drills completed today":"Keep stacking reps"}</div>
          </div>
          {[{label:"Streak",value:formatStreakDays(streak),sub:streakStatus,color:CYAN},{label:"Weekly",value:weeklyMakes,sub:`${weeklyPct}% of weekly target`,color:LIGHT},{label:"Trend",value:weeklyMakes>=PLAYER_WEEKLY_SHOT_TARGET/2?"↑ On track":"→ Build",sub:nextMilestoneLabel,color:ORANGE}].map(item=><div key={item.label} style={{padding:"10px 10px",borderRadius:12,background:"rgba(255,255,255,0.03)"}}><div style={{fontFamily:FB,color:T.SUB,fontSize:10,fontWeight:700}}>{item.label}</div><div style={{fontFamily:FD,color:item.color,fontSize:21,marginTop:3,lineHeight:1.1}}>{item.value}</div><div style={{fontFamily:FB,color:T.SUB,fontSize:9,marginTop:2,lineHeight:1.3}}>{item.sub}</div></div>)}
        </section>
        {(todaysMakes>0||todayS.length>0)&&<section aria-label="Momentum feedback" style={{padding:"11px 12px",borderRadius:12,border:`1px solid ${BORDER_CLR}`,background:"linear-gradient(145deg, rgba(0,176,255,0.10), rgba(0,0,0,0.16))"}}>
          <div style={{fontFamily:FB,color:CYAN,fontSize:10,fontWeight:700,letterSpacing:"0.09em"}}>MOMENTUM UPDATE</div>
          <div style={{fontFamily:FD,color:LIGHT,fontSize:18,letterSpacing:1,marginTop:4,lineHeight:1.15}}>{todayS.length>0?`Session logged · ${todaysMakes} makes today`:`${todaysMakes} makes recorded today`}</div>
          <div style={{fontFamily:FB,color:T.SUB,fontSize:11,marginTop:4,lineHeight:1.45}}>{weeklyMakes>=weeklyGoal?"Weekly target reached. Keep the standard high.":`Need ${Math.max(0,weeklyGoal-weeklyMakes)} more makes to close the weekly target.`}</div>
        </section>}
