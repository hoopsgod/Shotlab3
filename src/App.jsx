...
<div style={{flex:1,padding:"16px 20px 110px",overflowY:"auto",position:"relative",zIndex:1}}>
  ...
  {/** DRILLS */}
  {tab==="drills"&&!editD&&<div className="page pageShell fade-up" data-accent="drills" id="coach-drills-management" style={shellVars("drills")}><PageHeader title="DRILLS" subtitle="Build and launch shooting work" accent={PAGE_ACCENTS.drills.accent} icon={<DrillIcon type="ft" size={22} color={PAGE_ACCENTS.drills.accent}/>} rightSlot={<button className="pageHeaderPill" onClick={()=>setShowAdd(true)}>Add</button>} /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.drills.accent,fontSize:12,letterSpacing:2}}>QUICK START DRILL</div><div style={{fontFamily:FB,color:MUTED,fontSize:10}}>{drills.length} total drills ready to start</div></div><button className="pageHeaderPill" onClick={()=>setShowAdd(true)}>Start</button></div><div className="drillsMetrics"><div className="heroStat"><div className="heroStatVal">{drills.length}</div><div className="heroStatLbl">ACTIVE</div></div><div className="heroStat"><div className="heroStatVal">{programDrills.length}</div><div className="heroStatLbl">PROGRAM</div></div></div><button className="pageHeaderPill" onClick={()=>document.getElementById("coach-drills-management")?.scrollIntoView({behavior:"smooth"})}>Manage Drills</button></div>
...
return <nav className="bottom-nav" role="navigation" aria-label="Main navigation" style={{position:"fixed",left:0,right:0,bottom:0,display:"flex",justifyContent:"space-evenly",alignItems:"center",height:64,paddingBottom:"env(safe-area-inset-bottom)",background:"#1E1E1E",borderTop:"1px solid #242424",zIndex:20}}>{items.map(t=>{const a=active===t.k;
