...
const [leaderboardCategory,setLeaderboardCategory]=useState("home_shots");
...
<div ref={playerScrollRef} style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px 124px",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {tab==="leaderboards"&&!active&&<div className={slideClass} key="leaderboards">
    ...
    <section style={{display:"grid",gridTemplateColumns:isNarrow?"1fr":"repeat(2,minmax(0,1fr))",gap:8,marginBottom:10}}>
      {[{key:"home_shots",label:"At-Home Shots"},{key:"event_participation",label:"Events Attended"},{key:"strength_conditioning_participation",label:"Strength & Conditioning"},{key:"drill_shots",label:"Coach Custom Drills"}].map(item=>{const active=leaderboardCategory===item.key;return <button type="button" aria-selected={active} key={item.label} onClick={()=>setLeaderboardCategory(item.key)} style={{minHeight:42,padding:"9px 10px",borderRadius:999,border:active?"1px solid rgba(200,255,0,0.52)":"1px solid rgba(255,255,255,0.16)",background:active?"rgba(200,255,0,0.12)":"rgba(255,255,255,0.03)",boxShadow:active?"0 0 18px rgba(200,255,0,0.2)":"none",fontFamily:FB,fontSize:11,fontWeight:800,color:active?VOLT:LIGHT,textAlign:"center",cursor:"pointer"}}>{item.label}</button>;})}
    </section>
    {leaderboardCategory==="home_shots" ? <>
