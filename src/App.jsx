...
import { buildProgramDrillLeaderboardRows, buildProgramScoreRow, validateProgramDrillScore } from "./lib/programDrillScoring.js";
...
const scopedProgramScores=programScores.filter(s=>s.teamId===user?.teamId);
...
const teamProgramScores=programScoresFromDb;
const programScores=useMemo(()=>teamProgramScores.filter(s=>s.email===u.email),[teamProgramScores,u]);
  ...
  {tab==="duels"&&!active&&<div className="fade-up">
    ...
    {programSessionBlocks.grouped.map((block,blockIndex)=><div key={block.phase} style={{marginBottom:14,background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:16,padding:"12px 12px 4px"}}>
      ...
      {block.drills.map(d=>{const done=todayProgramScores.find(s=>s.drillId===d.id);const isPriority=programSessionBlocks.nextPriority===d.id;
        const leaderboardRows=buildProgramDrillLeaderboardRows({scores:teamProgramScores,drill:d,players,limit:3});
...
const my=useMemo(()=>scores.filter(s=>s.email===u.email),[scores,u]);
const homeScores=useMemo(()=>my.filter(s=>s.src==="home"||!s.src),[my]);
const programScores=useMemo(()=>programScoresFromDb.filter(s=>s.email===u.email),[programScoresFromDb,u]);
