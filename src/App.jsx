...
const[restoredSessionEmail,setRestoredSessionEmail]=useState("");
...
const activeUser=user||{email:restoredSessionEmail,role:"player",isCoach:false,teamId:""};
if(!activeUser?.email)return{ok:false,error:"No active account."};
const e=String(activeUser.email||"").trim().toLowerCase();
const isSelf=(row={})=>String(row?.playerId||row?.player_id||row?.email||row?.userId||row?.user_id||"").trim().toLowerCase()===e;
try{
if(isDemoMode()||isDemoAccount(activeUser))await cleanupDemoPlayerSessionData(activeUser);
...
if(!user&&!restoredSessionEmail)return <StaticLegalPage pageKey="delete-account"/>;
