...
if(isJoinConsumeActive)return{ok:false,retryable:true,err:"Join already in progress."};
if(!currentUserEmail){
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeAttemptBlocked:"no",consumeAttemptBlockedReason:"",consumeFetchStarted:"yes",consumeFetchFinished:"no",consumeGuardCleared:"no",consumeGuardClearReason:"",consumeInFlightAgeMs:0}}));
try{
timeoutId=setTimeout(()=>abortController.abort("consume_fetch_timeout"),15_000);
const res=await fetch(consumeEndpoint,{method:"POST",headers:{"Content-Type":"application/json","x-user-id":actor.email},signal:abortController.signal,body:JSON.stringify({join_context_token:resolvedContext.joinContextToken,subject_key:subject,client_request_id:clientRequestId||genId("join")})});
const body=await res.json().catch(()=>({}));
if(!res.ok){
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeFetchFinished:"yes"}}));
setIsJoinConsumeActive(false);
}
},[pendingJoinContext,normalizeStoredInviteContext,readInviteContextFromStorage,savePendingJoinContext,mapConsumeDiagnostic,isJoinConsumeActive]);
