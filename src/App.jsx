...
{board.map((p,i)=>{
  ...
  if(isLeader) return <div key={p.email} className="podium-glow lbRow listRow" style={{"--pod-c":accentColor,background:"rgba(10, 12, 14, 0.94)",backgroundClip:"padding-box",border:`2px solid ${accentColor}33`,position:"relative",overflow:"hidden"}}>
    ...
    <div className="lbMain listRowText">
      <div className="lbName listRowTitle">{p.name.toUpperCase()}{isMe&&<span className="chip chip--accent" style={{height:22,padding:"0 8px",fontFamily:FB,fontSize:10,color:BG,background:accentColor,borderColor:`${accentColor}44`,marginLeft:6,letterSpacing:1}}>YOU</span>}</div>
  ...
  return <div key={p.email} className="lbRow listRow" style={{background:isMe?"rgba(10, 12, 14, 0.94)":rowBg,backgroundClip:"padding-box",border:isMe?`2px solid ${accentColor}44`:`1px solid ${BORDER_CLR}`,position:"relative",overflow:"hidden"}}>
    ...
    <div className="lbMain listRowText">
      <div className="lbName listRowTitle" style={{fontSize:13,fontWeight:isMe?700:600}}>{p.name.toUpperCase()}{isMe&&<span className="chip chip--accent" style={{height:20,padding:"0 7px",fontFamily:FB,fontSize:9,color:BG,background:accentColor,borderColor:`${accentColor}44`,marginLeft:6,letterSpacing:1,verticalAlign:"middle"}}>YOU</span>}</div>
  ...
  {!nextTier
    ...
    :<div style={{marginTop:8}}>
      ...
      <div style={{marginTop:8}}>
        <div
          className="progress"
          style={{"--progress":`${rankBarPct}%`}}
          role="progressbar"
          aria-label="Attendance rank progress"
          aria-valuemin={0}
          aria-valuemax={100}
          aria-valuenow={rankBarPct}
        >
          <div
            className="progress__fill"
            style={{
              boxShadow:rankBarPulse?"0 6px 18px rgba(184,255,0,.2)":"0 6px 18px rgba(184,255,0,.12)",
...
{upcoming.map(ev=>{const evR=rsvps.filter(r=>r.eventId===ev.id);const going=evR.some(r=>r.email===user.email);const exp=expanded===ev.id;
  return <div key={ev.id}>
    <div className="ch eventCard listRow" style={{width:"100%",background:`linear-gradient(135deg,${CARD_BG},#141414)`,border:`1px solid ${going?VOLT+"33":BORDER_CLR}`,borderRadius:exp?"16px 16px 0 0":16,textAlign:"left",position:"relative",overflow:"hidden",cursor:"pointer"}} onClick={()=>setExpanded(exp?null:ev.id)}>
      ...
      <div className="listRowLeft">
      <div className="eventIcon" style={{background:BG,border:`1px solid ${BORDER_CLR}`,flexShrink:0}}><EventIcon type={ev.type} size={20} color={going?CYAN:MUTED}/></div>
      ...
      <button onClick={(e)=>{e.stopPropagation();toggleRsvp(ev.id)}} className={`btn btn-secondary btn--dense eventCta btn-primary chip ${going?"chip--accent":""}`} style={{fontFamily:FD,height:44,minHeight:44,boxShadow:"none"}}>
        {going?<><svg width="18" height="18" viewBox="0 0 20 20"><path d="M5 10l4 4 6-7" stroke={BG} strokeWidth="2.3" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>I'M GOING</>:"RSVP →"}
      </button>
    </div>
    {exp&&<div className="fade-up" style={{background:SURFACE,borderRadius:"0 0 16px 16px",padding:"16px 20px",border:`1px solid ${BORDER_CLR}`,borderTop:"none"}}>
      ...
      <button className={`btn btn-secondary btn--dense btn-v eventCta chip ${going?"chip--accent":""}`} onClick={()=>toggleRsvp(ev.id)} style={{marginBottom:14}}>
      ...
      {evR.length>0&&<div><div style={{fontFamily:FB,color:T.SUB,fontSize:10,letterSpacing:2,marginBottom:8,fontWeight:600}}>WHO'S GOING</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>
        {evR.map((r,i)=>{const tr=getTier(rsvps.filter(rr=>rr.email===r.email).length);return <div key={i} className="chip" style={{background:CARD_BG,borderColor:BORDER_CLR}}><Av n={r.name} sz={22} email={r.email}/><span style={{fontFamily:FB,color:LIGHT,fontSize:11,fontWeight:600}}>{r.name}</span>{tr.min>=2&&<span className="chip chip--accent" style={{height:22,padding:"0 8px",fontSize:10,color:tr.color,background:tr.bg,borderColor:`${tr.color}44`}}>{tr.name}</span>}</div>})}
      </div></div>}
    </div>}
  </div>})}
</div>
...
<div className="navIndicator" aria-hidden="true"/>
