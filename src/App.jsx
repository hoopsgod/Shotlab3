...
const [dataDebug,setDataDebug]=useState({join:{enteredCode:"",status:"idle",lookupCount:null,update:"idle",error:""},leaderboard:{endpoint:"",httpStatus:null,errorCode:"",resultCount:null,isEmpty:false}});
...
const normalizedCode=normalizeJoin(code).replace(/[-\s]+/g,"");
const localMatches=teams.filter(t=>normalizeJoin(t?.joinCode||"").replace(/[-\s]+/g,"")===normalizedCode);
setDataDebug(prev=>({...prev,join:{...prev.join,enteredCode:normalizedCode,status:"lookup",lookupCount:localMatches.length,update:"idle",error:""}}));
let resolvedTeamId=null;
if(localMatches.length>0){
resolvedTeamId=localMatches[0].id;
setDataDebug(prev=>({...prev,join:{...prev.join,status:"local_match",lookupCount:localMatches.length}}));
}else{
const ctx=await startJoinContext(normalizedCode,user.email);
if(!ctx.ok){
setDataDebug(prev=>({...prev,join:{...prev.join,status:"failed",error:ctx.err||"invalid_code"}}));
return{ok:false,err:ctx.err||"Invalid team code."};
}
const joined=await consumeJoinContext(user);
if(!joined.ok){
setDataDebug(prev=>({...prev,join:{...prev.join,status:"failed",error:joined.err||"join_failed"}}));
return{ok:false,err:joined.err||"Could not join team."};
}
resolvedTeamId=joined.teamId;
setDataDebug(prev=>({...prev,join:{...prev.join,status:"backend_match",lookupCount:0}}));
}
