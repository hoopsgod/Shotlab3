...
.eventsList{display:flex;flex-direction:column;gap:14px;}.eventCard{display:flex;flex-direction:column;gap:14px;padding:18px;border-radius:20px;}.eventTop{display:flex;align-items:flex-start;gap:12px;}.eventIcon{width:44px;height:44px;display:grid;place-items:center;border-radius:14px;flex-shrink:0;}.eventMain{min-width:0;flex:1;}.eventTitle{font-size:20px;font-weight:800;letter-spacing:.03em;line-height:1.1;margin:0;text-transform:uppercase;text-wrap:balance;}.eventDesc{margin-top:8px;font-size:12px;line-height:1.4;color:var(--text-secondary);}.eventMeta{margin-top:8px;font-size:12px;line-height:1.25;opacity:.9;display:flex;flex-wrap:wrap;gap:6px 12px;}.eventMetaChip{display:inline-flex;align-items:center;gap:4px;padding:4px 8px;border-radius:999px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);}.eventSide{display:flex;flex-direction:column;align-items:flex-end;gap:6px;min-width:72px;padding-top:4px;}.eventCta{height:42px;padding:0 16px;border-radius:12px;font-size:13px;letter-spacing:.08em;align-self:flex-end;width:auto!important;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.16);box-shadow:none;}.eventCard button.eventCta,.eventCard .eventCta button{width:auto!important;}
  ...
  return <div key={ev.id}>
    <div className="ch eventCard" style={{width:"100%",background:`linear-gradient(135deg,${CARD_BG},#141922)`,border:`1px solid ${going?VOLT+"33":BORDER_CLR}`,borderRadius:exp?"20px 20px 0 0":20,textAlign:"left",position:"relative",overflow:"hidden",cursor:"pointer"}} onClick={()=>setExpanded(exp?null:ev.id)}>
      ...
      <div className="eventTop">
        <div className="eventIcon" style={{background:BG,border:`1px solid ${BORDER_CLR}`}}><EventIcon type={ev.type} size={24} color={going?CYAN:MUTED}/></div>
        <div className="eventMain" style={{fontFamily:FB,color:MUTED}}>
          ...
          <div className="eventMeta">
            <span className="eventMetaChip" style={{color:VOLT,fontWeight:700}}>{formatEventDate(ev.date)}</span>
            <span className="eventMetaChip">🕒 {ev.time}</span>
            {relativeLabel&&<span className="eventMetaChip" style={{color:CYAN,fontWeight:600}}>{relativeLabel}</span>}
          </div>
          <p className="eventDesc">{ev.desc}</p>
          <div className="eventMeta"><span className="eventMetaChip" style={{color:T.SUB}}>📍 {ev.location}</span></div>
        </div>
      ...
      <button onClick={(e)=>{e.stopPropagation();toggleRsvp(ev.id)}} className="btn btn-secondary btn--dense eventCta btn-primary" style={{fontFamily:FD,minHeight:42,boxShadow:"none"}}>
