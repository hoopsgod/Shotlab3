...
.listRow{display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border-bottom:1px solid var(--stroke-1);}
.listRow:last-child{border-bottom:none;}
.listRowLead{display:flex;align-items:center;gap:12px;min-width:0;flex:1;}
.listRowBody{min-width:0;}
.cardTitle{font-family:${FD};font-size:14px;font-weight:700;letter-spacing:.04em;line-height:1.15;margin:0;color:${LIGHT};text-transform:none;}
.meta{margin-top:5px;font-family:${FB};font-size:11px;line-height:1.25;color:${T.SUB};opacity:.9;}
.metricValue{font-family:${FD};font-variant-numeric:tabular-nums;font-weight:800;letter-spacing:.02em;line-height:1;}
.eventsList{display:flex;flex-direction:column;gap:16px;}.eventCard{display:grid;grid-template-columns:52px 1fr auto;align-items:center;gap:14px;padding:16px;border-radius:16px;}.eventIcon{width:44px;height:44px;display:grid;place-items:center;border-radius:14px;}.eventMain{min-width:0;}.eventTitle{font-size:16px;font-weight:700;letter-spacing:.04em;line-height:1.15;margin:0;text-transform:none;}.eventMeta{margin-top:6px;font-size:12px;line-height:1.25;opacity:.86;display:flex;flex-wrap:wrap;gap:8px;color:${T.SUB};}.eventSide{display:flex;flex-direction:column;align-items:flex-end;gap:6px;min-width:64px;}.eventCta{height:44px;padding:0 12px;border-radius:12px;font-size:12px;letter-spacing:.08em;align-self:end;width:auto!important;max-width:160px;grid-column:2 / 4;justify-self:end;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.16);box-shadow:0 0 10px rgba(200,255,26,.08);}.eventCard button.eventCta,.eventCard .eventCta button{width:auto!important;}
...
.lbRow{display:grid;grid-template-columns:40px 52px minmax(0,1fr) 88px;align-items:center;gap:12px;padding:14px 16px;border-radius:16px;border-bottom:1px solid var(--stroke-1);}
...
.lbMain{min-width:0;}
.lbName{font-size:14px;font-weight:800;letter-spacing:.03em;line-height:1.15;margin:0;font-family:${FD};color:${LIGHT};}
.lbMeta{margin-top:6px;font-size:11px;line-height:1.2;opacity:.9;font-family:${FB};color:${T.SUB};}
.lbMetric{justify-self:end;text-align:right;font-size:20px;font-weight:800;font-variant-numeric:tabular-nums;letter-spacing:.02em;font-family:${FD};}
...
{resolved.map(ch=>{
  ...
  return <div key={ch.id+"-"+ch.ts} className="listRow" style={{background:CARD_BG,borderRadius:14,marginBottom:6,border:`1px solid ${isPending?ORANGE+"22":BORDER_CLR}`}}>
    <div className="listRowLead"><div style={{width:40,height:40,borderRadius:12,background:resultColor+"12",border:`1px solid ${resultColor}33`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
    ...
    <div className="listRowBody">
      <div className="cardTitle" style={{fontSize:13}}>{isMine?"YOU":"YOU"} vs {oppName.toUpperCase()}</div>
      <div className="meta" style={{fontSize:10}}>{ch.drillName} &#183; {isPending?<span style={{color:ORANGE,opacity:.65}}>Waiting for response</span>:<span style={{color:resultColor,fontWeight:600,opacity:.88}}>{resultText}</span>}</div>
    </div>
    </div><div style={{textAlign:"right",flexShrink:0,alignSelf:"center",minWidth:86}}>
      {isPending?<div className="metricValue" style={{color:ORANGE,fontSize:18}}>{ch.score}<span style={{color:MUTED,fontSize:11}}>/{ch.max}</span></div>
      :<><div className="metricValue" style={{color:won?VOLT:"#FF4545",fontSize:16}}>{myScore||"-"}<span style={{color:MUTED,fontSize:10}}> v </span><span style={{color:won?"#FF4545":VOLT}}>{oppScore}</span></div>
  ...
  return <div key={s.id} style={{marginBottom:12}}>
    <button onClick={()=>setExpanded(exp?null:s.id)} className="ch" style={{width:"100%",background:`linear-gradient(135deg,${CARD_BG},#141414)`,border:`1px solid ${going?SC_COLOR+"33":BORDER_CLR}`,borderRadius:exp?"16px 16px 0 0":16,padding:"18px 20px",textAlign:"left",cursor:"pointer",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",gap:14}}>
        ...
        <div style={{textAlign:"right",flexShrink:0,alignSelf:"center",minWidth:86}}><div style={{fontFamily:FD,color:sr.length>0?SC_COLOR:MUTED,fontSize:20}}>{sr.length}</div><div style={{fontFamily:FB,color:MUTED,fontSize:9,letterSpacing:1}}>GOING</div></div>
      </div>
    </button>
...
{board.map((p,i)=>{
  ...
  return <div key={p.email} className="lbRow" style={{background:isMe?"rgba(10, 12, 14, 0.94)":rowBg,backgroundClip:"padding-box",border:isMe?`2px solid ${accentColor}44`:`1px solid ${BORDER_CLR}`,position:"relative",overflow:"hidden"}}>
    ...
    <div className="lbMain">
      <div className="lbName cardTitle" style={{fontSize:13,fontWeight:isMe?700:600}}>{p.name.toUpperCase()}{isMe&&<span style={{fontFamily:FB,fontSize:8,fontWeight:700,padding:"1px 5px",borderRadius:4,background:accentColor,color:BG,marginLeft:6,letterSpacing:1,verticalAlign:"middle"}}>YOU</span>}</div>
      {isHome&&p.lastDate&&<div className="lbMeta meta" style={{fontSize:9,marginTop:2}}>{p.lastDate}</div>}
...
{upcoming.map(ev=>{const evR=rsvps.filter(r=>r.eventId===ev.id);const going=evR.some(r=>r.email===user.email);const exp=expanded===ev.id;
  return <div key={ev.id}>
    <div className="ch eventCard" style={{width:"100%",background:`linear-gradient(135deg,${CARD_BG},#141414)`,border:`1px solid ${going?VOLT+"33":BORDER_CLR}`,borderRadius:exp?"16px 16px 0 0":16,textAlign:"left",position:"relative",overflow:"hidden",cursor:"pointer"}} onClick={()=>setExpanded(exp?null:ev.id)}>
      ...
      <div className="eventMain" style={{flex:1}}><div className="eventTitle cardTitle" style={{color:LIGHT}}>{ev.title}</div><div className="eventMeta meta"><span style={{color:VOLT,fontWeight:700}}>{ev.date}</span><span>{ev.time}</span><span>{ev.location}</span></div></div>
      <div className="eventSide" style={{flexShrink:0}}>
        <div style={{textAlign:"right"}}><div className="metricValue" style={{color:evR.length>0?VOLT:LIGHT,fontSize:20}}>{evR.length}</div><div className="meta" style={{marginTop:2,fontSize:9,letterSpacing:1}}>GOING</div></div>
      </div>
      {/* Inline quick-RSVP pill */}
      <button onClick={(e)=>{e.stopPropagation();toggleRsvp(ev.id)}} className="btn btn--secondary eventCta" style={{fontFamily:FD,boxShadow:going?"0 0 12px rgba(200,255,26,.14)":"0 0 8px rgba(200,255,26,.06)"}}>
