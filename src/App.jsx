...
.premiumSummaryPanel{position:relative;overflow:hidden;border-radius:18px;padding:16px;border:1px solid color-mix(in srgb,var(--pageAccent) 22%, var(--surface-border));background:linear-gradient(145deg,color-mix(in srgb,var(--surface-panel-strong) 90%, #06080d) 0%,color-mix(in srgb,var(--surface-panel) 82%, #0f1520) 58%,#0d121a 100%);box-shadow:inset 0 1px 0 rgba(255,255,255,.04),0 18px 34px rgba(0,0,0,.34);}
.premiumSummaryPanel::after{content:"";position:absolute;inset:-40% -20% auto auto;width:220px;height:220px;background:radial-gradient(circle,color-mix(in srgb,var(--pageAccent) 24%, transparent) 0%,transparent 68%);pointer-events:none;}
.premiumSummaryTop{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;position:relative;z-index:1;}
.premiumSummaryBadge{width:38px;height:38px;border-radius:11px;border:1px solid color-mix(in srgb,var(--pageAccent) 34%, transparent);background:color-mix(in srgb,var(--pageAccent) 10%, var(--surface-panel));display:flex;align-items:center;justify-content:center;color:var(--pageAccent);box-shadow:0 0 0 1px rgba(255,255,255,.03) inset;}
.premiumSummaryMeta{font-family:${FB};font-size:10px;color:var(--text-2);margin-top:4px;letter-spacing:0.04em;}
.premiumSummaryActions{display:flex;gap:8px;margin-top:14px;flex-wrap:wrap;position:relative;z-index:1;}
.premiumStatGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:14px;position:relative;z-index:1;}
.premiumStatTile{padding:10px 8px;border-radius:12px;background:linear-gradient(180deg,rgba(255,255,255,.05),rgba(255,255,255,.015));border:1px solid rgba(255,255,255,.08);text-align:center;}
.premiumStatTile .heroStatVal{color:var(--pageAccent);font-size:22px;text-shadow:0 0 14px color-mix(in srgb,var(--pageAccent) 20%, transparent);}
.premiumStatTile .heroStatLbl{color:var(--text-2);margin-top:4px;}
...
const logout=async()=>{trackEvent("auth_logout");if(SUPABASE_AUTH_ENABLED)await supabase.auth.signOut();legacyAuthSecretRef.current={email:"",password:""};setUser(null);setView("auth");DB.set("sl:session",null)};
...
<div style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px 124px",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {tab==="home"&&!active&&<div className={slideClass} key="home">
    ...
    {(()=>{
      ...
      const missionCtaLabel=hasAnyShots?"Start Workout":"Log Shots";
        ...
        <section aria-label="Next 7 days events intelligence" style={{padding:isNarrow?"13px":"15px",borderRadius:18,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.12)",boxShadow:"0 8px 22px rgba(0,0,0,0.18)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:9}}>
            <div>
              ...
              <div style={{fontFamily:FB,color:T.SUB,fontSize:11,marginTop:2}}>Your practices, games, workouts, and RSVP priorities.</div>
            </div>
            <button className="pageHeaderPill" onClick={()=>switchTab("program")}>Open Events</button>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:9}}>
            <span style={{fontFamily:FB,fontSize:10,color:LIGHT,padding:"3px 8px",borderRadius:999,border:"1px solid rgba(255,255,255,0.24)"}}>Upcoming {upcomingWeekEvents.length}</span>
            <span style={{fontFamily:FB,fontSize:10,color:VOLT,padding:"3px 8px",borderRadius:999,border:"1px solid rgba(200,255,0,0.45)"}}>Confirmed {weekConfirmedCount}</span>
          ...
          <div style={{display:"grid",gap:7}}>
            {upcomingWeekEvents.length===0?<div style={{padding:"11px 12px",borderRadius:11,border:"1px dashed rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.015)"}}><div style={{fontFamily:FB,color:LIGHT,fontSize:11,fontWeight:700}}>Schedule window is clear.</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10,marginTop:3}}>Use this opening to recover, prep, and start your cycle strong before the next team block.</div></div>:upcomingWeekEvents.map((ev)=>{
              ...
              const day=dayLabel(ev.date);
              const isUrgent=day==="TODAY"||day==="TOMORROW";
              return <button key={ev.id} type="button" onClick={()=>switchTab("program")} style={{display:"grid",gridTemplateColumns:"auto 1fr auto",gap:8,alignItems:"center",textAlign:"left",padding:isNarrow?"9px 9px":"9px 10px",borderRadius:11,border:`1px solid ${isUrgent?"rgba(200,255,0,0.32)":"rgba(255,255,255,0.12)"}`,background:isUrgent?"rgba(200,255,0,0.08)":"rgba(12,14,18,0.58)",cursor:"pointer"}}>
                ...
                <div style={{minWidth:0}}><div style={{fontFamily:FB,color:LIGHT,fontSize:11,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{ev.title}</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10,marginTop:2}}>{day} · {ev.time||"TBD"}</div></div>
...
function Coach({u,team,regenerateJoinCode,addRosterPlayer,removeRosterPlayer,playerProfiles,drills,programDrills,scores,players,updateDrill,addDrill,removeDrill,addProgramDrill,removeProgramDrill,events,rsvps,addEvent,removeEvent,removeRsvp,addRsvp,scSessions,scRsvps,scLogs=[],addScSession,removeScSession,shotLogs,coachPriorities,onSaveCoachPriorities,logout,deleteAccount,openTeamBranding,coachTextSize="standard",demoSettingsBusy=false,onLoadDemoData,onClearDemoData,homeShotsLeaderboard,leaderboardScope,onLeaderboardScopeChange,refreshHomeShotsLeaderboard}){
