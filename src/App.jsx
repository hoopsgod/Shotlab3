...
<div style={{position:"sticky",top:0,zIndex:10,paddingTop:"max(0px,env(safe-area-inset-top))",background:TOKENS.BG_BASE,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
  <div style={{height:76,padding:"0 16px",display:"grid",gridTemplateColumns:"44px minmax(0,1fr) 44px",alignItems:"center",columnGap:12}}>
    {tab!=="home"?<button className="top-header-back" aria-label="Back to dashboard" onClick={()=>switchTab("home")}>← HOME</button>:<span className="top-header-back--spacer" aria-hidden="true"/>}
    <div style={{display:"flex",alignItems:"center",minWidth:0,gap:12}}>
      <button aria-label="Open profile" onClick={()=>switchTab("profile")} style={{width:40,height:40,borderRadius:"50%",background:"#1E1E1E",border:"1.5px solid #C8FF00",boxShadow:u.isCoach?"0 0 0 4px rgba(200, 255, 0, 0.15)":"none",color:"#FFFFFF",fontSize:15,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",padding:0,cursor:"pointer",fontFamily:FB,flexShrink:0}}>{(u.name||"?")[0].toUpperCase()}</button>
      <div style={{display:"flex",flexDirection:"column",justifyContent:"center",minWidth:0,maxWidth:"100%"}}>
        <div style={{fontFamily:FB,color:"#FFFFFF",fontSize:17,fontWeight:700,lineHeight:1.1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{u.name}</div>
        <div style={{fontFamily:FB,color:"rgba(255,255,255,0.58)",fontSize:11,fontWeight:500,lineHeight:1.2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",marginTop:2}}>{u.isCoach?"Your program awaits":"Today's mission awaits"}</div>
      </div>
    </div>
    <button className="top-header-btn" aria-label="Settings" title="Settings" onClick={()=>switchTab("profile")}>
...
return <nav className="bottom-nav" role="navigation" aria-label="Main navigation" style={{"--nav-accent":navAccent,position:"fixed",left:0,right:0,bottom:0,display:"flex",justifyContent:"space-evenly",alignItems:"center",height:70,padding:"0 6px env(safe-area-inset-bottom)",background:"var(--surface-1)",borderTop:"1px solid var(--stroke-1)",zIndex:20}}>{items.map(t=>{const a=active===t.k;
const tabAccent="var(--accent)";
return <button key={t.k} aria-label={t.l} aria-current={a?"page":undefined} className={`tab ${a?"is-active active":""}`} onClick={()=>onChange(t.k)} style={{"--tab-accent":tabAccent,flex:1,minWidth:56,minHeight:56,height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:5,padding:"10px 6px 8px",position:"relative",background:"none",border:"none",cursor:"pointer",transition:"color 150ms ease-out",outlineOffset:2}}>
<div className="tab-icon" style={{position:"relative"}}>{t.svg}</div>
<div className="tab-label" style={{fontFamily:FB,whiteSpace:"nowrap"}}>{t.l}</div>
</button>})}</nav>}
