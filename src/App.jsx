...
setDataDebug(prev=>({...prev,auth:{...prev.auth,signupHttpStatus:200,signupCode:"legacy_success",signupMessage:"",providerHint:"local_password_hash",sessionPresent:"yes"}}));
const hashed=hashPw(password);
const np=upsertPlayerProfile(players,{email:normalizedEmail,name,password:hashed,role,teamId:null,hideFromLeaderboards:false});
const seededDrills=mergeDefaultDrills(drills,DRILLS_INIT);
const seededProgramDrills=mergeDefaultDrills(programDrills,PROGRAM_DRILLS_INIT);
...
const pLocal=players.find(p=>normalizeEmail(p.email)===normalizedEmail);
if(!pLocal||pLocal.password!==hashPw(password)){
setDataDebug(prev=>({...prev,auth:{...prev.auth,loginHttpStatus:401,loginCode:"invalid_credentials",loginMessage:"Invalid email or password",providerHint:"local_password_hash"}}));
return{ok:false,err:"Invalid email or password"};
}
setDataDebug(prev=>({...prev,auth:{...prev.auth,loginHttpStatus:200,loginCode:"legacy_success",loginMessage:"",providerHint:"local_password_hash",sessionPresent:"yes"}}));
}
