...
import { createSeasonArchive } from "./lib/seasonArchive.js";
  ...
  {tab==="duels"&&!active&&<div className="fade-up">
    ...
    {programSessionBlocks.grouped.map((block,blockIndex)=><div key={block.phase} style={{marginBottom:14,background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:16,padding:"12px 12px 4px"}}>
      ...
      {block.drills.map(d=>{const done=todayProgramScores.find(s=>s.drillId===d.id);const isPriority=programSessionBlocks.nextPriority===d.id;
        ...
        return <button key={d.id} className="ch" onClick={()=>setActive(d)} style={{width:"100%",display:"flex",alignItems:"center",gap:14,background:"#131821",border:`1px solid ${isPriority&&!done?CYAN+"66":done?CYAN+"20":BORDER_CLR}`,borderRadius:14,padding:"12px",marginBottom:8,cursor:"pointer",textAlign:"left",opacity:1}}>
...
const statusColor=profile.statusLabel==="Active"?VOLT:profile.statusLabel==="Needs Follow-Up"?ORANGE:"#FF6969";
const statCard=(label,value,accent=VOLT)=><div style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:"12px 10px"}}><div style={{fontFamily:FD,color:accent,fontSize:24,lineHeight:1}}>{value}</div><div style={{fontFamily:FB,color:MUTED,fontSize:8,fontWeight:800,letterSpacing:"0.11em",textTransform:"uppercase",marginTop:5}}>{label}</div></div>;
return <div data-testid="coach-player-development-profile" style={{display:"grid",gap:12,marginBottom:18}}>
  <div style={{background:`linear-gradient(145deg, ${CARD_BG}, rgba(200,255,26,0.06))`,border:`1px solid ${BORDER_CLR}`,borderRadius:18,padding:16}}>
    <div style={{display:"flex",gap:12,alignItems:"center"}}><Av n={profile.identity.name} sz={56} email={profile.identity.email}/><div style={{flex:1,minWidth:0}}><div style={{fontFamily:FD,color:LIGHT,fontSize:22,letterSpacing:1.4,lineHeight:1.05}}>{profile.identity.name.toUpperCase()}</div><div style={{fontFamily:FB,color:MUTED,fontSize:11,marginTop:4,wordBreak:"break-word"}}>{profile.identity.email||profile.identity.playerId||"Manual roster profile"}</div><div style={{display:"inline-flex",marginTop:8,padding:"4px 9px",borderRadius:999,border:`1px solid ${statusColor}55`,background:statusColor+"16",color:statusColor,fontFamily:FB,fontSize:9,fontWeight:900,letterSpacing:"0.09em",textTransform:"uppercase"}}>{profile.statusLabel}</div></div></div>
  </div>
  <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}}>{statCard("At Home Makes",profile.totalAtHomeMakes,VOLT)}{statCard("Program Attempts",profile.totalProgramAttempts,CYAN)}{statCard("Event RSVPs",`${profile.eventSummary.attended}/${profile.eventSummary.rsvps}`,ORANGE)}{statCard("S&C Logs",profile.scSummary.logs,CYAN)}</div>
  {!profile.hasActivity&&<div style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:14,fontFamily:FB,color:MUTED,fontSize:12,lineHeight:1.5}}>No activity yet. When this player logs At Home shots, Program drills, RSVPs, or S&amp;C work, their development profile will populate here.</div>}
  <div style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:16,padding:14}}><div style={{fontFamily:FD,color:LIGHT,fontSize:13,letterSpacing:1.5,marginBottom:10}}>PROGRAM DRILL DEVELOPMENT</div>{profile.programByDrill.length===0?<div style={{fontFamily:FB,color:MUTED,fontSize:11}}>No Program drill attempts logged yet.</div>:profile.programByDrill.map(row=><div key={row.id||row.name} style={{borderTop:`1px solid ${BORDER_CLR}`,padding:"10px 0"}}><div style={{display:"flex",justifyContent:"space-between",gap:8}}><div style={{fontFamily:FB,color:LIGHT,fontSize:12,fontWeight:900}}>{row.name}</div><div style={{fontFamily:FD,color:CYAN,fontSize:16}}>{row.bestScore}</div></div><div style={{fontFamily:FB,color:MUTED,fontSize:10,marginTop:3}}>{row.attempts} attempts · best score · recent {row.recentScores.map(s=>`${s.score}${s.date?` (${s.date})`:""}`).join(", ")}</div></div>)}</div>
  <div style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:16,padding:14}}><div style={{fontFamily:FD,color:LIGHT,fontSize:13,letterSpacing:1.5,marginBottom:8}}>ACTIVITY SUMMARY</div><div style={{fontFamily:FB,color:MUTED,fontSize:11,lineHeight:1.7}}>Last activity: <span style={{color:LIGHT}}>{profile.lastActivityDate||"None yet"}</span><br/>Events: <span style={{color:LIGHT}}>{profile.eventSummary.rsvps} RSVP / {profile.eventSummary.attended} attended</span><br/>Strength &amp; Conditioning: <span style={{color:LIGHT}}>{profile.scSummary.rsvps} RSVP / {profile.scSummary.logs} logs</span></div></div>
</div>;
}
...
function ArchiveDetailStat({ label, value }){
  return <div style={{border:`1px solid ${BORDER_CLR}`,borderRadius:10,padding:"8px 10px",background:BG}}><div style={{fontFamily:FD,color:VOLT,fontSize:18}}>{value ?? 0}</div><div style={{fontFamily:FB,color:MUTED,fontSize:9,letterSpacing:1,textTransform:"uppercase",fontWeight:800}}>{label}</div></div>;
}
function SeasonArchiveDetail({ archive, onBack }){
  const summary=archive?.summary||{};
  const roster=Array.isArray(archive?.rosterSnapshot)?archive.rosterSnapshot:[];
  const events=Array.isArray(archive?.eventSnapshot)?archive.eventSnapshot:[];
  const scSessions=Array.isArray(archive?.scSessionSnapshot)?archive.scSessionSnapshot:[];
  const programDrills=Array.isArray(archive?.programDrillSnapshot)?archive.programDrillSnapshot:[];
  const archivedBy=[archive?.archivedBy?.name,archive?.archivedBy?.email].filter(Boolean).join(" · ")||"Unknown coach";
  const archiveDate=archive?.createdAt?new Date(archive.createdAt).toLocaleString():"Unknown date";
  const range=[archive?.seasonStartDate,archive?.seasonEndDate].filter(Boolean).join(" — ");
  ...
  const SnapshotSection=({title,items,render,empty})=><div style={{border:`1px solid ${BORDER_CLR}`,borderRadius:12,padding:12,background:BG}}><div style={{fontFamily:FD,color:LIGHT,fontSize:13,letterSpacing:1.5,marginBottom:8}}>{title}</div>{items.length===0?<div style={{fontFamily:FB,color:MUTED,fontSize:11}}>{empty}</div>:<div style={{display:"grid",gap:6}}>{items.slice(0,8).map((item,index)=><div key={item?.id||`${title}-${index}`} style={{fontFamily:FB,color:LIGHT,fontSize:11,lineHeight:1.35,borderTop:index===0?"none":`1px solid ${BORDER_CLR}66`,paddingTop:index===0?0:6}}>{render(item)}</div>)}</div>}</div>;
  return <div data-testid="season-archive-detail" style={{background:BG,border:`1px solid ${VOLT}55`,borderRadius:14,padding:14,marginTop:14}}><button type="button" onClick={onBack} style={{minHeight:40,borderRadius:9,border:`1px solid ${BORDER_CLR}`,background:"transparent",color:VOLT,fontFamily:FB,fontWeight:900,letterSpacing:1,cursor:"pointer",marginBottom:12}}>BACK TO ARCHIVED SEASONS</button><div style={{fontFamily:FD,color:LIGHT,fontSize:18,letterSpacing:2}}>{archive?.seasonName||"Archived Season"}</div><div style={{fontFamily:FB,color:MUTED,fontSize:11,lineHeight:1.5,marginTop:4}}>Created {archiveDate} · Archived by {archivedBy}{range?` · ${range}`:""}</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:8,marginTop:12}}><ArchiveDetailStat label="Roster" value={summary.rosterCount}/><ArchiveDetailStat label="Home Scores" value={summary.homeScoreCount}/><ArchiveDetailStat label="Program Scores" value={summary.programScoreCount}/><ArchiveDetailStat label="Shot Logs" value={summary.shotLogCount}/><ArchiveDetailStat label="Events" value={summary.eventCount}/><ArchiveDetailStat label="Event RSVPs" value={summary.eventRsvpCount}/><ArchiveDetailStat label="S&C Sessions" value={summary.scSessionCount}/><ArchiveDetailStat label="S&C RSVPs" value={summary.scRsvpCount}/><ArchiveDetailStat label="S&C Logs" value={summary.scLogCount}/><ArchiveDetailStat label="Home Makes" value={summary.totalHomeMakes}/><ArchiveDetailStat label="Program Score" value={summary.totalProgramScore}/><ArchiveDetailStat label="Shot Log Makes" value={summary.totalShotLogMakes}/></div><div style={{display:"grid",gap:10,marginTop:12}}><SnapshotSection title="ROSTER SNAPSHOT" items={roster} render={p=>`${playerLabel(p)}${p?.email||p?.player_email?` (${p.email||p.player_email})`:""}`} empty="No roster rows in this archive."/><SnapshotSection title="EVENT SNAPSHOT" items={events} render={eventLabel} empty="No events in this archive."/><SnapshotSection title="S&C SNAPSHOT" items={scSessions} render={scLabel} empty="No S&C sessions in this archive."/><SnapshotSection title="PROGRAM DRILL SNAPSHOT" items={programDrills} render={drillLabel} empty="No program drills in this archive."/></div></div>;
}
...
<div style={{flex:1,padding:`${showMiniHeader?"74px":"12px"} 16px 104px`,overflowY:"auto",position:"relative",zIndex:showAdd?40:1}}>
  ...
  {/** DRILLS */}
  {tab==="drills"&&!editD&&<div className="page pageShell fade-up" data-accent="drills" id="coach-drills-management" style={shellVars("drills")}><DashboardReturnButton onClick={()=>setTab("feed")} /><PageHeader title="DRILLS" subtitle="Elite skill development library, structured assignments, and execution standards" accent="cyan" icon={<DrillIcon type="ft" size={22} color={PAGE_ACCENTS.drills.accent}/>} actionLabel="Add" onAction={()=>setShowNewDrill(true)} /><div className="premiumSummaryPanel"><div className="premiumSummaryTop"><div style={{display:"flex",gap:10,alignItems:"flex-start"}}><div className="premiumSummaryBadge"><DrillIcon type="3p" size={16} color={PAGE_ACCENTS.drills.accent}/></div><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.drills.accent,fontSize:13,letterSpacing:"var(--tracking-default)"}}>QUICK START DRILL COMMAND</div><div className="premiumSummaryMeta">{drills.length} drill standards primed for player execution</div></div></div></div><div className="premiumStatGrid"><div className="premiumStatTile"><div className="heroStatVal">{drills.length}</div><div className="heroStatLbl">ACTIVE LIBRARY</div></div><div className="premiumStatTile"><div className="heroStatVal">{programDrills.length}</div><div className="heroStatLbl">PROGRAM SET</div></div><div className="premiumStatTile"><div className="heroStatVal">{drills.length+programDrills.length}</div><div className="heroStatLbl">TOTAL OPTIONS</div></div></div><div className="premiumSummaryActions"><button className="pageHeaderPill pageHeaderPillBrand" onClick={()=>setShowNewDrill(true)}>Start</button><button className="pageHeaderPill" onClick={()=>document.getElementById("coach-drills-management")?.scrollIntoView({behavior:"smooth"})}>Manage Drills</button></div></div>
  ...
  {tab==="leaderboards"&&<div className="page pageShell fade-up" data-accent="feed" style={shellVars("feed")}><DashboardReturnButton onClick={()=>setTab("feed")} />
    <PremiumLeaderboardsHub viewerRole="coach" leaderboardRows={canonicalCoachHomeLeaderboardRows} leaderboardStatus={coachDashboardLeaderboardStatus} programScores={safeProgramScores} programDrills={programDrills} players={leaderboardPlayers} teamId={u?.teamId||""} homeScores={scores} shotLogs={shotLogs} />
  </div>}
  ...
  {tab==="players"&&!selP&&<div data-testid="coach-season-archive" style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:16,padding:16,marginBottom:18}}><div style={{fontFamily:FD,color:LIGHT,fontSize:16,letterSpacing:2,marginBottom:6}}>SEASON ARCHIVE</div><p style={{fontFamily:FB,color:T.SUB,fontSize:11,lineHeight:1.55,marginBottom:12}}>Create a frozen season snapshot for future comparisons. Live team data stays active and unchanged.</p>{seasonArchiveMessage&&<div role="status" style={{fontFamily:FB,color:VOLT,fontSize:11,marginBottom:10}}>{seasonArchiveMessage}</div>}<div style={{display:"grid",gap:8,gridTemplateColumns:isDesktop?"2fr 1fr 1fr":"1fr"}}><FF l="Season name" v={seasonArchiveForm.seasonName} set={v=>{setSeasonArchiveForm(prev=>({...prev,seasonName:v}));setSeasonArchiveConfirm(false);setSeasonArchiveMessage("");}} ph="2026 Summer"/><FF l="Start date (optional)" tp="date" v={seasonArchiveForm.seasonStartDate} set={v=>setSeasonArchiveForm(prev=>({...prev,seasonStartDate:v}))}/><FF l="End date (optional)" tp="date" v={seasonArchiveForm.seasonEndDate} set={v=>setSeasonArchiveForm(prev=>({...prev,seasonEndDate:v}))}/></div>{!seasonArchiveConfirm?<button className="btn-v cta-primary" type="button" onClick={()=>{if(!seasonArchiveForm.seasonName.trim()){setSeasonArchiveMessage("Season name is required.");return;}setSeasonArchiveConfirm(true);}} style={{marginTop:10,width:"100%"}}>Archive Season</button>:<div style={{marginTop:10,background:BG,border:`1px solid ${VOLT}55`,borderRadius:12,padding:12}}><div style={{fontFamily:FB,color:LIGHT,fontSize:12,lineHeight:1.5,marginBottom:10}}>Archive Season creates a frozen copy of this team’s current roster, scores, attendance, events, and S&amp;C data. It does not delete, reset, or change live data.</div><div style={{display:"flex",gap:8}}><button type="button" onClick={()=>setSeasonArchiveConfirm(false)} style={{flex:1,minHeight:40,borderRadius:8,border:`1px solid ${BORDER_CLR}`,background:"transparent",color:LIGHT,fontFamily:FB,fontWeight:800,cursor:"pointer"}}>CANCEL</button><button type="button" onClick={async()=>{const r=await archiveSeason(seasonArchiveForm);setSeasonArchiveMessage(r.ok?`Archived ${r.archive.seasonName}.`:r.error||"Could not archive season.");if(r.ok){setSeasonArchiveForm({seasonName:"",seasonStartDate:"",seasonEndDate:""});setSeasonArchiveConfirm(false);}}} style={{flex:1,minHeight:40,borderRadius:8,border:`1px solid ${VOLT}77`,background:VOLT+"22",color:VOLT,fontFamily:FB,fontWeight:900,cursor:"pointer"}}>CONFIRM ARCHIVE</button></div></div>}<div style={{marginTop:16,paddingTop:12,borderTop:`1px solid ${BORDER_CLR}`}}><div style={{fontFamily:FD,color:T.SUB,fontSize:12,letterSpacing:2,marginBottom:8}}>ARCHIVED SEASONS</div>{selectedSeasonArchive?<SeasonArchiveDetail archive={selectedSeasonArchive} onBack={()=>setSelectedSeasonArchiveId(null)}/>:seasonArchives.length===0?<div style={{fontFamily:FB,color:MUTED,fontSize:11}}>No archived seasons yet.</div>:seasonArchives.slice().reverse().map(a=><button key={a.id} type="button" data-testid="season-archive-view-button" onClick={()=>setSelectedSeasonArchiveId(a.id)} style={{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,padding:"12px 0",border:"none",borderBottom:`1px solid ${BORDER_CLR}66`,background:"transparent",textAlign:"left",cursor:"pointer"}}><div><div style={{fontFamily:FB,color:LIGHT,fontSize:13,fontWeight:800}}>{a.seasonName}</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10}}>{a.createdAt?new Date(a.createdAt).toLocaleDateString():"Unknown date"}</div><div style={{fontFamily:FB,color:VOLT,fontSize:10,fontWeight:900,letterSpacing:1,marginTop:4}}>VIEW ARCHIVE</div></div><div style={{fontFamily:FB,color:VOLT,fontSize:11,textAlign:"right"}}>{a.summary?.rosterCount||0} players<br/>{(a.summary?.totalHomeMakes||0)+(a.summary?.totalShotLogMakes||0)} makes<br/><span aria-hidden="true">›</span></div></button>)}</div></div>}
    ...
    <div style={{marginTop:16}}>
      ...
      {ups.map((p,i)=>{const playerIdentity=p.email||p.profileId||p.playerId||p.id;const tot=getPlayerHomeShotMakes(p.email,shotLogs,u?.teamId);
        return <div key={playerIdentity||i} className="ch" onClick={()=>setSelP(p)} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==="Enter"||e.key===" ")setSelP(p);}} style={{width:"100%",display:"flex",alignItems:"center",gap:12,background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:"14px 16px",marginBottom:8,cursor:"pointer",textAlign:"left"}}>
          ...
          <button onClick={async(e)=>{e.stopPropagation();const r=await archiveRosterPlayer(playerIdentity);setPlayerDataMessage(r.ok?"Player archived and hidden from active views.":r.err||"Could not archive player");if((selP?.email||selP?.profileId||selP?.playerId||selP?.id)===playerIdentity)setSelP(null);}} style={{background:VOLT+"12",border:`1px solid ${VOLT}44`,borderRadius:8,color:VOLT,fontFamily:FB,fontSize:10,fontWeight:700,letterSpacing:"0.06em",padding:"6px 10px",cursor:"pointer"}}>ARCHIVE</button>
    ...
    <div style={{marginTop:32,paddingTop:20,borderTop:`1px solid ${BORDER_CLR}44`}}>
      ...
      <div style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:"14px",marginBottom:14}}>
        <div style={{fontFamily:FD,color:LIGHT,fontSize:14,letterSpacing:2,marginBottom:6}}>LEGAL & SUPPORT</div>
