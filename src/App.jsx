...
if(quietFallback){
setStatSyncError("");
console.warn("home_shots_remote_fallback",{mode:"quiet_local_fallback",errorCode:backendErrorCode,status:e?.status||null,userEmail:String(user?.email||""),teamId:String(user?.teamId||""),made:validation.made,date:validation.date});
trackEvent("shot_log_added",{made:validation.made,date:validation.date,mode:"quiet_local_fallback",fallbackReason:backendErrorCode});
return{ok:true,mode:"quiet_local_fallback",fallbackReason:backendErrorCode};
}
...
const[shotMade,setShotMade]=useState(""),[shotDate,setShotDate]=useState(todayStr()),[shotSaved,setShotSaved]=useState(false),[shotSaving,setShotSaving]=useState(false),[shotInputError,setShotInputError]=useState("");
...
<div ref={playerScrollRef} style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px 124px",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {(tab==="log-drill")&&!active&&!showShotStats&&<div className="fade-up">
    ...
    <div style={{background:CARD_BG,borderRadius:16,padding:"16px 16px",border:`1px solid ${BORDER_CLR}`,marginBottom:24}}>
      ...
      <button className="btn-v cta-primary" disabled={shotSaving} onClick={async()=>{if(shotSaving)return;const validation=validateHomeShotLogInput({made:shotMade,date:shotDate});if(!validation.ok){setShotInputError(validation.error);return;}setShotInputError("");setShotSaving(true);try{const result=await addShotLog(validation.made,shotDate);if(result?.ok){setShotSaved(true);setShotMade("");setTimeout(()=>setShotSaved(false),1800)}}finally{setShotSaving(false);}}} style={{opacity:shotSaving||shotSaved?0.7:1,cursor:shotSaving?"not-allowed":"pointer"}}>
  ...
  {tab==="log-drill"&&showShotStats&&!active&&<div className="fade-up">
    ...
    <ShotTracker u={u} shotLogs={shotLogs} addShotLog={addShotLog} shotMade={shotMade} setShotMade={setShotMade} shotDate={shotDate} setShotDate={setShotDate} shotSaved={shotSaved} setShotSaved={setShotSaved} shotSaving={shotSaving} setShotSaving={setShotSaving}/>
  </div>}
...
// ═══════════════════════════════════════
function ShotTracker({u,shotLogs,addShotLog,shotMade,setShotMade,shotDate,setShotDate,shotSaved,setShotSaved,shotSaving,setShotSaving}){
...
if(result?.ok){setShotSaved(true);setShotMade("");setTimeout(()=>setShotSaved(false),1800);}
}finally{
setShotSaving(false);
}
};
