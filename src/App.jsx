...
const restoreCoachTeamContext=useCallback(async(profile)=>{
if(!profile||profile.role!=="coach"||!profile.teamId||!profile.email)return null;
try{
const res=await fetch("/v1/teams/restore-context",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:profile.email,team_id:profile.teamId})});
const body=await res.json().catch(()=>({}));
if(!res.ok||!body?.team)return null;
const restoredTeam={id:String(body.team.id||profile.teamId),name:String(body.team.name||"Team"),joinCode:String(body.team.joinCode||""),ownerCoachId:profile.email,joinCodeUpdatedAt:Date.now(),createdAt:Date.now(),branding:DEFAULT_BRANDING};
setTeams(prev=>{const existing=prev.find(t=>t.id===restoredTeam.id);if(existing)return prev.map(t=>t.id===restoredTeam.id?{...existing,...restoredTeam,branding:existing.branding||restoredTeam.branding,ownerCoachId:existing.ownerCoachId||restoredTeam.ownerCoachId}:t);return [...prev,restoredTeam];});
return restoredTeam;
...
},[migrateData,navigateToPlayerHome,normalizeStoredInviteContext,readInviteContextFromStorage,restoreCoachTeamContext]);
