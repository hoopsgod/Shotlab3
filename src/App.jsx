...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeHttpStatus:null,consumeDiagnosticCode:"join_in_progress",consumeDiagnosticMessage:"Join request already running locally.",membershipInsertStatus:"pending",membershipInsertError:"",finalRouteDecision:"pending_consume",error:""}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeHttpStatus:400,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:diagnostic.message,consumeTokenPresent:"no",consumeTeamIdUsed:"",consumeUserEmail:"",consumeResolvedUserUuid:"",membershipInsertStatus:"failed",membershipInsertError:diagnostic.code,finalRouteDecision:"stay_join_team",inviteContextSaved:"no",inviteContextStorageKey:INVITE_CONTEXT_STORAGE_KEY,inviteContextTokenPresent:"no",inviteContextTeamId:"",inviteContextSubject:"",currentUserEmail:"",contextSubjectMatchesUser:"no",error:diagnostic.code}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeHttpStatus:400,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:`${diagnostic.message} Checked ${INVITE_CONTEXT_STORAGE_KEY}.`,consumeTokenPresent:"no",consumeTeamIdUsed:"",consumeUserEmail:currentUserEmail,consumeResolvedUserUuid:"",membershipInsertStatus:"failed",membershipInsertError:diagnostic.code,finalRouteDecision:"stay_join_team",inviteContextSaved:"no",inviteContextStorageKey:INVITE_CONTEXT_STORAGE_KEY,inviteContextTokenPresent:"no",inviteContextTeamId:"",inviteContextSubject:"",currentUserEmail,contextSubjectMatchesUser:"no",error:`missing_invite_context:${INVITE_CONTEXT_STORAGE_KEY}`}}));
...
if(joinSubmitInFlightRef.current)return{ok:false,retryable:true,err:"Join in progress."};
joinSubmitInFlightRef.current=true;
try{
const normalizedCode=normalizeJoin(code).replace(/[-\s]+/g,"");
...
joinSubmitInFlightRef.current=false;
}
};
...
const[code,setCode]=useState("");const[err,setErr]=useState("");const[isJoining,setIsJoining]=useState(false);
const submit=async()=>{
if(isJoining)return;
setIsJoining(true);
try{
