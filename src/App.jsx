...
if(user.teamId===resolvedTeamId){setDataDebug(prev=>({...prev,join:{...prev.join,profileUpdateStatus:"not_needed",finalRouteDecision:"route_player_dashboard"}}));navigateToPlayerHome();setView("player");return{ok:true,alreadyJoined:true};}
try{
const np=players.map(p=>p.email===user.email?{...p,teamId:resolvedTeamId}:p);
await P("sl:players",np,setPlayers);
setDataDebug(prev=>({...prev,join:{...prev.join,profileUpdateStatus:"player_team_updated"}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,profileUpdateStatus:"profile_inserted"}}));
}
...
return playerProfiles.find(pp=>pp.teamId===user?.teamId&&[pp.id,pp.profileId,pp.userId,pp.user_id,pp.email,pp.player_email,pp.playerId,pp.player_id].map(v=>normalizeEmail(v)).includes(target));
};
...
const appLeaderboardPlayers=useMemo(()=>[...(Array.isArray(scopedPlayers)?scopedPlayers:[]),...(Array.isArray(playerProfiles)?playerProfiles.filter(pp=>pp.teamId===user?.teamId):[])],[scopedPlayers,playerProfiles,user?.teamId]);
