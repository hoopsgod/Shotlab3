...
const renderPremiumLeaderboardsHub=({mode,containerClassName,showReturnButton=false})=><div className={containerClassName} key={`${mode}-leaderboards`} data-accent={mode==="coach"?"feed":undefined} style={mode==="coach"?shellVars("feed"):undefined}>{showReturnButton&&<DashboardReturnButton onClick={()=>setTab("feed")} />}
...
<div ref={playerScrollRef} style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px 124px",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {tab==="leaderboards"&&!active&&renderPremiumLeaderboardsHub({mode:"player",containerClassName:slideClass,showReturnButton:false})}
...
<div style={{flex:1,padding:`${showMiniHeader?"74px":"12px"} 16px 104px`,overflowY:"auto",position:"relative",zIndex:showAdd?40:1}}>
  ...
  {tab==="leaderboards"&&renderPremiumLeaderboardsHub({mode:"coach",containerClassName:"page pageShell fade-up",showReturnButton:true})}
