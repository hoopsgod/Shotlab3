...
<div style={{flex:1,padding:"14px 16px 124px",overflowY:"auto",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s"}} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
  ...
  {(tab==="log-drill")&&!active&&!showShotStats&&<div className="fade-up">
    ...
    <div className="surface-tier-2" style={{padding:"20px",marginBottom:24}}>
    ...
    {drills.map(d=>{const done=todayS.find(s=>s.drillId===d.id);
      return <button key={d.id} className="ch surface-tier-2" onClick={()=>!done&&setActive(d)} style={{width:"100%",display:"flex",alignItems:"center",gap:14,border:`1px solid ${done?"rgba(200, 255, 0, 0.20)":"var(--stroke-1)"}`,padding:"20px",marginBottom:24,cursor:done?"default":"pointer",textAlign:"left",opacity:done?.6:1}}>
...
{/* Leaderboard toggle */}
<button onClick={()=>setShowBoard(!showBoard)} className="ch surface-tier-2" style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px",marginBottom:24,cursor:"pointer",textAlign:"left"}}>
...
<div className="surface-tier-3" style={{marginBottom:24,padding:"20px"}}>
...
<div className="surface-tier-2" style={{marginBottom:24,minHeight:100,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:"20px"}}>
...
{upcoming.map(s=>{const sr=scRsvps.filter(r=>r.sessionId===s.id);const going=sr.some(r=>r.email===user.email);const exp=expanded===s.id;
  return <div key={s.id} style={{marginBottom:24}}>
    <button onClick={()=>setExpanded(exp?null:s.id)} className="ch surface-tier-2" style={{width:"100%",border:`1px solid ${going?SC_COLOR+"33":"var(--stroke-1)"}`,borderRadius:exp?"20px 20px 0 0":20,padding:"20px",textAlign:"left",cursor:"pointer",position:"relative",overflow:"hidden"}}>
    ...
    {exp&&<div className="fade-up surface-tier-1" style={{borderRadius:"0 0 20px 20px",padding:"20px",borderTop:"none"}}>
...
{past.length>0&&<><CourtDivider color={SC_COLOR} my={12}/><SH isCoach={typeof u!=="undefined"&&u?.isCoach} t="PAST SESSIONS" s={`${past.length} COMPLETED`}/>
  {past.map(s=>{const sr=scRsvps.filter(r=>r.sessionId===s.id);const went=sr.some(r=>r.email===user.email);
    return <div key={s.id} className="surface-tier-2" style={{display:"flex",alignItems:"center",gap:12,padding:"16px 20px",marginBottom:24,opacity:.7}}>
...
return <div className="surface-tier-2" style={{padding:"16px",minHeight:98,display:"flex",flexDirection:"column",justifyContent:"space-between"}}><div style={{fontFamily:FD,color:color||LIGHT,fontSize:24,lineHeight:1.05,wordBreak:"break-word"}}>{value}</div><div style={{fontFamily:FB,color:TOKENS.TEXT_SECONDARY,fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase"}}>{label}</div></div>
}
...
const baseBorder=isActive?`1px solid rgba(255,255,255,0.20)`:`1px solid rgba(255,255,255,0.12)`;
const baseShadow=isActive?"0 12px 28px rgba(0,0,0,.52)":"0 8px 24px rgba(0,0,0,.45)";
return <button type="button" onClick={onClick} className="mode-card surface-tier-3" style={{width:"100%",background:`radial-gradient(circle at 12% 10%, ${a.tint} 0%, transparent 55%), var(--surface-3)`,border:baseBorder,borderRadius:20,padding:22,cursor:"pointer",textAlign:"left",position:"relative",minHeight:272,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:baseShadow,transition:"transform .12s ease, border-color .2s ease, box-shadow .2s ease"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(255,255,255,0.22)";e.currentTarget.style.boxShadow="0 12px 28px rgba(0,0,0,.50)"}} onMouseLeave={e=>{e.currentTarget.style.border=baseBorder;e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}} onMouseDown={e=>{e.currentTarget.style.transform="scale(0.99)";e.currentTarget.style.boxShadow="0 10px 20px rgba(0,0,0,.52)"}} onMouseUp={e=>{e.currentTarget.style.transform="scale(1)"}} onFocus={e=>{e.currentTarget.style.outline="none";e.currentTarget.style.boxShadow=`0 0 0 3px ${MODE_CARD_TOKENS.FOCUS_RING}, 0 12px 28px rgba(0,0,0,.50)`}} onBlur={e=>{e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}}>
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:16}}>
    <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}>
      <div style={{width:50,height:50,borderRadius:14,background:MODE_CARD_TOKENS.ICON_INNER,border:"1px solid rgba(255,255,255,0.14)",display:"flex",alignItems:"center",justifyContent:"center",color:a.iconStroke,flexShrink:0}}>{icon}</div>
...
{upcoming.map(ev=>{const evR=rsvps.filter(r=>r.eventId===ev.id);const going=evR.some(r=>r.email===user.email);const exp=expanded===ev.id;
  return <div key={ev.id} className="stack-24" style={{marginBottom:24}}>
    <div className="ch surface-tier-2" style={{width:"100%",border:`1px solid ${going?VOLT+"33":"var(--stroke-1)"}`,borderRadius:exp?"20px 20px 0 0":20,padding:"20px",textAlign:"left",position:"relative",overflow:"hidden"}}>
...
<div className="surface-tier-3" style={{borderRadius:20,padding:"28px 24px 24px",position:"relative",overflow:"hidden",textAlign:"center",marginBottom:28}}> 
{/* Corner accents */}
<div style={{position:"absolute",top:0,left:0,width:60,height:60,borderTop:`3px solid ${VOLT}`,borderLeft:`3px solid ${VOLT}`,borderRadius:"24px 0 0 0",opacity:.4}}/>
<div style={{position:"absolute",bottom:0,right:0,width:60,height:60,borderBottom:`3px solid ${VOLT}`,borderRight:`3px solid ${VOLT}`,borderRadius:"0 0 24px 0",opacity:.4}}/>
...
{/* Overall stats */}
<div className="surface-tier-2" style={{padding:"4px 20px",marginBottom:24}}>
...
{drillStats.map(d=>{const accentColor=getDrillAccentColor(d.name);return <div key={d.id} className="surface-tier-2" style={{padding:"20px",borderLeft:`5px solid ${accentColor}`,marginBottom:24}}>
  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
  ...
  <div style={{display:"flex",gap:8,marginBottom:d.last10.length>=2?12:0}}>
    <div className="surface-tier-1" style={{flex:1,padding:"16px",textAlign:"center"}}>
...
{roster.map(p=>{const c=p.statusMeta.color;const isNudged=nudged.includes(p.email);
  ...
  return <div key={p.email} className="surface-tier-2" style={{display:"flex",marginBottom:24,border:`1px solid ${p.statusMeta.pill==="INACTIVE"?"#FF454533":"var(--stroke-1)"}`,overflow:"hidden"}}>
...
function SectionHero({icon,title,subtitle,accent=VOLT,deco,isCoach=false}){return <div style={{marginBottom:12}}><div style={{height:80,display:"flex",alignItems:"center",gap:14}}><div style={{width:42,height:42,borderRadius:12,background:accent+"12",border:`1px solid ${accent}33`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",flexShrink:0}}>{icon}{deco&&<div style={{position:"absolute",bottom:-6,right:-6,opacity:.6}}>{deco}</div>}</div><div><div className="u-allcaps-long" style={{fontFamily:FD,fontSize:24,color:accent,lineHeight:1,display:"flex",alignItems:"center",gap:6}}>{title}{isCoach&&<ShieldIcon size={12} color="#C8FF00" style={{opacity:.5,pointerEvents:"none"}}/>}</div><div className="u-secondary-text" style={{fontFamily:FB,fontSize:12,marginTop:4}}>{subtitle}</div></div></div><div style={{height:1,background:BORDER_CLR}}/></div>}
...
function SH({t,s,isCoach=false,identity=false}){return <div style={{marginBottom:16,display:"flex",alignItems:"baseline",justifyContent:"space-between"}}><div>{identity?<div className="page-header"><h1 className="page-title u-allcaps-long" style={{fontFamily:FD,color:LIGHT,fontSize:18,display:"inline-flex",alignItems:"center",gap:6,margin:0}}>{t}{isCoach&&<ShieldIcon size={12} color="#C8FF00" style={{opacity:.5,pointerEvents:"none"}}/>}</h1><div className="page-identity-bar" aria-hidden="true"></div></div>:<div className="u-allcaps-long" style={{fontFamily:FD,color:LIGHT,fontSize:18,display:"inline-flex",alignItems:"center",gap:6}}>{t}{isCoach&&<ShieldIcon size={12} color="#C8FF00" style={{opacity:.5,pointerEvents:"none"}}/>}</div>}</div>{s&&<div className="u-meta-label" style={{fontFamily:FB,fontSize:11}}>{s}</div>}</div>}
...
const tabAccent=t.accentVar?`var(${t.accentVar})`:"var(--nav-accent)";
