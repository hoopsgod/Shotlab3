...
const [dataDebug,setDataDebug]=useState({join:{enteredCode:"",normalizedCode:"",status:"idle",lookupSource:"none",lookupField:"none",lookupCount:null,matchedTeamId:"",update:"idle",error:""},leaderboard:{endpoint:"",httpStatus:null,errorCode:"",resultCount:null,isEmpty:false}});
...
setDataDebug(prev=>({...prev,join:{...prev.join,enteredCode:String(code||""),normalizedCode,status:"lookup",lookupSource:"backend_invite_context",lookupField:"team_invites.code_hash",lookupCount:null,matchedTeamId:"",update:"idle",error:""}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,status:"failed",lookupCount:0,error:safeError}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,status:"validated",lookupCount:1,matchedTeamId:body.team_id||""}}));
...
let bootstrapBody={};
try{
const bootstrapRes=await fetch("/v1/coach-signup/bootstrap",{method:"POST",headers:{"Content-Type":"application/json","x-user-id":user.email},body:JSON.stringify({team_name:san(name)||"Team"})});
bootstrapBody=await bootstrapRes.json().catch(()=>({}));
if(!bootstrapRes.ok)return{ok:false,err:bootstrapBody?.error==="team_invite_creation_failed"?"Could not create team code.":"Team setup unavailable."};
}catch{
return{ok:false,err:"Team setup unavailable."};
}
const code=normalizeJoin(bootstrapBody?.invite_code||"");
const teamId=String(bootstrapBody?.team_id||"").trim();
if(!code||!teamId)return{ok:false,err:"Could not create team code."};
