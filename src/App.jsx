...
const Styles=()=><><style>{_STYLES_CSS}</style><style>{_BUTTON_SYSTEM_CSS}</style><style>{_PAGE_SIGNATURE_CSS}</style><style>{_DESKTOP_SHELL_CSS}</style></>;
...
if(drills.length===0)return <div style={{marginBottom:14}}><SH isCoach={typeof u!=="undefined"&&u?.isCoach} t="PROGRAM DRILLS" s="COACH ADDED"/><Empty t="NO DRILLS YET" action="Your coach can add up to 7 custom shooting drills" cta="CONTACT YOUR COACH" ctaVariant="tertiaryLink" icon={<DrillIcon type="sb" size={48} color="#555555"/>}/></div>;
...
{upcoming.length===0&&<Empty t="NO EVENTS SCHEDULED" action="Your coach will post workouts and events here" cta="NOTIFY MY COACH" ctaVariant="tertiaryLink" icon={<EventIcon type="clinic" size={48} color="#555555"/>}/>}
...
function Empty({t,action,onTap,cta="GET STARTED",ctaVariant="primary",icon=<DrillIcon type="sb" size={48} color="#555555"/>}){return <div style={{textAlign:"center",padding:"40px 20px"}}><div style={{opacity:.8,display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#555555"}}>{icon}</div><p className="u-allcaps-long" style={{fontFamily:FD,color:LIGHT,fontSize:18,marginTop:14,lineHeight:1.2}}>{t}</p>{action&&<p className="u-secondary-text" style={{fontFamily:FB,fontSize:13,margin:"8px auto 0",lineHeight:1.5,fontWeight:500,maxWidth:260}}>{action}</p>}<Button onClick={onTap||(()=>{})} variant={ctaVariant} className={ctaVariant==="tertiaryLink"?"":"btn-v"} style={{marginTop:14,width:ctaVariant==="tertiaryLink"?"auto":"calc(100% - 32px)",marginLeft:ctaVariant==="tertiaryLink"?"auto":16,marginRight:ctaVariant==="tertiaryLink"?"auto":16}}>{cta}</Button></div>}
