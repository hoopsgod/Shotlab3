...
setPlayers(m.playersMigrated);setPlayerProfiles(m.profilesMigrated);setTeams(m.teamsMigrated);setScores(m.scoresM);setEvents(m.eventsM);setRsvps(m.rsvpsM);setShotLogs(m.shotM);setProgressSnapshots(Array.isArray(psn)?psn:[]);setChallenges(m.chM);setScSessions(m.scSM);setScRsvps(m.scRM);setScLogs(m.scLM);
await Promise.all([DB.set("sl:drills",seededDrills),DB.set("sl:program-drills",seededProgramDrills),DB.set("sl:players",m.playersMigrated),DB.set("sl:player-profiles",m.profilesMigrated),DB.set("sl:teams",m.teamsMigrated),DB.set("sl:scores",m.scoresM),DB.set("sl:events",m.eventsM),DB.set("sl:rsvps",m.rsvpsM),DB.set("sl:shotlogs",m.shotM),DB.set("sl:progress-snapshots",Array.isArray(psn)?psn:[]),DB.set("sl:challenges",m.chM),DB.set("sl:sc-sessions",m.scSM),DB.set("sl:sc-rsvps",m.scRM),DB.set("sl:sc-logs",m.scLM)]);
...
const scopedProgressSnapshots=progressSnapshots.filter(snapshot=>snapshot.teamId===user?.teamId);
...
<div style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px 124px",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {tab==="home"&&!active&&<div className={slideClass} key="home">
    ...
    {/* ══════ LEADERBOARD ══════ */}
    <DashboardLeaderboard scores={scores} drills={drills} programDrills={programDrills} user={u} scRsvps={scRsvps} rsvps={rsvps} shotLogs={shotLogs} players={players}/>
  </div>}
...
// COACH SCREEN
// ═══════════════════════════════════════
function Coach({u,team,regenerateJoinCode,addRosterPlayer,playerProfiles,drills,programDrills,scores,players,updateDrill,addDrill,removeDrill,addProgramDrill,removeProgramDrill,events,rsvps,addEvent,removeEvent,removeRsvp,addRsvp,scSessions,scRsvps,scLogs=[],addScSession,removeScSession,shotLogs,progressSnapshots=[],logout,deleteAccount,openTeamBranding,coachTextSize="standard",demoSettingsBusy=false,onLoadDemoData,onClearDemoData}){
...
// HISTORY
// ═══════════════════════════════════════
function HistPanel({sc,dr,programDr=[],progressSnapshots=[]}){
const sorted=useMemo(()=>[...sc,...progressSnapshots.map(snapshot=>({type:"snapshot",date:String(snapshot?.capturedAt||"").slice(0,10),ts:snapshot?.capturedAt?Date.parse(snapshot.capturedAt):0,snapshot}))].sort((a,b)=>(b.ts||0)-(a.ts||0)),[sc,progressSnapshots]);
const grouped=useMemo(()=>{const m={};sorted.forEach(entry=>{if(!m[entry.date])m[entry.date]=[];m[entry.date].push(entry)});return Object.entries(m)},[sorted]);
const findDrillForScore=score=>((score?.src||"home")==="program"?programDr:dr).find(d=>d.id===score.drillId);
return <div><SH isCoach={typeof u!=="undefined"&&u?.isCoach} t="SCORE HISTORY" s="ALL SOURCES"/>{grouped.length===0&&<Empty t="No scores yet"/>}{grouped.map(([date,entries])=><div key={date} style={{marginBottom:24}}><div style={{fontFamily:FD,color:T.SUB,fontSize:12,letterSpacing:4,marginBottom:8}}>{date}</div>{entries.map((entry,i)=>{if(entry?.type==="snapshot"){const metrics=entry.snapshot?.metrics||{};return <div key={entry.snapshot?.id||`snapshot-${i}`} style={{display:"flex",alignItems:"center",gap:12,background:CARD_BG,borderRadius:12,padding:"12px 16px",marginBottom:5,border:`1px solid ${BORDER_CLR}`}}><div style={{width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",background:BG,borderRadius:8,flexShrink:0,fontFamily:FD,fontSize:11,color:CYAN}}>SN</div><div style={{flex:1}}><div style={{fontFamily:FD,color:LIGHT,fontSize:13,letterSpacing:2,display:"flex",alignItems:"center",gap:6}}>{entry.snapshot?.label||"PROGRESS SNAPSHOT"}<span style={{fontFamily:FB,fontSize:7,fontWeight:700,padding:"1px 4px",borderRadius:3,color:CYAN,background:CYAN+"15"}}>SNAPSHOT</span></div><div style={{fontFamily:FB,color:MUTED,fontSize:10,marginTop:4}}>Sessions {metrics.scoringSessionsLogged??"—"} · Program Avg {metrics.averageProgramScore??"—"} · Weekly Shots {metrics.weeklyShotVolume??"—"} · Attendance {typeof metrics.attendanceRate==="number"?`${Math.round(metrics.attendanceRate*100)}%`:"—"}</div></div></div>}const s=entry;const d=findDrillForScore(s);const pct=d&&hasDrillMax(d)?Math.round(s.score/d.max*100):null;const isH=s.src==="home"||!s.src;return <div key={i} style={{display:"flex",alignItems:"center",gap:12,background:CARD_BG,borderRadius:12,padding:"12px 16px",marginBottom:5,border:`1px solid ${BORDER_CLR}`}}><div style={{width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",background:BG,borderRadius:8,flexShrink:0}}><DrillIcon type={d?.icon} size={18}/></div><div style={{flex:1}}><div style={{fontFamily:FD,color:LIGHT,fontSize:13,letterSpacing:2,display:"flex",alignItems:"center",gap:6}}>{d?.name}<span style={{fontFamily:FB,fontSize:7,fontWeight:700,padding:"1px 4px",borderRadius:3,color:isH?VOLT:CYAN,background:isH?VOLT+"15":CYAN+"15"}}>{isH?"HOME":"PROG"}</span></div></div><div style={{textAlign:"right"}}><div style={{fontFamily:FD,color:isH?VOLT:CYAN,fontSize:16}}>{s.score}{hasDrillMax(d)&&<span style={{color:MUTED,fontSize:11}}>/{d?.max}</span>}</div>{typeof pct==="number"&&<div style={{fontFamily:FB,fontSize:9,fontWeight:700,color:pct>=80?"#C8FF00":pct>=50?"#FFA500":"#FF4545"}}>{pct}%</div>}</div></div>})}</div>)}</div>;
}
