...
{upcoming.length===0&&<EmptyState icon={LiftIcon} iconColor="#555555" headline={user?.isCoach?"NO UPCOMING SESSIONS":"NO UPCOMING SESSIONS"} supportingText={user?.isCoach?"Create the next S&C block for your team.":"Your coach will add S&C sessions here. Check back soon!"} ctaLabel={user?.isCoach?"CREATE SESSION":"CHECK BACK SOON"} ctaAction={()=>{}} secondaryLabel={user?.isCoach?"View session history":undefined} secondaryAction={()=>{}} isCoach={!!user?.isCoach} />}
...
// ═══════════════════════════════════════
function EventsPanel({events,rsvps,user,toggleRsvp,scores,drills,onCreateEvent}){
...
{upcoming.length===0&&<EmptyState icon={EventCalendarIcon} iconColor="#555555" headline={user?.isCoach?"NO UPCOMING EVENTS":"NO EVENTS SCHEDULED"} supportingText={user?.isCoach?"Create your first team event to get attendance rolling.":"Your coach will post workouts and events here"} ctaLabel={user?.isCoach?"CREATE EVENT":"NOTIFY MY COACH"} ctaAction={user?.isCoach?onCreateEvent:(()=>{})} secondaryLabel={user?.isCoach?"View past events":undefined} secondaryAction={()=>{}} isCoach={!!user?.isCoach} showGlow />}
...
function SH({t,s,isCoach=false}){return <div style={{marginBottom:16,display:"flex",alignItems:"baseline",justifyContent:"space-between"}}><div style={{fontFamily:FD,color:LIGHT,fontSize:18,letterSpacing:4,display:"inline-flex",alignItems:"center",gap:6}}>{t}{isCoach&&<ShieldIcon size={12} color="#C8FF00" style={{opacity:.5,pointerEvents:"none"}}/>}</div>{s&&<div style={{fontFamily:FB,color:T.SUB,fontSize:11,letterSpacing:"0.06em",fontWeight:600,textTransform:"uppercase"}}>{s}</div>}</div>}
...
function EventCalendarIcon({size=48,color="#555555"}){return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>}
function DrillClipboardIcon({size=48,color="#555555"}){return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="2" width="6" height="4" rx="1"/><path d="M9 4H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-3"/><path d="M8 12h8"/><path d="M8 16h5"/></svg>}
function CircleDotIcon({size=48,color="#555555"}){return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="2.5" fill={color} stroke="none"/></svg>}
...
function CreateEventScreen({onBack}){return <div className="fade-up" style={{paddingBottom:20}}><button onClick={onBack} style={{background:"none",border:"none",color:VOLT,fontFamily:FB,fontSize:13,cursor:"pointer",fontWeight:700,letterSpacing:2,marginBottom:20,padding:0}}>&#8592; BACK</button><div style={{fontFamily:FB,fontSize:13,color:"#C8FF00",textTransform:"uppercase",letterSpacing:"0.10em",marginBottom:36}}>CREATE EVENT</div><div style={{minHeight:260,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:14,color:"#A0A0A0",fontFamily:FB}}>Coming soon</div></div></div>}
