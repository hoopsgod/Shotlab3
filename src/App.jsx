...
const canonicalRoster=useMemo(()=>players.filter(p=>p.role!=="coach"),[players]);
const rosterByEmail=useMemo(()=>new Map(canonicalRoster.map(player=>[player.email,player])),[canonicalRoster]);
const playerSummaryByEmail=useMemo(()=>{const summary=new Map(canonicalRoster.map(player=>[player.email,{scores:[],homeScoreCount:0,programScoreCount:0,totalScore:0,eventCount:0}]));scores.forEach(score=>{const playerSummary=summary.get(score.email);if(!playerSummary)return;playerSummary.scores.push(score);playerSummary.totalScore+=Number(score.score)||0;if(score.src==="program")playerSummary.programScoreCount+=1;else playerSummary.homeScoreCount+=1;});rsvps.forEach(rsvp=>{const playerSummary=summary.get(rsvp.email);if(playerSummary)playerSummary.eventCount+=1;});return summary;},[canonicalRoster,scores,rsvps]);
const ups=canonicalRoster.map(player=>({email:player.email,name:player.name}));
const allKnown=useMemo(()=>{const m={};canonicalRoster.forEach(p=>m[p.email]=p.name);scores.forEach(s=>{if(!m[s.email])m[s.email]=s.name||s.email});return Object.entries(m).map(([email,name])=>({email,name}))},[canonicalRoster,scores]);
...
<div style={{flex:1,padding:`${showMiniHeader?"88px":"16px"} 20px 110px`,overflowY:"auto",position:"relative",zIndex:showAdd?40:1}}>
  ...
  {tab==="players"&&!selP&&<div className="page pageShell" data-accent="players" style={shellVars("players")}><PageHeader title="PLAYERS" subtitle="Roster insights, development, and availability" accent="purple" icon={<UsersIcon size={20} color={PAGE_ACCENTS.players.accent}/>} actionLabel="Add" onAction={()=>document.getElementById("coach-add-player-form")?.scrollIntoView({behavior:"smooth"})} /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.players.accent,fontSize:12,letterSpacing:"var(--tracking-default)"}}>ROSTER SNAPSHOT</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10}}>{canonicalRoster.length} players on roster</div></div></div></div>
    <div id="coach-add-player-form" className="accent-card" style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:14,marginBottom:12}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}}>
        <div style={{fontSize:10,color:MUTED,fontFamily:FB}}>{canonicalRoster.length} player{canonicalRoster.length===1?"":"s"} on team</div>
    ...
    <div style={{marginTop:16}}>
      ...
      {ups.map((p,i)=>{const playerSummary=playerSummaryByEmail.get(p.email)||{totalScore:0,eventCount:0};
        return <button key={i} className="ch" onClick={()=>setSelP(p)} style={{width:"100%",display:"flex",alignItems:"center",gap:12,background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:"14px 16px",marginBottom:8,cursor:"pointer",textAlign:"left"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontFamily:FB,color:T.SUB,fontSize:10,marginTop:2}}>{playerSummary.totalScore} makes · {playerSummary.eventCount} events</div>
          </div>
  ...
  {tab==="players"&&selP&&<div className="fade-up">{(()=>{const selectedPlayer=rosterByEmail.get(selP.email)||selP;const playerSummary=playerSummaryByEmail.get(selectedPlayer.email)||{scores:[],homeScoreCount:0,programScoreCount:0,eventCount:0};return <><button onClick={()=>setSelP(null)} style={{background:"none",border:"none",color:VOLT,fontFamily:FB,fontSize:13,cursor:"pointer",fontWeight:700,letterSpacing:2,marginBottom:20}}>&#8592; BACK</button><div style={{textAlign:"center",marginBottom:24}}><Av n={selectedPlayer.name} sz={64} email={selectedPlayer.email} style={{margin:"0 auto 14px"}}/><div style={{fontFamily:FD,color:LIGHT,fontSize:24,letterSpacing:2}}>{selectedPlayer.name.toUpperCase()}</div><div style={{color:MUTED,fontSize:12,marginTop:4}}>{selectedPlayer.email}</div><div style={{display:"flex",gap:8,justifyContent:"center",marginTop:12,flexWrap:"wrap"}}><span style={{fontFamily:FB,fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:5,color:VOLT,background:VOLT+"15"}}>HOME: {playerSummary.homeScoreCount}</span><span style={{fontFamily:FB,fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:5,color:LIGHT,background:LIGHT+"10"}}>PROGRAM: {playerSummary.programScoreCount}</span><span style={{fontFamily:FB,fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:5,color:ORANGE,background:ORANGE+"15"}}>{playerSummary.eventCount} EVENTS</span></div></div><HistPanel sc={playerSummary.scores} dr={drills} programDr={programDrills}/></>;})()}</div>}
