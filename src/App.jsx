...
import { archivePlayerForTeam, deleteTeamLocalPlayerData, filterActiveTeamChallengeRows, filterActiveTeamLeaderboardRows, filterActiveTeamPlayerRows, getActiveTeamPlayerIdentity, isActiveRosterPlayer, isPlayerHiddenFromActiveLeaderboards, removePlayerFromTeam, resolvePlayerDisplayName } from "./lib/playerDataManagement.js";
...
const playerDashboardHomeLeaderboardRows=useMemo(()=>buildAtHomeLeaderboardRows({scores,shotLogs,programDrills,limit:3}),[scores,shotLogs,programDrills]);
...
const coachRosterPlayers=useMemo(()=>players.filter(p=>isActiveRosterPlayer(p,u?.teamId)).map(p=>({...p,name:resolvePlayerDisplayName(p,playerProfiles)})),[players,playerProfiles,u?.teamId]);
const ups=useMemo(()=>coachRosterPlayers.map(p=>({email:p.email,name:resolvePlayerDisplayName(p,playerProfiles),playerId:p.playerId||p.player_id||p.id||p.userId||p.user_id})),[coachRosterPlayers,playerProfiles]);
...
const coachDashboardHomeLeaderboardRows=useMemo(()=>buildAtHomeLeaderboardRows({scores:safeScores,shotLogs,programDrills,limit:5}),[safeScores,shotLogs,programDrills]);
const coachDashboardLeaderboardRows=activeLeaderboardRows.length?activeLeaderboardRows:coachDashboardHomeLeaderboardRows;
const coachDashboardLeaderboardStatus=homeShotsLeaderboard?.status==="success"||coachDashboardHomeLeaderboardRows.length>0?"success":homeShotsLeaderboard?.status||"idle";
...
<div style={{flex:1,padding:`${showMiniHeader?"74px":"12px"} 16px 104px`,overflowY:"auto",position:"relative",zIndex:showAdd?40:1}}>
  ...
  {tab==="leaderboards"&&<div className="page pageShell fade-up" data-accent="feed" style={shellVars("feed")}><DashboardReturnButton onClick={()=>setTab("feed")} />
    <PremiumLeaderboardsHub viewerRole="coach" leaderboardRows={activeLeaderboardRows} leaderboardStatus={homeShotsLeaderboard?.status||"idle"} programScores={safeProgramScores} programDrills={programDrills} players={leaderboardPlayers} teamId={u?.teamId||""} homeScores={safeScores} shotLogs={shotLogs} />
  </div>}
  ...
  {tab==="players"&&!selP&&<div className="page pageShell" data-accent="players" style={shellVars("players")}><DashboardReturnButton onClick={()=>setTab("feed")} /><PageHeader title="PLAYERS" subtitle="Roster insights, development, and availability" accent="purple" icon={<UsersIcon size={20} color={PAGE_ACCENTS.players.accent}/>} actionLabel="Add" onAction={()=>document.getElementById("coach-add-player-form")?.scrollIntoView({behavior:"smooth"})} /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.players.accent,fontSize:12,letterSpacing:"var(--tracking-default)"}}>ROSTER SNAPSHOT</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10}}>{ups.length} players on roster</div></div></div></div>
    ...
    <div style={{marginTop:16}}>
      ...
      {ups.map((p,i)=>{const tot=getPlayerHomeShotMakes(p.email,shotLogs,u?.teamId);
...
{roster.map(p=>{const c=p.statusMeta.color;const isNudged=nudged.includes(p.email);
  ...
  return <div key={p.email} role="button" tabIndex={0} onClick={()=>onSelectPlayer?.(p)} onKeyDown={e=>{if(e.key==="Enter"||e.key===" ")onSelectPlayer?.(p);}} style={{display:"flex",background:CARD_BG,borderRadius:14,marginBottom:10,border:`1px solid ${p.statusMeta.pill==="INACTIVE"?"#FF454533":BORDER_CLR}`,overflow:"hidden",cursor:onSelectPlayer?"pointer":"default"}}>
    ...
    {p.statusMeta.pill==="INACTIVE"&&<button onClick={(e)=>{e.stopPropagation();if(!isNudged)setNudged(n=>[...n,p.email])}} style={{minHeight:40,padding:"0 12px",borderRadius:8,border:`1px solid ${isNudged?VOLT+"44":"#FF454544"}`,background:isNudged?VOLT+"12":"#FF454512",cursor:"pointer",fontFamily:FB,fontSize:10,fontWeight:700,letterSpacing:1,color:isNudged?VOLT:"#FF4545",whiteSpace:"nowrap",width:"100%"}}>
      {isNudged?"✓ NUDGED":"NUDGE"}
    </button>}
    <button onClick={(e)=>{e.stopPropagation();onRemovePlayer?.(p.email)}} style={{minHeight:34,padding:"0 12px",borderRadius:8,border:"1px solid #FF454533",background:"#FF454510",cursor:"pointer",fontFamily:FB,fontSize:10,fontWeight:700,letterSpacing:"0.06em",color:"#FF4545",whiteSpace:"nowrap",width:"100%"}}>
      REMOVE
    </button>
      </div>
    </div>
