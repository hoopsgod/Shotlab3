...
if(!result.ok)return result;await P("sl:season-archives",result.seasonArchives,setSeasonArchives,{strictLocal:true});trackEvent("season_archive_created",{archiveId:result.archive.id,seasonName:result.archive.seasonName});return result;
}catch(error){return{ok:false,error:error?.message||"Could not archive season."};}
},[user,players,playerProfiles,scores,programScores,shotLogs,events,rsvps,scSessions,scRsvps,scLogs,programDrills,drills,challenges,seasonArchives,P,trackEvent]);
