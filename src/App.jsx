...
BG_ELEVATED:"rgba(255,255,255,0.055)",
...
const SHADOW_CARD = "0 10px 30px rgba(0,0,0,0.45)";
const GLOW_LIME = "0 0 0 2px rgba(190,255,0,0.18), 0 8px 30px rgba(190,255,0,0.08)";
...
BASE_BG:"var(--bg-1)",
BASE_BORDER:"var(--border-soft)",
BASE_SHADOW:"var(--shadow-card)",
...
FOCUS_RING:"rgba(190,255,0,0.30)",
...
.heroModule{position:relative;overflow:hidden;border:1px solid var(--border-soft);border-radius:var(--radius-card);padding:var(--card-pad);margin-bottom:var(--stack-gap);background:var(--bg-1);box-shadow:var(--shadow-card);}
...
.heroStat{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:8px;text-align:center;}
...
{pending.length>0&&<><SH isCoach={typeof u!=="undefined"&&u?.isCoach} t="INCOMING" s={`${pending.length} WAITING`}/>
  {pending.map(ch=>{const dr=drills.find(d=>d.id===ch.drillId);const isResp=respId===ch.id;
    return <div key={ch.id} className="fade-up card-glow-o" style={{background:"var(--bg-2)",borderRadius:16,padding:"18px 20px",marginBottom:10,border:`1px solid ${BORDER_MED}`,boxShadow:SHADOW_CARD,position:"relative",overflow:"hidden"}}>
...
return <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"12px 10px",minHeight:98,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:"none"}}><div style={{fontFamily:FD,color:color||LIGHT,fontSize:24,lineHeight:1.05,wordBreak:"break-word"}}>{value}</div><div style={{fontFamily:FB,color:TOKENS.TEXT_SECONDARY,fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase"}}>{label}</div></div>
}
...
const baseBorder=isActive?`1px solid ${BORDER_MED}`:`1px solid ${MODE_CARD_TOKENS.BASE_BORDER}`;
const baseShadow=isActive?`${GLOW_LIME}, ${MODE_CARD_TOKENS.BASE_SHADOW}`:MODE_CARD_TOKENS.BASE_SHADOW;
return <button type="button" onClick={onClick} className={`mode-card ${isActive?"is-active":""}`} style={{width:"100%",background:`radial-gradient(circle at 12% 10%, ${a.tint} 0%, transparent 55%), ${isActive?"var(--bg-2)":MODE_CARD_TOKENS.BASE_BG}`,border:baseBorder,borderRadius:24,padding:22,cursor:"pointer",textAlign:"left",position:"relative",minHeight:272,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:baseShadow,transition:"transform .12s ease, border-color .2s ease, box-shadow .2s ease"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="var(--border-med)";if(!isActive)e.currentTarget.style.boxShadow=MODE_CARD_TOKENS.BASE_SHADOW}} onMouseLeave={e=>{e.currentTarget.style.border=baseBorder;e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}} onMouseDown={e=>{e.currentTarget.style.transform="scale(0.99)"}} onMouseUp={e=>{e.currentTarget.style.transform="scale(1)"}} onFocus={e=>{e.currentTarget.style.outline="none";e.currentTarget.style.boxShadow=`${GLOW_LIME}, ${MODE_CARD_TOKENS.BASE_SHADOW}`}} onBlur={e=>{e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}}>
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:16}}>
    <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}>
      <div style={{width:50,height:50,borderRadius:14,background:MODE_CARD_TOKENS.ICON_INNER,border:`1px solid ${BORDER_CLR}`,display:"flex",alignItems:"center",justifyContent:"center",color:a.iconStroke,flexShrink:0}}>{icon}</div>
...
{board.map((p,i)=>{
  ...
  if(isLeader) return <div key={p.email} className="podium-glow lbRow is-active" style={{"--pod-c":accentColor,background:"var(--bg-2)",backgroundClip:"padding-box",border:`1px solid ${BORDER_MED}`,position:"relative",overflow:"hidden"}}>
  ...
  return <div key={p.email} className={`lbRow ${isMe?"is-active":""}`} style={{background:isMe?"var(--bg-2)":rowBg,backgroundClip:"padding-box",border:isMe?`1px solid ${BORDER_MED}`:`1px solid ${BORDER_CLR}`,position:"relative",overflow:"hidden"}}>
...
{upcoming.map(ev=>{const evR=rsvps.filter(r=>r.eventId===ev.id);const going=evR.some(r=>r.email===user.email);const exp=expanded===ev.id;
  return <div key={ev.id}>
    <div className={`ch eventCard ${going?"is-active":""}`} style={{width:"100%",background:going?"var(--bg-2)":"var(--bg-1)",border:`1px solid ${going?BORDER_MED:BORDER_CLR}`,borderRadius:exp?"16px 16px 0 0":16,textAlign:"left",position:"relative",overflow:"hidden",cursor:"pointer"}} onClick={()=>setExpanded(exp?null:ev.id)}>
      ...
      <button onClick={(e)=>{e.stopPropagation();toggleRsvp(ev.id)}} className="btn btn--secondary eventCta" style={{fontFamily:FD}}>
    ...
    {exp&&<div className="fade-up" style={{background:SURFACE,borderRadius:"0 0 16px 16px",padding:"16px 20px",border:`1px solid ${BORDER_CLR}`,borderTop:"none"}}>
      ...
      <button className="btn btn--secondary btn-v eventCta" onClick={()=>toggleRsvp(ev.id)} style={{marginBottom:14}}>
...
<div style={{flex:1,padding:`${showMiniHeader?"88px":"16px"} 20px 110px`,overflowY:"auto",position:"relative",zIndex:1}}>
  ...
  {tab==="feed"&&<div className="page pageShell page-feed fade-up" data-accent="feed" style={shellVars("feed")}><PageHeader title="FEED" subtitle="Daily team activity and momentum" accent="lime" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" rx="2"/><rect x="14" y="3" width="7" height="7" rx="2"/><rect x="3" y="14" width="7" height="7" rx="2"/><rect x="14" y="14" width="7" height="7" rx="2"/></svg>} actionLabel="Coach Mode" /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.feed.accent,fontSize:12,letterSpacing:"var(--tracking-default)"}}>TODAY'S PULSE</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10}}>Who's active, streaking, and needs attention</div></div><button className="pageHeaderPill" onClick={()=>setTab("players")}>View Team</button></div>
    ...
    <div className="accent-card" style={{background:"var(--bg-1)",borderRadius:18,padding:"20px 20px",border:`1px solid ${BORDER_CLR}`,boxShadow:SHADOW_CARD,marginBottom:20,position:"relative",overflow:"hidden"}}>
      <div style={{position:"absolute",top:0,left:0,width:4,height:"100%",background:ORANGE,borderRadius:"4px 0 0 4px"}}/>
