...
{allRows.length===0&&<Empty variant="drills" t="No duels yet" subtitle="Duels are your head-to-head battle lane. It's empty because no one has thrown or accepted a challenge yet. Set the tone by logging a score and launching the first challenge." cta="START A DUEL" onTap={()=>setTab("log-drill")} secondaryCta="HOW DUELS WORK" onSecondaryTap={()=>setShowGuide(true)} secondaryCtaVariant="tertiary"/>}
{allRows.length>0&&filteredRows.length===0&&<Empty variant="leaderboard" t="No duels in this filter" subtitle="This view only shows one duel category, and there are none here right now. Switch filters to scout active matchups and keep your competitive edge." cta="VIEW ALL DUELS" onTap={()=>setActiveFilter("all")} ctaVariant="secondary"/>}
...
{showBoard&&<div className="fade-up" style={{marginBottom:20}}>
  {board.length===0&&<Empty variant="lifting" t="No attendance on the board yet" subtitle="This leaderboard tracks S&C session attendance. It's empty because no RSVPs have been logged yet. Be the first to lock in and set the standard for the group." cta="RSVP NEXT SESSION" onTap={()=>setShowBoard(false)} ctaVariant="secondary"/>}
...
{upcoming.length===0&&<Empty variant="lifting" t="No upcoming sessions" subtitle="This section lists your next S&C work. It's empty because no future sessions are posted yet. Stay ready by logging today's lift so momentum keeps building." cta="LOG A SESSION" onTap={handleAddScLog} secondaryCta="CHECK EVENTS" onSecondaryTap={()=>setTab("events")} secondaryCtaVariant="tertiary"/>}
...
{board.length===0&&<Empty variant="leaderboard" t={`No ${unit} logged yet`} subtitle={`This leaderboard tracks team ${unit}. It's empty because nobody has logged data yet. Be first on the board and set the benchmark.`} cta="LOG SCORE NOW" onTap={()=>setTab("log-drill")}/>}
...
{my.length===0&&<Empty variant="drills" t="No shots logged yet" subtitle="This is your personal shooting tracker. It's empty because your first session isn't logged yet. Start with today's makes and build your confidence one rep at a time." cta="LOG SHOTS" onTap={()=>setTab("log-drill")}/>}
...
{showBoard&&<div className="fade-up" style={{marginBottom:20}}>
  ...
  {lbMode==="attend"&&<>{attendBoard.length===0&&<Empty variant="leaderboard" t="No RSVPs yet" subtitle="Attendance ranks commitment to team events. It's empty because players haven't RSVP'd yet. Lock in your next event to claim the first spot." cta="VIEW UPCOMING EVENTS" onTap={()=>setShowBoard(false)} ctaVariant="secondary"/>}{attendBoard.map((p,i)=>{const t=getTier(p.count);return <div key={p.email} style={{display:"flex",alignItems:"center",gap:12,background:CARD_BG,borderRadius:12,padding:"12px 14px",marginBottom:6,border:`1px solid ${p.email===user.email?VOLT+"33":BORDER_CLR}`}}><RB r={i+1} m={medals}/><Av n={p.name} sz={30} email={p.email}/><div style={{flex:1,display:"flex",alignItems:"center",gap:6}}><span style={{fontFamily:FD,color:LIGHT,fontSize:13,letterSpacing:1}}>{p.name.toUpperCase()}</span><span className="tb" style={{fontFamily:FB,fontSize:8,fontWeight:700,letterSpacing:1,padding:"1px 6px",borderRadius:3,color:t.color,background:`linear-gradient(90deg,${t.bg},${t.color}18,${t.bg})`}}>{t.name}</span></div><div style={{fontFamily:FD,color:t.color,fontSize:18}}>{p.count}</div></div>})}</>}
  {lbMode==="overall"&&<>{(()=>{const m={};scores.forEach(s=>{if(!m[s.email])m[s.email]={email:s.email,name:s.name||s.email,total:0};m[s.email].total+=s.score});const a=Object.values(m).sort((a,b)=>b.total-a.total);return a.length===0?<Empty variant="leaderboard" t="No scores yet" subtitle="Overall scores stack every drill point. It's empty because no drill totals are logged yet. Start the race by recording your first score today." cta="LOG DRILL SCORE" onTap={()=>setTab("log-drill")}/>:a.map((p,i)=><div key={p.email} style={{display:"flex",alignItems:"center",gap:12,background:CARD_BG,borderRadius:12,padding:"12px 14px",marginBottom:6,border:`1px solid ${p.email===user.email?VOLT+"33":BORDER_CLR}`}}><RB r={i+1} m={medals}/><Av n={p.name} sz={30} email={p.email}/><div style={{flex:1,fontFamily:FD,color:LIGHT,fontSize:13,letterSpacing:1}}>{p.name.toUpperCase()}{p.email===user.email?" (YOU)":""}</div><div style={{fontFamily:FD,color:VOLT,fontSize:18}}>{p.total}</div></div>)})()}</>}
  {lbMode==="streaks"&&<>{(()=>{const es=[...new Set(scores.map(s=>s.email))];const st=es.map(e=>({email:e,name:scores.find(s=>s.email===e)?.name||e,streak:calcStreak(scores.filter(s=>s.email===e))})).sort((a,b)=>b.streak-a.streak);return st.length===0?<Empty variant="leaderboard" t="No streaks yet" subtitle="Streaks measure consistency day after day. It's empty because no scoring streak has started yet. Log a score now and begin your run." cta="START MY STREAK" onTap={()=>setTab("log-drill")}/>:st.map((p,i)=><div key={p.email} style={{display:"flex",alignItems:"center",gap:12,background:CARD_BG,borderRadius:12,padding:"12px 14px",marginBottom:6,border:`1px solid ${p.email===user.email?VOLT+"33":BORDER_CLR}`}}><RB r={i+1} m={medals}/><Av n={p.name} sz={30} email={p.email}/><div style={{flex:1,fontFamily:FD,color:LIGHT,fontSize:13,letterSpacing:1}}>{p.name.toUpperCase()}</div><div style={{fontFamily:FD,color:ORANGE,fontSize:18}}>{p.streak} &#128293;</div></div>)})()}</>}
</div>}

{/* Upcoming */}
<SH isCoach={typeof u!=="undefined"&&u?.isCoach} t="UPCOMING EVENTS" s={`${upcoming.length} SCHEDULED`}/>
{upcoming.length===0&&<Empty variant="events" t="No upcoming events yet" subtitle="This section is where team events and meetups drop first. It's empty because nothing has been scheduled yet. Keep your edge by logging home reps while the next session gets posted." cta="LOG SHOTS" ctaVariant="primary" secondaryCta="OPEN TEAM FEED" secondaryCtaVariant="tertiary" onSecondaryTap={()=>setTab("feed")} onTap={()=>setTab("log-drill")}/>} 
...
{isDesktop&&<aside className="insights-panel"><div className="panel-title">COACH INSIGHTS</div><div className="placeholder">No insights widgets live yet. This area highlights momentum signals once activity, events, and S&C data are flowing. Next best action: add your first widget and spotlight one team habit to improve this week.</div></aside>}
  </div>;
}
...
{roster.length===0&&<Empty variant="leaderboard" t="No players on roster yet" subtitle="This roster shows athletes who joined your team and logged activity. It's empty because no player has completed that first step yet. Share the join code and invite your squad to check in today." cta="COPY JOIN CODE" onTap={()=>navigator.clipboard?.writeText(team?.joinCode||"")} secondaryCta="OPEN TEAM SETTINGS" onSecondaryTap={()=>setTab("settings")} secondaryCtaVariant="tertiary"/>}
