...
function StatTile({value,label,color,isPrimary=false,accent="home"}){
const isProgram=accent==="program";
return <div style={{background:isPrimary?"rgba(255,255,255,0.045)":CARD_BG,border:`1px solid ${isPrimary?(isProgram?"rgba(0, 229, 255, 0.28)":"rgba(200, 255, 26, 0.26)"):BORDER_CLR}`,borderRadius:14,padding:isPrimary?"13px 11px":"12px 10px",minHeight:98,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:isPrimary?`inset 0 1px 0 rgba(255,255,255,0.05), 0 10px 22px rgba(0,0,0,0.22)`:"inset 0 1px 0 rgba(255,255,255,0.02)"}}><div style={{fontFamily:FD,color:color||LIGHT,fontSize:isPrimary?26:24,lineHeight:1.05,wordBreak:"break-word"}}>{value}</div><div style={{fontFamily:FB,color:isPrimary?LIGHT:TOKENS.TEXT_SECONDARY,fontSize:10,fontWeight:isPrimary?800:700,letterSpacing:isProgram?"0.10em":"0.08em",textTransform:"uppercase"}}>{label}</div></div>
}

function ModeCard({title,subtitle,icon,stats,accent="home",isActive,onClick}){
const accentMap={
home:{tint:MODE_CARD_TOKENS.HOME_TINT,glow:MODE_CARD_TOKENS.HOME_GLOW,iconStroke:VOLT,gradientOrigin:"11% 9%",patternOpacity:.24},
program:{tint:MODE_CARD_TOKENS.PROGRAM_TINT,glow:MODE_CARD_TOKENS.PROGRAM_GLOW,iconStroke:CYAN,gradientOrigin:"88% 11%",patternOpacity:.34}
};
...
const baseShadow=isActive?`0 14px 30px rgba(0,0,0,.45), 0 0 0 1px ${a.glow} inset`:MODE_CARD_TOKENS.BASE_SHADOW;
const hoverShadow=`0 16px 32px rgba(0,0,0,.47), 0 0 0 1px ${a.glow} inset${accent==="home"?`, 0 0 18px ${a.glow}`:""}`;
return <button type="button" onClick={onClick} className="mode-card" style={{width:"100%",background:`radial-gradient(circle at ${a.gradientOrigin}, ${a.tint} 0%, transparent 56%), ${MODE_CARD_TOKENS.BASE_BG}`,border:baseBorder,borderRadius:24,padding:22,cursor:"pointer",textAlign:"left",position:"relative",minHeight:272,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:baseShadow,transition:"transform .12s ease, border-color .2s ease, box-shadow .2s ease"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=a.glow;e.currentTarget.style.boxShadow=hoverShadow}} onMouseLeave={e=>{e.currentTarget.style.border=baseBorder;e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}} onMouseDown={e=>{e.currentTarget.style.transform="scale(0.99)";e.currentTarget.style.boxShadow=`0 0 0 2px ${a.glow}, 0 12px 26px rgba(0,0,0,.43)`}} onMouseUp={e=>{e.currentTarget.style.transform="scale(1)"}} onFocus={e=>{e.currentTarget.style.outline="none";e.currentTarget.style.boxShadow=`0 0 0 3px ${MODE_CARD_TOKENS.FOCUS_RING}, 0 12px 24px rgba(0,0,0,.42), 0 0 0 1px ${a.glow} inset`}} onBlur={e=>{e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}}>
  {accent==="program"&&<div aria-hidden style={{position:"absolute",right:22,top:70,width:96,height:110,borderRadius:16,background:`repeating-linear-gradient(to bottom, rgba(255,255,255,${a.patternOpacity}) 0 1px, transparent 1px 12px)`,opacity:.45,pointerEvents:"none"}}/>}
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:16}}>
    <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}>
      <div style={{width:50,height:50,borderRadius:14,background:MODE_CARD_TOKENS.ICON_INNER,border:`1.5px solid ${a.glow}`,boxShadow:`inset 0 0 ${accent==="home"?11:7}px ${a.glow}`,display:"flex",alignItems:"center",justifyContent:"center",color:a.iconStroke,flexShrink:0}}>{icon}</div>
    ...
    <div style={{width:38,height:38,borderRadius:10,background:MODE_CARD_TOKENS.CHEVRON_BG,border:`1.5px solid ${a.glow}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 0 ${accent==="home"?9:5}px ${a.glow}`}}><svg width="16" height="16" viewBox="0 0 16 16"><path d="M6 3l5 5-5 5" stroke={a.iconStroke} strokeWidth="2.2" fill="none" strokeLinecap="round"/></svg></div>
  </div>
  <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:10}}>{stats.map((s,i)=><StatTile key={s.label} value={s.value} label={s.label} color={s.color} isPrimary={i===0} accent={accent}/>)}</div>
</button>
}
