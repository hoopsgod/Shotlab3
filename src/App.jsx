...
const PAGE_ACCENTS={
feed:{accent:"var(--accent)",glow:"var(--accent-soft)",bg:"rgba(255,210,77,0.14)"},
drills:{accent:"var(--accent)",glow:"var(--accent-soft)",bg:"rgba(255,210,77,0.14)"},
events:{accent:"var(--accent)",glow:"var(--accent-soft)",bg:"rgba(255,210,77,0.14)"},
sc:{accent:"var(--accent)",glow:"var(--accent-soft)",bg:"rgba(255,210,77,0.14)"},
players:{accent:"var(--accent)",glow:"var(--accent-soft)",bg:"rgba(255,210,77,0.14)"},
};
...
PROGRAM_GLOW:"rgba(255,210,77,.16)",
ICON_INNER:"rgba(255, 255, 255, 0.06)",
FOCUS_RING:"rgba(124,199,255,0.35)",
...
{id:"form-foundation",title:"Form Shooting Foundations",description:"Placeholder coach script: teach stance, one-motion release, and balanced follow-through for every rep.",videoUrl:"https://www.youtube.com/embed/2h5V9f2R6gQ",techniqueTips:["Start square and keep your guide hand quiet.","Hold your follow-through until the ball reaches the rim.","Track makes from five spots before adding range."]},
{id:"catch-shoot",title:"Catch-and-Shoot Footwork",description:"Placeholder coach script: explain shot prep footwork, early hands, and quick transfer into the shot pocket.",videoUrl:"https://www.youtube.com/embed/poL2mONn4cY",techniqueTips:["Show your hands early and load your hips before the catch.","Land on balance with feet under your shoulders.","Reset quickly so each rep mirrors game pace."]},
{id:"floater-series",title:"Floater and Touch Series",description:"Placeholder coach script: detail pace control, one-foot and two-foot floaters, and finishing touch.",videoUrl:"https://www.youtube.com/embed/aG8A4JhT82M",techniqueTips:["Read the help defender before choosing release point.","Keep eyes on the top corner of the square for soft touch.","Alternate right/left finishes to build both-hand confidence."]}
];
const EVENTS_INIT=[
...
const TIERS=[{min:0,name:"ROOKIE",color:"#555",bg:"#55555515"},{min:2,name:"IRON",color:"#A0A0A0",bg:"#A0A0A015"},{min:3,name:"BRONZE",color:"#A0A0A0",bg:"#A0A0A015"},{min:5,name:"SILVER",color:"#A0A0A0",bg:"#A0A0A015"},{min:8,name:"GOLD",color:"#FFD24D",bg:"#FFD24D15"},{min:12,name:"DIAMOND",color:CYAN,bg:CYAN+"15"}];
...
const STREAK_BADGES=[{days:7,name:"WEEK WARRIOR",icon:"7",color:"#A0A0A0"},{days:14,name:"TWO-WEEK GRIND",icon:"14",color:"#A0A0A0"},{days:30,name:"MONTHLY BEAST",icon:"30",color:"#FFD24D"},{days:60,name:"IRON WILL",icon:"60",color:CYAN},{days:100,name:"CENTURION",icon:"💯",color:VOLT}];
...
const getDrillAccentColor=name=>DRILL_ACCENTS[name]||"#FFD24D";
...
.pageShell{--pageAccent:#FFD24D;--pageAccentGlow:rgba(184,255,0,.35);--pageAccentBg:rgba(184,255,0,.08);--page-accent:#FFD24D;--page-accent-soft:rgba(184,255,0,.08);--page-accent-border:rgba(184,255,0,.35);display:flex;flex-direction:column;gap:var(--space-5);}
...
.pageHeaderPill{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:0 16px;border-radius:12px;border:1px solid rgba(255,255,255,0.22);background:transparent;font-family:${FB};font-size:10px;color:var(--text-2);font-weight:700;letter-spacing:var(--tracking-tight);text-transform:uppercase;transition:background .15s ease,border-color .15s ease,transform .1s ease,filter .15s ease;opacity:.9;}
...
.pageHeaderPillBrand{border:1px solid transparent;background:var(--page-accent,var(--color-primary,#FFD24D));color:#111827;box-shadow:0 4px 16px rgba(255,210,77,.30);}
.pageHeaderPillBrand:hover{text-decoration:none;filter:brightness(1.03);}
.pageHeaderPillBrand:focus-visible{outline-color:#FFD24D;}
...
useEffect(()=>{
  const storedScale = Number(localStorage.getItem("sl:text-scale") || "100");
  if (Number.isFinite(storedScale)) {
    setTextScale(Math.min(125, Math.max(90, storedScale)));
  }
},[]);

useEffect(()=>{
  document.documentElement.style.setProperty("--user-font-scale", `${textScale}%`);
  localStorage.setItem("sl:text-scale", String(textScale));
},[textScale]);
...
<aside className="text-scale-control" aria-label="Text size controls">
  <label htmlFor="text-scale-range">Text size</label>
  <div className="text-scale-row">
    <button type="button" className="btn btn--tertiary" onClick={()=>setTextScale(v=>Math.max(90,v-5))} aria-label="Decrease text size">A-</button>
    <input id="text-scale-range" type="range" min="90" max="125" step="5" value={textScale} onChange={(e)=>setTextScale(Number(e.target.value))} aria-label="Adjust text size"/>
    <button type="button" className="btn btn--tertiary" onClick={()=>setTextScale(v=>Math.min(125,v+5))} aria-label="Increase text size">A+</button>
  </div>
  <output htmlFor="text-scale-range" aria-live="polite">{textScale}%</output>
</aside>
</>;
}
...
{["login","register"].map(m=><button key={m} role="tab" aria-selected={mode===m} onClick={()=>{setMode(m);setErr("")}} style={{flex:1,height:44,borderRadius:10,border:"none",cursor:"pointer",fontFamily:FD,fontSize:16,letterSpacing:3,textTransform:"uppercase",transition:"all .15s ease",background:mode===m?VOLT:"transparent",color:mode===m?"#000000":"#555555",fontWeight:mode===m?700:600}}>{m==="login"?"SIGN IN":"REGISTER"}</button>)}
    ...
    {mode==="register"&&<>
      ...
      <input type="text" value={name} onChange={e=>{setName(e.target.value);setErr("")}} placeholder="First Last" style={{...inp,marginBottom:14}} onFocus={e=>{e.target.style.borderColor=VOLT;e.target.style.boxShadow="0 0 0 3px rgba(124,199,255,0.20)"}} onBlur={e=>{e.target.style.borderColor="#333333";e.target.style.boxShadow="none"}}/>
    </>}
    ...
    <input type="email" autoComplete="email" value={email} onChange={e=>{setEmail(e.target.value);setErr("")}} onKeyDown={e=>e.key==="Enter"&&(mode==="login"?doLogin():doRegister())} placeholder="you@example.com" style={{...inp,marginBottom:14}} onFocus={e=>{e.target.style.borderColor=VOLT;e.target.style.boxShadow="0 0 0 3px rgba(124,199,255,0.20)"}} onBlur={e=>{e.target.style.borderColor="#333333";e.target.style.boxShadow="none"}}/>
...
<div style={{flex:1,padding:"14px 16px 148px",overflowY:"auto",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s"}} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
  ...
  {(tab==="log-drill")&&!active&&!showShotStats&&<div className="fade-up">
    ...
    <div style={{background:CARD_BG,borderRadius:16,padding:"16px 16px",border:`1px solid ${BORDER_CLR}`,marginBottom:24}}>
      ...
      <div style={{marginBottom:12}}>
        ...
        <input aria-label="Date for shot log" type="date" value={shotDate} onChange={e=>setShotDate(e.target.value)} style={{width:"100%",padding:"12px 8px",background:BG,border:`1px solid ${BORDER_CLR}`,borderRadius:12,color:LIGHT,fontFamily:FB,fontSize:16,outline:"none"}} onFocus={e=>{e.target.style.borderColor=VOLT;e.target.style.boxShadow="0 0 0 3px rgba(124,199,255,0.20)"}} onBlur={e=>{e.target.style.borderColor="#333333";e.target.style.boxShadow="none"}}/>
      </div>
  ...
  {(tab==="home"||tab==="log-drill")&&active&&<div className="detail-enter" style={{textAlign:"center",paddingTop:12,position:"relative"}}>
    ...
    :<><button onClick={()=>{setActive(null);if(tab==="home")setTab("log-drill")}} style={{background:"none",border:"none",color:VOLT,fontFamily:FB,fontSize:13,cursor:"pointer",fontWeight:700,letterSpacing:2,marginBottom:32,padding:"8px 16px"}}>&#8592; BACK</button>
      ...
      {(()=>{const v=parseInt(input)||0;const pct=active.max>0?v/active.max:0;const glowColor=pct>=.9?VOLT:pct>=.6?ORANGE:pct>.01?"#FF6B6B":VOLT;const borderColor=v>0?glowColor:VOLT;
      ...
      {(()=>{const v=parseInt(input)||0;if(v<=0)return null;const pct=Math.round(v/active.max*100);const label=pct>=90?"ELITE":pct>=75?"STRONG":pct>=50?"SOLID":"KEEP PUSHING";const c=pct>=90?VOLT:pct>=75?VOLT:pct>=50?ORANGE:"#FF6B6B";
...
const pct=data.pct||0;const pcol=pct>=80?VOLT:pct>=50?ORANGE:"#FF6B6B";
...
{resolved.map(ch=>{
  ...
  const resultColor=isPending?MUTED:won?VOLT:tied?"#FFD24D":"#FF6B6B";
  ...
  return <ListItem key={ch.id+"-"+ch.ts} className="listRow" style={{background:CARD_BG,border:isPending?`1px solid ${ORANGE}22`:undefined,padding:`${spacing.md}px ${spacing.md}px`}}>
    ...
    <div style={{width:40,height:40,borderRadius:12,background:resultColor+"12",border:`1px solid ${resultColor}33`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
      ...
      :tied?<span style={{fontFamily:FD,color:"#FFD24D",fontSize:14}}>=</span>
      :<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#FF6B6B" strokeWidth="2.5"><path d="M18 6L6 18M6 6l12 12"/></svg>}
    </div>
    ...
    <div className="listRowRight">
      ...
      :<div className="listRowStat" style={{fontFamily:FD,color:won?VOLT:"#FF6B6B",fontSize:18}}>{myScore||"-"}<span style={{color:MUTED,fontSize:10}}> v </span><span style={{color:won?"#FF6B6B":VOLT}}>{oppScore}</span></div>}
...
<div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at top left, rgba(255,210,77,0.12) 0%, transparent 70%)",pointerEvents:"none"}}/>
...
<div className="card" style={{marginBottom:16,padding:"16px"}}>
  ...
  {logErr&&<div style={{fontFamily:FB,color:"#FF6B6B",fontSize:11,marginTop:8}}>{logErr}</div>}
...
<div className="card" style={{marginBottom:16,minHeight:100,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:"16px"}}>
  <div style={{fontFamily:FD,color:"#FFD24D",fontSize:48,fontWeight:900,lineHeight:1}}>{myScLogs.length}</div>
...
prog:{accent:CYAN,bg:"rgba(124,199,255,0.20)",glow:"0 0 18px rgba(124,199,255,0.36)",label:"📅"}
};
  ...
  {(()=>{
    ...
    return <div style={{overflowX:"auto",paddingBottom:8}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:4,marginTop:10,justifyContent:"flex-end"}}>
        ...
        {[0,.2,.4,.7,1].map((v,i)=><div key={i} style={{width:10,height:10,borderRadius:2,background:`rgba(255,210,77,${.1+v*.9})`,border:`1px solid ${VOLT}22`}}/>)}
...
<div style={{marginBottom:12}}>{INSTRUCTIONAL_CONTENT.slice(0,2).map(item=><DrillDetail key={item.id} title={item.title} description={item.description} videoUrl={item.videoUrl} techniqueTips={item.techniqueTips}/> )}</div>
...
<div className="uiDecor" aria-hidden="true" style={{width:42,height:42,borderRadius:12,background:"rgba(255,210,77,.16)",border:"1px solid rgba(255,210,77,.30)",display:"flex",alignItems:"center",justifyContent:"center"}}>
  ...
  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",position:"relative",gap:14}}>
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginTop:8}}>
        <svg className={rankFx?"rank-badge-flash":""} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#FFD24D" strokeWidth="2"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>
...
<div style={{flex:1,padding:`${showMiniHeader?"88px":"var(--space-4)"} var(--page-gutter) 110px`,overflowY:"auto",position:"relative",zIndex:1}}>
  ...
  {tab==="drills"&&!editD&&<div className="page pageShell fade-up" data-accent="drills" id="coach-drills-management" style={shellVars("drills")}><PageHeader title="DRILLS" subtitle="Skill plans, assignments, and drill library" accent="cyan" icon={<DrillIcon type="ft" size={22} color={PAGE_ACCENTS.drills.accent}/>} actionLabel="Add" onAction={()=>setShowNewDrill(true)} /><div className="heroModule"><div className="heroHead"><div><div className="heroKicker" style={{color:PAGE_ACCENTS.drills.accent}}>QUICK START DRILL</div><div className="heroSub">{drills.length} total drills ready to run today</div></div><button className="pageHeaderPill pageHeaderPillBrand" onClick={()=>setShowNewDrill(true)}>Add Drill</button></div><div className="drillsMetrics" style={{marginTop:10,marginBottom:0}}><div className="heroStat drillsMetricTile"><div className="heroStatVal">{drills.length}</div><div className="heroStatLbl">ACTIVE</div></div><div className="heroStat drillsMetricTile"><div className="heroStatVal">{programDrills.length}</div><div className="heroStatLbl">PROGRAM</div></div></div><div className="heroMetaStrip" style={{marginTop:8}}>Next action: assign program drills and review leaderboard entries for each session.</div><div className="heroActionRow"><button className="pageHeaderPill" onClick={()=>document.getElementById("coach-drills-management")?.scrollIntoView({behavior:"smooth"})}>Manage Drills</button><button className="pageHeaderPill" onClick={()=>setShowNewDrill(true)}>Quick Add</button></div></div>
    ...
    {drills.map(d=>{const dS=scores.filter(s=>s.drillId===d.id);const avg=dS.length?Math.round(dS.reduce((a,s)=>a+s.score,0)/dS.length*10)/10:0;return <div key={d.id} style={{display:"flex",gap:8,marginBottom:10,alignItems:"stretch"}}>
      ...
      <button onClick={()=>handleRemoveDrill(d.id)} style={{width:44,background:`#FF6B6B12`,border:`1px solid #FF6B6B33`,borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#FF6B6B" strokeWidth="2" strokeLinecap="round"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
      </button>
    </div>})}
      ...
      return <div key={ev.id} style={{marginBottom:10}}>
        ...
        {isExp&&<div className="fade-up" style={{background:SURFACE,borderRadius:"0 0 14px 14px",padding:"16px 16px",border:`1px solid ${BORDER_CLR}`,borderTop:"none"}}>
          ...
          {evR.map((r,i)=>{const t=getTier(rsvps.filter(rr=>rr.email===r.email).length);return <div key={i} style={{display:"flex",alignItems:"center",gap:8,background:CARD_BG,borderRadius:10,padding:"9px 12px",marginBottom:5,border:`1px solid ${BORDER_CLR}`}}>
            ...
            <button onClick={()=>removeRsvp(ev.id,r.email)} style={{background:"#FF6B6B12",border:"1px solid #FF6B6B33",borderRadius:7,color:"#FF6B6B",fontFamily:FD,fontSize:9,letterSpacing:1,padding:"4px 8px",cursor:"pointer",display:"flex",alignItems:"center",gap:3,whiteSpace:"nowrap"}}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#FF6B6B" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12"/></svg>NO-SHOW</button>
          </div>})}
          ...
          <button onClick={()=>removeEvent(ev.id)} className="btn-v cta-danger" style={{marginTop:14}}>
            <span className="cta-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#FF6B6B" strokeWidth="2"><path d="M3 6h18M8 6V4a1 1 0 011-1h6a1 1 0 011 1v2M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg></span>DELETE EVENT
          </button>
        </div>}
      </div>})}
  </div>}
  ...
  {tab==="players"&&!selP&&<div className="page pageShell" data-accent="players" style={shellVars("players")}><PageHeader title="PLAYERS" subtitle="Roster insights, development, and availability" accent="purple" icon={<UsersIcon size={20} color={PAGE_ACCENTS.players.accent}/>} actionLabel="Add" onAction={()=>document.getElementById("coach-add-player-form")?.scrollIntoView({behavior:"smooth"})} /><div className="heroModule"><div className="heroHead"><div><div className="heroKicker" style={{color:PAGE_ACCENTS.players.accent}}>ROSTER SNAPSHOT</div><div className="heroSub">{ups.length} players on roster</div></div><button className="pageHeaderPill" onClick={()=>document.getElementById("coach-add-player-form")?.scrollIntoView({behavior:"smooth"})}>Add Player</button></div><div className="heroBodyGrid"><div className="heroStatGroup"><div className="heroStatTop"><div className="heroStatVal" style={{color:PAGE_ACCENTS.players.accent}}>{ups.length}</div></div><div className="heroStatLbl">TOTAL ROSTER</div></div><div className="heroStatGroup"><div className="heroStatTop"><div className="heroStatVal" style={{color:inactivePlayersCount>0?ORANGE:VOLT}}>{inactivePlayersCount}</div></div><div className="heroStatLbl">INACTIVE THIS WEEK</div></div></div></div>
    <div id="coach-add-player-form" className="accent-card" style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:14,marginBottom:12}}>
      ...
      {profileErr&&<div style={{color:"#FF6B6B",fontSize:11,marginTop:8}}>{profileErr}</div>}
    ...
    <div style={{marginTop:32,paddingTop:20,borderTop:`1px solid ${BORDER_CLR}44`}}>
      <button onClick={deleteAccount} style={{width:"100%",padding:"12px",background:"transparent",border:`1px solid #FF6B6B33`,borderRadius:10,cursor:"pointer",fontFamily:FB,fontSize:12,color:"#FF6B6B",fontWeight:600,letterSpacing:1}}>Delete My Coach Account & Data</button>
  ...
  {tab==="sc"&&<div className="page pageShell fade-up" data-accent="sc" style={shellVars("sc")}><PageHeader title="S&C" subtitle="Strength blocks, readiness, and recovery" accent="blue" icon={<LiftIcon size={22} color={PAGE_ACCENTS.sc.accent}/>} actionLabel={showAddSC?"Close":"Add"} onAction={()=>setShowAddSC(!showAddSC)} /><div className="heroModule"><div className="heroHead"><div><div className="heroKicker" style={{color:PAGE_ACCENTS.sc.accent}}>TODAY'S LIFT</div><div className="heroSub">{scSessions[0]?`${scSessions[0].sport||scSessions[0].title} · ${scSessions[0].date}`:"No lift scheduled"}</div></div><button className="pageHeaderPill" onClick={()=>setShowAddSC(true)}>Add Session</button></div><div className="heroStats" style={{marginTop:10}}><div className="heroStat"><div className="heroStatVal">{scSessions.length}</div><div className="heroStatLbl">SESSIONS</div></div><div className="heroStat"><div className="heroStatVal">{scRsvps.length}</div><div className="heroStatLbl">RSVPS</div></div><div className="heroStat"><div className="heroStatVal">{scLogs.length}</div><div className="heroStatLbl">LOGS</div></div></div><div className="heroMetaStrip">Focus: keep attendance up and complete post-session lift logs for readiness tracking.</div></div>
    ...
    {scSessions.sort((a,b)=>a.date.localeCompare(b.date)).map(s=>{const sr=scRsvps.filter(r=>r.sessionId===s.id);
      return <div key={s.id} className="scSection" style={{display:"flex",alignItems:"center",gap:12,background:CARD_BG,borderRadius:12,padding:"14px 16px",marginBottom:8,border:`1px solid ${BORDER_CLR}`}}>
        ...
        <button onClick={()=>removeScSession(s.id)} style={{background:"none",border:"none",color:"#FF6B6B",cursor:"pointer",fontSize:16,padding:4}}>&#10005;</button>
      </div>;
    })}
  </div>}
  ...
  {tab==="settings"&&<div className="page pageShell fade-up" data-accent="feed" id="coach-settings" style={shellVars("feed")}><PageHeader title="SETTINGS" subtitle="Program identity, premium polish, and account controls" accent="lime" icon={<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 3.09 14H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 8.92 4H9a1.65 1.65 0 0 0 1-1.51V2a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51h.01a1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9v.01a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>} />
    ...
    <div className="accent-card" style={{background:SURFACE,border:`1px solid ${BORDER_CLR}`,borderRadius:16,padding:"16px 16px",marginBottom:18}}>
      ...
      {brandingWarnings.length>0&&<div style={{marginTop:10,padding:"8px 10px",borderRadius:10,border:`1px solid ${alphaFromHex("#FFC166",0.45)}`,background:alphaFromHex("#FFC166",0.1)}}>{brandingWarnings.map(w=><div key={w} style={{fontFamily:FB,color:"#FFD79A",fontSize:10,lineHeight:1.4}}>⚠ {w}</div>)}</div>}{brandingMsg&&<div style={{fontFamily:FB,color:brandingMsg.includes("saved")?"#9CE77B":"#FF6B6B",fontSize:10,marginTop:8}}>{brandingMsg}</div>}
    </div>
    <button onClick={deleteAccount} style={{width:"100%",padding:"12px",background:"transparent",border:`1px solid #FF6B6B33`,borderRadius:10,cursor:"pointer",fontFamily:FB,fontSize:12,color:"#FF6B6B",fontWeight:600,letterSpacing:1}}>Delete My Coach Account & Data</button>
  </div>}
...
{drillStats.map(d=>{const accentColor=getDrillAccentColor(d.name);return <div key={d.id} style={{background:CARD_BG,borderRadius:14,padding:"16px 18px",border:`1px solid ${BORDER_CLR}`,borderLeft:`5px solid ${accentColor}`,marginBottom:10}}>
  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
    ...
    <div style={{fontFamily:FB,fontSize:9,fontWeight:700,letterSpacing:1,padding:"2px 8px",borderRadius:5,
      color:d.trend==="up"?"#FFD24D":d.trend==="down"?"#FF6B6B":MUTED,
      background:d.trend==="up"?"#FFD24D15":d.trend==="down"?"#FF6B6B15":BORDER_CLR}}>
...
<div style={{marginTop:32,paddingTop:24,borderTop:`1px solid ${BORDER_CLR}44`}}>
  ...
  {!confirmDel?<button onClick={()=>setConfirmDel(true)} style={{width:"100%",padding:"14px",background:"transparent",border:`1px solid #FF6B6B44`,borderRadius:12,cursor:"pointer",fontFamily:FB,fontSize:13,color:"#FF6B6B",fontWeight:600,letterSpacing:1}}>
    Delete Account & All Data
  </button>
  :<div className="fade-up" style={{background:"#FF6B6B08",borderRadius:16,padding:"20px",border:`1px solid #FF6B6B33`}}>
    <div style={{fontFamily:FD,color:"#FF6B6B",fontSize:18,letterSpacing:3,marginBottom:8}}>DELETE ACCOUNT?</div>
...
{roster.map(p=>{const c=p.statusMeta.color;const isNudged=nudged.includes(p.email);
  ...
  return <div key={p.email} style={{display:"flex",background:CARD_BG,borderRadius:14,marginBottom:10,border:`1px solid ${p.statusMeta.pill==="INACTIVE"?"#FF6B6B33":BORDER_CLR}`,overflow:"hidden"}}>
    ...
    {p.statusMeta.pill==="INACTIVE"&&<button onClick={()=>{if(!isNudged)setNudged(n=>[...n,p.email])}} style={{minHeight:40,padding:"0 12px",borderRadius:8,border:`1px solid ${isNudged?VOLT+"44":"#FF6B6B44"}`,background:isNudged?VOLT+"12":"#FF6B6B12",cursor:"pointer",fontFamily:FB,fontSize:10,fontWeight:700,letterSpacing:1,color:isNudged?VOLT:"#FF6B6B",whiteSpace:"nowrap",width:"100%"}}>
...
const AVG=[["#FFD24D","#00E5FF"],["#FFD24D","#FFD24D"],["#00E5FF","#FFD24D"],["#FFD24D","#FFD24D"],["#A0A0A0","#00E5FF"],["#FFD24D","#FFD24D"],["#FFD24D","#00E5FF"],["#FFD24D","#FFD24D"]];
...
function BrandBackdrop(){return <><div style={{position:"fixed",inset:0,background:"radial-gradient(ellipse 80% 40% at 50% 0%, rgba(255,210,77,0.10) 0%, transparent 100%)",pointerEvents:"none",zIndex:0}}/><div style={{position:"fixed",left:"50%",top:"50%",transform:"translate(-50%,-35%)",opacity:.03,pointerEvents:"none",zIndex:0,width:180}}><SLLogo size={180}/></div></>}
