...
import { getPlayerPriorities, savePlayerPriorities } from "./lib/playerPriorities.js";
import {
...
const saveCoachPriorities = useCallback(async (nextPriorities) => {
  const sanitized = sanitizeCoachPriorities(nextPriorities);
  // Verification path: Coach Save -> localStorage(key) -> hydrate app state -> Player dashboard render.
  savePlayerPriorities(sanitized);
  setCoachPriorities(sanitized);
...
const handleSaveCoachPriorities=async()=>{
  ...
  const result=await onSaveCoachPriorities?.(next);
  if(result?.ok){
