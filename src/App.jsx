...
const isTeamAdmin=(actor,teamId)=>requireCoach(actor,teamId)&&teams.some(t=>t.id===teamId&&t.ownerCoachId===actor.email);
...
const removeDrill=async(id)=>{if(!isTeamAdmin(user,user?.teamId))return{ok:false,err:"Only team admins can delete drills."};await P("sl:drills",drills.filter(d=>d.id!==id),setDrills);return{ok:true}};
...
const removeProgramDrill=async(id)=>{if(!isTeamAdmin(user,user?.teamId))return{ok:false,err:"Only team admins can delete drills."};await P("sl:program-drills",programDrills.filter(d=>d.id!==id),setProgramDrills);return{ok:true}};
...
if(!isTeamAdmin(user,user?.teamId))return{ok:false,err:"Only team admins can update team branding."};
...
const isCurrentUserTeamAdmin=isTeamAdmin(user,user?.teamId);
...
// COACH SCREEN
// ═══════════════════════════════════════
function Coach({u,team,regenerateJoinCode,addRosterPlayer,playerProfiles,drills,programDrills,scores,players,updateDrill,addDrill,removeDrill,addProgramDrill,removeProgramDrill,events,rsvps,addEvent,removeEvent,removeRsvp,addRsvp,scSessions,scRsvps,scLogs=[],addScSession,removeScSession,shotLogs,logout,deleteAccount,openTeamBranding,coachTextSize="standard",canManageBranding=false,canDeleteDrills=false}){
...
{u?.isCoach&&<div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}>{canManageBranding?<button onClick={openTeamBranding} style={{padding:"8px 12px",borderRadius:10,border:"1px solid var(--team-brand-badge-border)",background:"var(--team-brand-badge-bg)",color:"var(--team-brand-badge-text)",fontFamily:FB,fontSize:12,fontWeight:700,cursor:"pointer"}}>Team Branding Settings</button>:<div style={{padding:"8px 12px",borderRadius:10,border:`1px solid ${BORDER_CLR}`,background:"rgba(255,255,255,0.03)",color:MUTED,fontFamily:FB,fontSize:11,fontWeight:700,letterSpacing:".04em",textTransform:"uppercase"}}>Admin access required to change logo</div>}</div>}
<CoachHero
  heroRef={heroRef}
  userName={u.name}
  accentColor={ORANGE}
    ...
    {drills.map(d=>{const dS=scores.filter(s=>s.drillId===d.id);const avg=dS.length?Math.round(dS.reduce((a,s)=>a+s.score,0)/dS.length*10)/10:0;return <div key={d.id} style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:16,padding:"14px 14px 12px",marginBottom:10}}>
      ...
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:12,paddingTop:10,borderTop:`1px solid ${BORDER_CLR}66`}}>
        ...
        {d.isDefaultDemo?<span style={{minHeight:34,background:"transparent",border:`1px solid ${BORDER_CLR}`,borderRadius:999,display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"0 12px",color:MUTED,fontFamily:FB,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:".06em"}}>DEMO DEFAULT</span>:canDeleteDrills?<button onClick={()=>handleRemoveDrill(d.id)} style={{minHeight:34,background:"transparent",border:`1px solid #FF454544`,borderRadius:999,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"0 12px",color:"#FF6A6A",fontFamily:FB,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:".06em"}}>
          ...
          Delete
        </button>:<span style={{minHeight:34,background:"transparent",border:`1px solid ${BORDER_CLR}`,borderRadius:999,display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"0 12px",color:MUTED,fontFamily:FB,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:".06em"}}>Admin only</span>}
      </div>
    </div>})}
