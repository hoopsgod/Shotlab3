...
if(code==="rate_limited")return{code:"rate_limited",message:"A join request is already in progress. Please wait."};
...
if(code==="consume_membership_insert_failed")return{code:"consume_membership_insert_failed",message:"Backend could not insert membership for this player."};
...
const diagnostic=mapConsumeDiagnostic("rate_limited",429);
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeHttpStatus:429,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:diagnostic.message,finalRouteDecision:"stay_join_team",error:diagnostic.code}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeHttpStatus:400,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:diagnostic.message,consumeTokenPresent:"no",consumeTeamIdUsed:"",consumeUserEmail:"",consumeUserUuid:"",membershipInsertStatus:"failed",finalRouteDecision:"stay_join_team",inviteContextSaved:"no",inviteContextStorageKey:INVITE_CONTEXT_STORAGE_KEY,inviteContextTokenPresent:"no",inviteContextTeamId:"",inviteContextSubject:"",currentUserEmail:"",contextSubjectMatchesUser:"no",error:diagnostic.code}}));
return{ok:false,err:diagnostic.message,diagnosticCode:diagnostic.code,diagnosticMessage:diagnostic.message};
}
if(!resolvedContext?.joinContextToken){
const diagnostic=mapConsumeDiagnostic("consume_context_token_missing",400);
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeHttpStatus:400,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:`${diagnostic.message} Checked ${INVITE_CONTEXT_STORAGE_KEY}.`,consumeTokenPresent:"no",consumeTeamIdUsed:"",consumeUserEmail:currentUserEmail,consumeUserUuid:currentUserEmail,membershipInsertStatus:"failed",finalRouteDecision:"stay_join_team",inviteContextSaved:"no",inviteContextStorageKey:INVITE_CONTEXT_STORAGE_KEY,inviteContextTokenPresent:"no",inviteContextTeamId:"",inviteContextSubject:"",currentUserEmail,contextSubjectMatchesUser:"no",error:`missing_invite_context:${INVITE_CONTEXT_STORAGE_KEY}`}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeHttpStatus:null,consumeDiagnosticCode:"",consumeDiagnosticMessage:"",consumeTokenPresent:resolvedContext.joinContextToken?"yes":"no",consumeTeamIdUsed:resolvedContext.teamId||"",consumeUserEmail:currentUserEmail,consumeUserUuid:currentUserEmail,membershipInsertStatus:"pending",finalRouteDecision:"pending_consume",inviteContextSaved:"yes",inviteContextStorageKey:INVITE_CONTEXT_STORAGE_KEY,inviteContextTokenPresent:resolvedContext.joinContextToken?"yes":"no",inviteContextTeamId:resolvedContext.teamId||"",inviteContextSubject:subject||"",currentUserEmail,contextSubjectMatchesUser}}));
if(contextSubjectMatchesUser!=="yes"){
const diagnostic=mapConsumeDiagnostic("consume_subject_mismatch",400);
setDataDebug(prev=>({...prev,join:{...prev.join,consumeHttpStatus:400,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:diagnostic.message,membershipInsertStatus:"failed",finalRouteDecision:"stay_join_team",error:diagnostic.code}}));
return{ok:false,err:diagnostic.message,diagnosticCode:diagnostic.code,diagnosticMessage:diagnostic.message};
}
try{
consumeInFlightRef.current=true;
const res=await fetch(consumeEndpoint,{method:"POST",headers:{"Content-Type":"application/json","x-user-id":actor.email},body:JSON.stringify({join_context_token:resolvedContext.joinContextToken,subject_key:subject,client_request_id:clientRequestId||genId("join")})});
const body=await res.json().catch(()=>({}));
if(!res.ok){
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeHttpStatus:res.status,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:diagnosticMessage,consumeUserUuid:String(body?.resolved_uuid||prev.join.consumeUserUuid||""),membershipInsertStatus:"failed",finalRouteDecision:"stay_join_team",error:diagnostic.code}}));
return{ok:false,err:diagnostic.message,diagnosticCode:diagnostic.code,diagnosticMessage:diagnostic.message};
}
await savePendingJoinContext(null);
setDataDebug(prev=>({...prev,join:{...prev.join,consumeHttpStatus:res.status,consumeDiagnosticCode:"",consumeDiagnosticMessage:"",consumeUserUuid:String(body?.resolved_user_uuid||prev.join.consumeUserUuid||""),membershipInsertStatus:"success",finalRouteDecision:"apply_profile_update"}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeHttpStatus:500,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:diagnostic.message,membershipInsertStatus:"failed",finalRouteDecision:"stay_join_team",error:diagnostic.code}}));
return{ok:false,err:diagnostic.message,diagnosticCode:diagnostic.code,diagnosticMessage:diagnostic.message};
}finally{
...
setDataDebug(prev=>({...prev,join:{...prev.join,status:"backend_match",lookupCount:1,matchedTeamId:resolvedTeamId,consumeUserUuid:joined?.resolvedUserUuid||prev.join.consumeUserUuid}}));
