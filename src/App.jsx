...
<div style={{flex:1,padding:`${showMiniHeader?"88px":"16px"} 20px 110px`,overflowY:"auto",position:"relative",zIndex:showAdd?40:1}}>
  ...
  {tab==="feed"&&<div className="page pageShell page-feed fade-up" data-accent="feed" style={shellVars("feed")}><PageHeader title="COACH HOME" subtitle="Today-first command surface for your program" accent="lime" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" rx="2"/><rect x="14" y="3" width="7" height="7" rx="2"/><rect x="3" y="14" width="7" height="7" rx="2"/><rect x="14" y="14" width="7" height="7" rx="2"/></svg>} actionLabel="Coach Mode" />
    ...
    {(()=>{
      ...
      return <>
        <section className="accent-card" style={{background:"radial-gradient(125% 140% at 0% 0%, color-mix(in srgb,var(--accent) 32%, transparent) 0%, rgba(11,13,16,0.985) 48%, rgba(6,8,12,1) 100%)",border:`1px solid color-mix(in srgb,var(--accent) 42%, transparent)`,borderRadius:24,padding:isDesktop?"30px 30px 26px":"22px 18px 18px",marginBottom:12,boxShadow:"0 26px 56px rgba(0,0,0,0.38)"}}>
          <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"flex-start",marginBottom:14}}>
            <div>
              <div style={{fontFamily:FB,color:"var(--text-3)",fontSize:10,fontWeight:700,letterSpacing:"0.11em"}}>TEAM MISSION · TODAY'S PRACTICE</div>
              <div style={{fontFamily:FD,color:"var(--text-1)",fontSize:isDesktop?36:30,lineHeight:0.95,marginTop:8,letterSpacing:"0.04em"}}>{session?.title||"No session scheduled"}</div>
            ...
            <button className="pageHeaderPill pageHeaderPillBrand" style={{minHeight:42,padding:"0 16px",fontSize:12,borderColor:"var(--accent)",boxShadow:"0 0 0 1px rgba(200,255,26,0.25), 0 8px 20px rgba(120,255,77,0.15)"}} onClick={()=>setTab("events")}>{session?"Open Session":"Add Session"}</button>
          </div>
          <div style={{display:"grid",gridTemplateColumns:isDesktop?"1.2fr 1fr":"1fr",gap:10}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:8}}>
              {[{k:"Readiness",v:readinessCopy},{k:"Participation",v:session?`${rsvpPct}% RSVP`:"No RSVPs"}].map(item=><div key={item.k} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.09)",borderRadius:12,padding:"10px 10px"}}><div style={{fontFamily:FB,fontSize:9,color:"var(--text-3)",letterSpacing:"0.06em"}}>{item.k}</div><div style={{fontFamily:FD,fontSize:26,color:"var(--text-1)",lineHeight:0.95,marginTop:5}}>{item.k==="Participation"?item.v.split(" ")[0]:activeTodaySet.size}</div><div style={{fontFamily:FB,fontSize:11,color:"var(--text-2)",marginTop:4}}>{item.k==="Participation"?"Team RSVP rate":item.v}</div></div>)}
            </div>
            <div style={{display:"grid",gridTemplateColumns:isDesktop?"1fr 1fr":"repeat(2,minmax(0,1fr))",gap:8}}>
