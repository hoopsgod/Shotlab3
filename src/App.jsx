...
return <nav className="bottom-nav" role="navigation" aria-label="Main navigation" style={{"--nav-accent":navAccent,position:"fixed",left:0,right:0,bottom:0,display:"flex",justifyContent:"space-evenly",alignItems:"center",height:"var(--nav-height)",paddingBottom:"env(safe-area-inset-bottom)",zIndex:20}}>{items.map(t=>{const a=active===t.k;
const tabAccent="var(--accent)";
return <button key={t.k} aria-label={t.l} aria-current={a?"page":undefined} className={`bottom-nav-item tab ${a?"is-active active":""}`} onClick={()=>onChange(t.k)} style={{"--tab-accent":tabAccent,flex:1,minWidth:48,minHeight:48,height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:4,padding:"8px 4px 6px",position:"relative",background:"none",border:"none",cursor:"pointer",transition:"color 150ms ease-out",outlineOffset:2}}>
<div className="nav-indicator"></div><div className="tab-icon" style={{position:"relative"}}>{t.svg}</div>
