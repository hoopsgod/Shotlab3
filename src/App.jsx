...
await P("sl:player-profiles",playerProfiles.map(pp=>pp.id===profile.id?{...pp,archived:true,rosterStatus:"archived",hideFromLeaderboards:true,archivedAt:Date.now(),archivedBy:user.email}:pp),setPlayerProfiles);
return{ok:true};
}
...
const profile=findRosterProfile(playerEmail);
if(!profile)return result;
await P("sl:player-profiles",playerProfiles.map(pp=>pp.id===profile.id?{...pp,teamId:null,rosterStatus:"removed",removedFromTeamId:user.teamId,removedAt:Date.now(),removedBy:user.email,hideFromLeaderboards:true}:pp),setPlayerProfiles);
return{ok:true};
}
await P("sl:players",result.players,setPlayers);
return{ok:true};
...
await P("sl:player-profiles",playerProfiles.filter(pp=>pp.id!==profile.id),setPlayerProfiles);
return{ok:true};
}
await Promise.all([
...
const playerActiveTeamPlayerKeySet=playerActiveTeamIdentity.keySet;
const playerActiveTeamPlayerEmailSet=playerActiveTeamIdentity.emailSet;
const playerActiveTeamPlayerNameSet=playerActiveTeamIdentity.nameSet;
const teamFacingScores=useMemo(()=>filterActiveTeamPlayerRows(scores,playerActiveTeamPlayerEmailSet,playerActiveTeamPlayerKeySet),[scores,playerActiveTeamPlayerEmailSet,playerActiveTeamPlayerKeySet]);
const teamFacingShotLogs=useMemo(()=>filterActiveTeamPlayerRows(shotLogs,playerActiveTeamPlayerEmailSet,playerActiveTeamPlayerKeySet),[shotLogs,playerActiveTeamPlayerEmailSet,playerActiveTeamPlayerKeySet]);
...
const my=useMemo(()=>teamFacingScores.filter(s=>(!u?.teamId||!String(s?.teamId||s?.team_id||"")||String(s?.teamId||s?.team_id)===String(u.teamId))&&rowMatchesPlayerIdentity(s)),[teamFacingScores,u?.teamId,rowMatchesPlayerIdentity]);
...
const teamFacingProgramScores=useMemo(()=>filterActiveTeamPlayerRows(teamProgramScores,playerActiveTeamPlayerEmailSet,playerActiveTeamPlayerKeySet),[teamProgramScores,playerActiveTeamPlayerEmailSet,playerActiveTeamPlayerKeySet]);
const programScores=useMemo(()=>getProgramScoresForPlayer(teamFacingProgramScores,u.email,u.teamId),[teamFacingProgramScores,u.email,u.teamId]);
...
const playerDashboardHomeLeaderboardRows=useMemo(()=>buildAtHomeLeaderboardRows({scores:teamFacingScores,shotLogs:teamFacingShotLogs,programDrills,players:playerLeaderboardPlayers,limit:3}),[teamFacingScores,teamFacingShotLogs,programDrills,playerLeaderboardPlayers]);
const filteredPlayerHomeLeaderboardRows=useMemo(()=>filterActiveTeamLeaderboardRows(playerLeaderboardState.rows,playerActiveTeamPlayerKeySet,playerActiveTeamPlayerEmailSet,playerActiveTeamPlayerNameSet),[playerLeaderboardState.rows,playerActiveTeamPlayerKeySet,playerActiveTeamPlayerEmailSet,playerActiveTeamPlayerNameSet]);
useEffect(()=>{if(!isShotLabDebugMode())return;(playerLeaderboardState.rows||[]).forEach(row=>{const filtered=!filteredPlayerHomeLeaderboardRows.some(allowed=>String(allowed?.email||allowed?.player_email||allowed?.player_id||allowed?.playerId||allowed?.id)===String(row?.email||row?.player_email||row?.player_id||row?.playerId||row?.id));if(filtered)console.warn("[player-leaderboard] filtered non-roster leaderboard row",{normalizedRowEmail:normalizeEmail(row?.email||row?.player_email),rowIdentity:{playerId:row?.playerId,player_id:row?.player_id,userId:row?.userId,user_id:row?.user_id,profileId:row?.profileId,profile_id:row?.profile_id,id:row?.id},teamId:u?.teamId||"",activeRosterCount:playerActiveTeamIdentity.players.length,reason:"excluded_not_active_roster_member"});});},[playerLeaderboardState.rows,filteredPlayerHomeLeaderboardRows,u?.teamId,playerActiveTeamIdentity.players.length]);
const playerDashboardLeaderboardRows=filteredPlayerHomeLeaderboardRows.length?filteredPlayerHomeLeaderboardRows:playerDashboardHomeLeaderboardRows;
...
<div ref={playerScrollRef} className="player-scroll-container" style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px var(--player-scroll-bottom-padding)",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {tab==="home"&&!active&&<div className={slideClass} key="home">
    {(()=>{
      ...
      const recentPlayerActivity=deriveActivityFeedItems({view:"player",user:u,events,rsvps,shotLogs:teamFacingShotLogs,players:playerLeaderboardPlayers,scores:teamFacingScores,today});
  ...
  {tab==="leaderboards"&&!active&&<div className={slideClass} key="leaderboards">
    <PremiumLeaderboardsHub viewerRole="player" leaderboardRows={playerDashboardLeaderboardRows} leaderboardStatus={playerDashboardLeaderboardStatus} userEmail={u?.email||""} currentUser={u} programScores={teamFacingProgramScores} programDrills={programDrills} players={playerLeaderboardPlayers} teamId={u?.teamId||""} homeScores={teamFacingScores} shotLogs={teamFacingShotLogs} />
  </div>}
...
<div style={{flex:1,padding:`${showMiniHeader?"74px":"12px"} 16px 104px`,overflowY:"auto",position:"relative",zIndex:showAdd?40:1}}>
  ...
  {tab==="leaderboards"&&<div className="page pageShell fade-up" data-accent="feed" style={shellVars("feed")}><DashboardReturnButton onClick={()=>setTab("feed")} />
    <PremiumLeaderboardsHub viewerRole="coach" leaderboardRows={canonicalCoachHomeLeaderboardRows} leaderboardStatus={coachDashboardLeaderboardStatus} programScores={safeProgramScores} programDrills={programDrills} players={leaderboardPlayers} teamId={u?.teamId||""} homeScores={safeScores} shotLogs={safeShotLogs} />
  </div>}
  ...
  {tab==="players"&&!selP&&<div className="page pageShell" data-accent="players" style={shellVars("players")}><DashboardReturnButton onClick={()=>setTab("feed")} /><PageHeader title="PLAYERS" subtitle="Roster insights, development, and availability" accent="purple" icon={<UsersIcon size={20} color={PAGE_ACCENTS.players.accent}/>} actionLabel="Add" onAction={()=>document.getElementById("coach-add-player-form")?.scrollIntoView({behavior:"smooth"})} /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.players.accent,fontSize:12,letterSpacing:"var(--tracking-default)"}}>ROSTER SNAPSHOT</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10}}>{ups.length} players on roster</div></div></div></div>
    ...
    <CoachRoster players={coachRosterPlayers} scores={safeScores} shotLogs={safeShotLogs} drills={drills} nudged={nudged} setNudged={setNudged} onRemovePlayer={removeRosterPlayer} onSelectPlayer={setSelP}/>
