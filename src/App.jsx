...
{/* Leaderboard toggle */}
<button onClick={()=>setShowBoard(!showBoard)} className="ch" style={{width:"100%",minHeight:44,display:"flex",alignItems:"center",justifyContent:"space-between",background:`linear-gradient(135deg,${CARD_BG},#141414)`,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:"14px 18px",marginBottom:16,cursor:"pointer",textAlign:"left"}}>
...
{upcoming.map(s=>{const sr=scRsvps.filter(r=>r.sessionId===s.id);const going=sr.some(r=>r.email===user.email);const exp=expanded===s.id;
  return <div key={s.id} style={{marginBottom:12}}>
    <button onClick={()=>setExpanded(exp?null:s.id)} className="ch" style={{width:"100%",minHeight:44,background:`linear-gradient(135deg,${CARD_BG},#141414)`,border:`1px solid ${going?SC_COLOR+"33":BORDER_CLR}`,borderRadius:exp?"16px 16px 0 0":16,padding:"18px 20px",textAlign:"left",cursor:"pointer",position:"relative",overflow:"hidden"}}>
...
{/* Timeline strip — next 3 events with explicit label */}
{upcoming.length>0&&<div style={{display:"flex",flexDirection:"column",gap:8,paddingLeft:4}}>
<div style={{fontFamily:FB,fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:`${MUTED}`}}>Upcoming timeline (preview)</div>
...
<div style={{width:8,height:8,borderRadius:"50%",background:going?`${VOLT}AA`:`${MUTED}55`,border:`1px solid ${going?`${VOLT}66`:`${MUTED}40`}`}}/>
<div style={{fontFamily:FB,fontSize:8,fontWeight:700,letterSpacing:1,color:MUTED,whiteSpace:"nowrap"}}>{ev.date.slice(5)}</div>
</div>
{i<Math.min(upcoming.length,3)-1&&<div style={{flex:1,height:1,background:`linear-gradient(90deg,${going?`${VOLT}33`:`${BORDER_CLR}99`},${BORDER_CLR})`,margin:"0 6px",marginBottom:14,opacity:.28}}/>}
</div>})}
</div>
...
{showBoard&&<div className="fade-up" style={{marginBottom:20}}>
  <div style={{display:"flex",gap:4,marginBottom:14}}>{[{k:"attend",l:"ATTENDANCE"},{k:"overall",l:"DRILL SCORES"},{k:"streaks",l:"STREAKS"}].map(t=><button key={t.k} onClick={()=>setLbMode(t.k)} style={{flex:1,minHeight:44,padding:"9px 6px",borderRadius:10,border:lbMode===t.k?"none":`1px solid ${BORDER_CLR}`,cursor:"pointer",fontFamily:FD,fontSize:12,letterSpacing:1,background:lbMode===t.k?CYAN:CARD_BG,color:lbMode===t.k?BG:MUTED}}>{t.l}</button>)}</div>
...
{upcoming.map(ev=>{const evR=rsvps.filter(r=>r.eventId===ev.id);const going=evR.some(r=>r.email===user.email);const exp=expanded===ev.id;
  return <div key={ev.id} style={{marginBottom:12}}>
    <div className="ch" style={{width:"100%",background:`linear-gradient(135deg,${CARD_BG},#141414)`,border:`1px solid ${going?VOLT+"33":BORDER_CLR}`,borderRadius:exp?"16px 16px 0 0":16,padding:"18px 20px",textAlign:"left",position:"relative",overflow:"hidden"}}>
      ...
      <button onClick={(e)=>{e.stopPropagation();toggleRsvp(ev.id)}} style={{marginTop:12,minHeight:44,padding:"8px 0",width:"100%",borderRadius:10,border:"none",background:VOLT,cursor:"pointer",fontFamily:FD,fontSize:12,letterSpacing:3,color:BG,display:"flex",alignItems:"center",justifyContent:"center",gap:6,transition:"all .2s"}}>
...
function CourtDivider({color=VOLT,my=20}){return <div aria-hidden="true" style={{margin:`${my}px 0`,position:"relative",height:20,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",pointerEvents:"none"}}><svg width="100%" height="20" viewBox="0 0 400 20" preserveAspectRatio="none" fill="none" style={{position:"absolute",inset:0,opacity:.18}}><line x1="0" y1="10" x2="162" y2="10" stroke={color} strokeWidth="1"/><path d="M162 10Q200 1 238 10" stroke={color} strokeWidth="1" fill="none"/><line x1="238" y1="10" x2="400" y2="10" stroke={color} strokeWidth="1"/></svg><div style={{width:4,height:4,borderRadius:"50%",background:color,opacity:.2,position:"relative",zIndex:1}}/></div>}
function DividerDot(){return <div aria-hidden="true" style={{display:"flex",alignItems:"center",gap:10,width:"100%",margin:"14px 0",pointerEvents:"none",opacity:.22}}><div style={{height:1,background:BORDER_CLR,flex:1}}/><div style={{width:3,height:3,borderRadius:"50%",background:MUTED}}/><div style={{height:1,background:BORDER_CLR,flex:1}}/></div>}
