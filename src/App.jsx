...
<div className={tab==="home"&&!active?"player-mobile-header player-home-compactHeader":"player-mobile-header"} style={{position:"sticky",top:0,zIndex:10,padding:isDesktop?"max(0px,env(safe-area-inset-top)) 16px 10px":"var(--player-mobile-header-pad, max(0px,env(safe-area-inset-top)) 20px 10px)",background:"linear-gradient(180deg, rgba(11,13,16,0.96), rgba(11,13,16,0.88))",borderBottom:`1px solid ${TOKENS.BG_SUBTLE}`}}>
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
    <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}>
      <button className="player-mobile-headerAvatar" aria-label="Open profile" onClick={()=>switchTab("profile")} style={{width:"var(--player-mobile-avatar-size, 36px)",height:"var(--player-mobile-avatar-size, 36px)",borderRadius:"50%",background:"#1E1E1E",border:"1.5px solid #C8FF00",boxShadow:u.isCoach?"0 0 0 4px rgba(200, 255, 0, 0.15)":"none",color:"#FFFFFF",fontSize:"var(--player-mobile-avatar-font, 14px)",fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",padding:0,cursor:"pointer",fontFamily:FB,flexShrink:0}}>{(u.name||"?")[0].toUpperCase()}</button>
      <div style={{minWidth:0}}><div className="player-mobile-headerKicker" style={{fontFamily:FB,color:VOLT,fontSize:"var(--player-mobile-kicker-font, 10px)",letterSpacing:"var(--player-mobile-kicker-track, 0.15em)",fontWeight:700,textTransform:"uppercase",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{(()=>{
      ...
      })()}</div><div style={{display:"flex",alignItems:"center",gap:8,marginTop:1}}><div className="player-mobile-headerName" style={{fontFamily:FD,color:TOKENS.TEXT_PRIMARY,fontSize:"var(--player-mobile-name-font, 20px)",fontWeight:900,letterSpacing:"var(--player-mobile-name-track, 1.5px)",textTransform:"uppercase",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{u.name.toUpperCase()}</div>{u.isCoach&&<span style={{background:"rgba(200, 255, 0, 0.12)",border:"1px solid #C8FF00",borderRadius:20,padding:"2px 8px",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.10em",color:"#C8FF00",display:"inline-flex",alignItems:"center",gap:4}}><WhistleIcon size={9} color="#C8FF00"/>COACH</span>}</div>
      {!u.isCoach&&<div className="player-mobile-headerSubtitle" style={{fontFamily:FB,color:"var(--player-mobile-subtitle-color, var(--text-2))",fontSize:"var(--player-mobile-subtitle-font, 10px)",marginTop:1}}>Personal training dashboard · own your rep quality today.</div>}
      </div>
    </div>
    <div style={{display:"flex",gap:6,alignItems:"center"}}>
      <div className="player-mobile-headerLogo" style={{padding:"var(--player-mobile-logo-pad, 5px 8px)",borderRadius:12,border:"1px solid rgba(255,255,255,0.10)",background:"rgba(255,255,255,0.02)"}}><BrandWordmark size={20} small/></div>
      <button className="player-mobile-headerAction" aria-label="Toggle theme" onClick={()=>setTheme(t=>t==="dark"?"light":"dark")} style={{background:"var(--player-mobile-action-bg, var(--surface-2))",border:`1px solid ${T.BORDER}`,borderRadius:"var(--player-mobile-action-radius, 12px)",color:"var(--player-mobile-action-color, var(--text-2))",width:"var(--player-mobile-action-size, 44px)",height:"var(--player-mobile-action-size, 44px)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .3s"}}>
      ...
      <button className="player-mobile-headerAction" aria-label="Log out" onClick={logout} style={{background:"var(--player-mobile-action-bg, var(--surface-2))",border:`1px solid ${T.BORDER}`,borderRadius:"var(--player-mobile-action-radius, 12px)",color:"var(--player-mobile-action-color, var(--text-2))",width:"var(--player-mobile-action-size, 44px)",height:"var(--player-mobile-action-size, 44px)",cursor:"pointer",fontSize:14,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>&#10005;</button>
    </div>
  </div>

</div>

<div className={tab==="home"&&!active?"player-mobile-scroll player-home-compactScroll":"player-mobile-scroll"} ref={playerScrollRef} style={{flex:1,padding:isDesktop?"14px 20px 36px":"var(--player-mobile-scroll-pad, 16px 20px 124px)",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
...
return <nav className="bottom-nav" role="navigation" aria-label="Main navigation" style={{"--nav-accent":navAccent,position:"fixed",left:0,right:0,bottom:"var(--bottom-nav-offset, max(24px, calc(env(safe-area-inset-bottom, 0px) + 16px)))",display:"flex",justifyContent:"space-evenly",alignItems:"center",height:"calc(74px + ((var(--coach-text-scale-medium) - 1) * 12px))",padding:"8px 0 max(8px, env(safe-area-inset-bottom, 0px))",background:"var(--surface-1)",borderTop:"1px solid var(--stroke-1)",zIndex:60,pointerEvents:"auto",touchAction:"manipulation"}}>{items.map(t=>{const a=active===t.k;
