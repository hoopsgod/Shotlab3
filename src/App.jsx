...
const SUPPORT_EMAIL="support@REPLACE-WITH-SHOTLAB-DOMAIN.example";
const PRIVACY_EMAIL="privacy@REPLACE-WITH-SHOTLAB-DOMAIN.example";
const LEGAL_ROUTES={
  "/privacy":{
...
{["login","register"].map(m=><button key={m} onClick={()=>{setMode(m);setErr("")}} style={{flex:1,height:44,borderRadius:10,border:"none",cursor:"pointer",fontFamily:FD,fontSize:16,letterSpacing:3,textTransform:"uppercase",transition:"all .15s ease",background:mode===m?VOLT:"transparent",color:mode===m?"#000000":"#555555",fontWeight:mode===m?700:600}}>{m==="login"?"SIGN IN":"REGISTER"}</button>)}
    ...
    <nav aria-label="Legal and support pages" style={{display:"flex",flexWrap:"wrap",gap:"8px 10px",justifyContent:"center",marginTop:16,paddingTop:14,borderTop:`1px solid ${BORDER_CLR}`}}>
      {LEGAL_LINKS.map(link=><a key={link.href} href={link.href} style={{fontFamily:FB,color:TOKENS.TEXT_MUTED,fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",textDecoration:"none"}}>{link.label}</a>)}
    </nav>
  </div>
</div>
