...
import { buildConsumeInFlightKey, clearConsumeGuard, evaluateConsumeGuard, markConsumeGuardStarted } from "./lib/joinConsumeGuard.js";
...
useEffect(()=>{
const now=Date.now();
const guardStatus=evaluateConsumeGuard(consumeInFlightRef.current,"",now);
if(guardStatus.staleCleared){
setDataDebug(prev=>({...prev,join:{...prev.join,consumeGuardCleared:"yes",consumeAttemptBlocked:"no",consumeAttemptBlockedReason:"stale_lock_cleared_on_mount",consumeInFlightAgeMs:guardStatus.ageMs}}));
}
},[]);
...
const consumeInFlightKey=buildConsumeInFlightKey({
email:currentUserEmail,
inviteCode:resolvedContext?.inviteCode||"",
joinContextToken:resolvedContext?.joinContextToken||"",
teamId:resolvedContext?.teamId||"",
});
const now=Date.now();
const guardStatus=evaluateConsumeGuard(consumeInFlightRef.current,consumeInFlightKey,now);
if(guardStatus.blocked){
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeAttemptStarted:"yes",consumeAttemptBlocked:"yes",consumeAttemptBlockedReason:"active_request",consumeInFlightKey,consumeInFlightAgeMs:guardStatus.ageMs,consumeGuardCleared:"no",consumeHttpStatus:null,consumeDiagnosticCode:"join_in_progress",consumeDiagnosticMessage:"Join request already running locally.",membershipInsertStatus:"pending",membershipInsertError:"",finalRouteDecision:"pending_consume",error:""}}));
return{ok:false,retryable:true,err:"Join in progress.",diagnosticCode:"join_in_progress",diagnosticMessage:"Join request already running locally."};
}
if(guardStatus.staleCleared){
setDataDebug(prev=>({...prev,join:{...prev.join,consumeGuardCleared:"yes",consumeAttemptBlocked:"no",consumeAttemptBlockedReason:"stale_timeout",consumeInFlightAgeMs:guardStatus.ageMs}}));
}
if(!currentUserEmail){
const diagnostic=mapConsumeDiagnostic("consume_user_id_type_mismatch",400);
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeAttemptStarted:"yes",consumeAttemptBlocked:"no",consumeAttemptBlockedReason:"",consumeInFlightKey,consumeHttpStatus:400,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:diagnostic.message,consumeTokenPresent:"no",consumeTeamIdUsed:"",consumeUserEmail:"",consumeResolvedUserUuid:"",membershipInsertStatus:"failed",membershipInsertError:diagnostic.code,finalRouteDecision:"stay_join_team",inviteContextSaved:"no",inviteContextStorageKey:INVITE_CONTEXT_STORAGE_KEY,inviteContextTokenPresent:"no",inviteContextTeamId:"",inviteContextSubject:"",currentUserEmail:"",contextSubjectMatchesUser:"no",error:diagnostic.code}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeEndpoint,consumeAttemptStarted:"yes",consumeAttemptBlocked:"no",consumeAttemptBlockedReason:"",consumeInFlightKey,consumeHttpStatus:400,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:`${diagnostic.message} Checked ${INVITE_CONTEXT_STORAGE_KEY}.`,consumeTokenPresent:"no",consumeTeamIdUsed:"",consumeUserEmail:currentUserEmail,consumeResolvedUserUuid:"",membershipInsertStatus:"failed",membershipInsertError:diagnostic.code,finalRouteDecision:"stay_join_team",inviteContextSaved:"no",inviteContextStorageKey:INVITE_CONTEXT_STORAGE_KEY,inviteContextTokenPresent:"no",inviteContextTeamId:"",inviteContextSubject:"",currentUserEmail,contextSubjectMatchesUser:"no",error:`missing_invite_context:${INVITE_CONTEXT_STORAGE_KEY}`}}));
...
try{
markConsumeGuardStarted(consumeInFlightRef.current,consumeInFlightKey,Date.now());
consumeStarted=true;
setIsJoinConsumeActive(true);
setDataDebug(prev=>({...prev,join:{...prev.join,consumeFetchStarted:"yes",consumeFetchFinished:"no",consumeGuardCleared:"no",consumeInFlightKey}}));
...
setDataDebug(prev=>({...prev,join:{...prev.join,consumeFetchFinished:consumeStarted?"yes":"no"}}));
if(consumeStarted){
clearConsumeGuard(consumeInFlightRef.current,Date.now(),"attempt_finished");
setDataDebug(prev=>({...prev,join:{...prev.join,consumeGuardCleared:"yes",consumeInFlightAgeMs:0}}));
}
}
...
if(!joined.ok){
if(joined?.retryable)return{ok:false,retryable:true,err:joined.err||"Join in progress."};
