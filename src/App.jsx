...
const legacyAuthFetch=async(path,payload={})=>{const res=await fetch(path,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const body=await res.json().catch(()=>({}));return{ok:res.ok,status:res.status,body};};
...
const reg=await legacyAuthFetch("/v1/legacy-auth/register",{email:normalizedEmail,password,name,role});
if(!reg.ok){setDataDebug(prev=>({...prev,auth:{...prev.auth,legacyAuthRegisterStatus:"failed",signupHttpStatus:reg.status,signupCode:String(reg.body?.error||"register_failed"),signupMessage:"",providerHint:"legacy_backend"}}));return{ok:false,err:reg.body?.error==="account_exists"?"Account already exists. Please sign in.":"Could not register account."};}
const profile=normalizeLegacyProfile(reg.body?.profile||{});
setDataDebug(prev=>({...prev,auth:{...prev.auth,legacyAuthRegisterStatus:"success",signupHttpStatus:200,signupCode:"legacy_success",signupMessage:"",providerHint:"legacy_backend",sessionPresent:"yes",profilePersistStatus:"success",profileTeamId:profile.teamId||""}}));
...
setUser(profile);setView(profile.role==="coach"?"create-team":"join-team");DB.set("sl:session",{email:profile.email});trackEvent("auth_register",{targetRole:profile.role,userEmail:profile.email,userRole:profile.role},{email:profile.email,role:profile.role,teamId:profile.teamId||null});return{ok:true};
};
...
const remoteProfile=normalizeLegacyProfile(auth.body?.profile||{});
...
const logout=async()=>{trackEvent("auth_logout");if(SUPABASE_AUTH_ENABLED)await supabase.auth.signOut();setUser(null);setView("auth");DB.set("sl:session",null)};
...
await P("sl:players",np,setPlayers);
setUser({...user,teamId:nt.id});setView("coach");if(!SUPABASE_AUTH_ENABLED){await legacyAuthFetch("/v1/legacy-auth/update-profile",{email:user.email,team_id:nt.id});setDataDebug(prev=>({...prev,auth:{...prev.auth,legacyAuthUpdateStatus:"success",profileTeamId:nt.id}}));}
...
setUser({...user,teamId:resolvedTeamId});if(!SUPABASE_AUTH_ENABLED){await legacyAuthFetch("/v1/legacy-auth/update-profile",{email:user.email,team_id:resolvedTeamId});setDataDebug(prev=>({...prev,auth:{...prev.auth,legacyAuthUpdateStatus:"success",profileTeamId:resolvedTeamId}}));}navigateToPlayerHome();setView("player");
