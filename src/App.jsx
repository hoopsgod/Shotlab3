...
const coachHomeLeaderboardRows=useMemo(()=>buildAtHomeLeaderboardRows({scores:scopedHomeScores,shotLogs:scopedShotLogs,programDrills,players:appLeaderboardPlayers,limit:HOME_SHOTS_LEADERBOARD_LIMIT}),[scopedHomeScores,scopedShotLogs,programDrills,appLeaderboardPlayers]);
...
const playerDashboardHomeLeaderboardRows=useMemo(()=>buildAtHomeLeaderboardRows({scores,shotLogs,programDrills,players,limit:3}),[scores,shotLogs,programDrills,players]);
const playerDashboardLeaderboardRows=playerLeaderboardState.rows.length?playerLeaderboardState.rows:playerDashboardHomeLeaderboardRows;
...
<div ref={playerScrollRef} className="player-scroll-container" style={{flex:1,padding:isDesktop?"14px 20px 36px":"16px 20px var(--player-scroll-bottom-padding)",overflowY:"auto",overflowX:"hidden",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s",width:"100%",maxWidth:isDesktop?"none":760,margin:"0 auto"}} onTouchStart={isDesktop?undefined:onTS} onTouchMove={isDesktop?undefined:onTM} onTouchEnd={isDesktop?undefined:onTE}>
  ...
  {tab==="leaderboards"&&!active&&<div className={slideClass} key="leaderboards">
    <PremiumLeaderboardsHub viewerRole="player" leaderboardRows={homeShotsLeaderboard?.rows||[]} leaderboardStatus={homeShotsLeaderboard?.status||"idle"} userEmail={u?.email||""} programScores={teamProgramScores} programDrills={programDrills} players={players} teamId={u?.teamId||""} homeScores={homeScores} shotLogs={shotLogs} />
  </div>}
...
function CoachPlayerDevelopmentProfile({player,programDrills=[],programScores=[],scores=[],shotLogs=[],rsvps=[],events=[],scRsvps=[],scLogs=[],teamId=""}){
const profile=buildCoachPlayerDevelopmentProfile({player,programDrills,programScores,scores,shotLogs,rsvps,events,scRsvps,scLogs,teamId});
...
const ups=useMemo(()=>coachRosterPlayers.map(p=>({email:p.email||"",name:p.name||resolvePlayerDisplayName(p,playerProfiles),playerId:p.playerId||p.player_id||p.id||p.userId||p.user_id||p.profileId,profileId:p.profileId,source:p.source})),[coachRosterPlayers,playerProfiles]);
...
const safeProgramScores=useMemo(()=>getAllProgramScoreRows(programScores).filter(score=>!u?.teamId||score.teamId===u.teamId),[programScores,u?.teamId]);
