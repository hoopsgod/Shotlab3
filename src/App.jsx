  ...
  {tab==="home"&&!active&&<div className={slideClass} key="home">
    ...
    <div style={{display:"grid",gridTemplateColumns:isNarrow?"1fr":"repeat(2,minmax(0,1fr))",gap:10,alignItems:"start"}}>
      <section style={{background:CARD_BG,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:"12px 12px"}}>
        ...
        {upcomingEvents.length===0?<Empty variant="events" t="No sessions lined up yet" subtitle="Team workouts and training sessions show up here as soon as they are scheduled." ctaLabel="View all sessions" onTap={()=>setTab("program")} />:upcomingEvents.slice(0,2).map((ev,idx)=><div key={ev.id} style={{padding:idx===0?"0 0 8px":"8px 0 0",borderTop:idx===0?"none":`1px solid ${BORDER_CLR}`}}><div style={{fontFamily:FB,color:LIGHT,fontSize:12,fontWeight:700,lineHeight:1.25}}>{ev.title}</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10,marginTop:4}}>{ev.date} · {ev.time}</div></div>)}
...
{allRows.length===0&&<Empty variant="duels" t="No duels yet" subtitle="Challenge a teammate or wait for a head-to-head matchup to appear." ctaLabel="Browse duels" onTap={()=>setActiveFilter("open")} />} 
{allRows.length>0&&filteredRows.length===0&&<Empty variant="duels" t="No duels in this view" subtitle="Switch filters to find open challenges, pending replies, and finished battles." ctaLabel="Show all duels" onTap={()=>setActiveFilter("all")} />} 
...
{showBoard&&<div className="fade-up" style={{marginBottom:20}}>
  {board.length===0&&<Empty variant="lifting" t="No attendance logged yet" subtitle="Session check-ins will build this board once the team starts logging reps." ctaLabel="Log a session" onTap={handleAddScLog} />} 
...
{upcoming.length===0&&<Empty variant="lifting" t="No sessions lined up yet" subtitle="Strength sessions will show here once your coach posts the next block." ctaLabel="Check events" onTap={()=>setTab("events")} />} 
...
{upcoming.length===0&&<Empty variant="events" t="No events on the calendar yet" subtitle="When your coach posts team events, they will show up here." ctaLabel="Check back later" onTap={()=>setTab("home")} />} 
...
<div style={{flex:1,padding:`${showMiniHeader?"88px":"var(--space-4)"} var(--page-gutter) 110px`,overflowY:"auto",position:"relative",zIndex:1}}>
  ...
  {tab==="events"&&<div className="page pageShell fade-up accent-card" data-accent="events" id="coach-events-management" style={shellVars("events")}><PageHeader title="EVENTS" subtitle="Schedule team moments and track attendance" accent="amber" icon={<EventIcon type="event" size={22} color={PAGE_ACCENTS.events.accent}/>} actionLabel={showAdd?"Close":"Create"} onAction={()=>setShowAdd(!showAdd)} /><div className="heroModule"><div className="heroHead"><div><div className="heroKicker" style={{color:PAGE_ACCENTS.events.accent}}>NEXT EVENT</div><div className="heroSub">{nextEvent?`${nextEvent.title} · ${nextEvent.date} ${nextEvent.time}`:"No event scheduled"}</div></div><button className="pageHeaderPill" onClick={()=>setShowAdd(true)}>Create Event</button></div><div className="heroMetaStrip">{nextEvent?`Attendance check: ${rsvps.filter(r=>r.eventId===nextEvent.id).length} RSVPs locked in.`:"Add an event to set the week rhythm."}</div><div className="heroActionRow"><button className="pageHeaderPill" onClick={()=>document.getElementById("coach-events-management")?.scrollIntoView({behavior:"smooth"})}>Manage Events</button><button className="pageHeaderPill" onClick={()=>setShowAdd(true)}>Quick Create</button></div></div>
    ...
    {events.length===0&&<Empty variant="events" t="No events on the calendar yet" subtitle="Create the first team event to set the week's rhythm and get RSVPs rolling." ctaLabel="Create event" onTap={()=>setShowAdd(true)} align="left" />}
...
function Empty({t,action,description,onTap,cta,ctaLabel,ctaVariant="primary",secondaryCta,onSecondaryTap,secondaryCtaVariant="tertiary",icon=<DrillIcon type="sb" size={48} color="#555555"/>,variant,subtitle,align}){return <EmptyState variant={variant} title={t} description={description||subtitle||action} onCtaClick={onTap} ctaLabel={ctaLabel||cta} ctaVariant={ctaVariant} secondaryCta={secondaryCta} onSecondaryTap={onSecondaryTap} secondaryCtaVariant={secondaryCtaVariant} icon={icon} align={align}/>}
