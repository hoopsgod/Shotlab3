...
const body=await res.json().catch(()=>({}));
if(!res.ok){
const diagnostic=mapConsumeDiagnostic(body?.error,res.status);
setDataDebug(prev=>({...prev,join:{...prev.join,consumeHttpStatus:res.status,consumeDiagnosticCode:diagnostic.code,consumeDiagnosticMessage:diagnostic.message,membershipInsertStatus:"failed",finalRouteDecision:"stay_join_team",error:diagnostic.code}}));
return{ok:false,err:diagnostic.message,diagnosticCode:diagnostic.code,diagnosticMessage:diagnostic.message};
}
await savePendingJoinContext(null);
setDataDebug(prev=>({...prev,join:{...prev.join,consumeHttpStatus:res.status,consumeDiagnosticCode:"",consumeDiagnosticMessage:"",membershipInsertStatus:"success",finalRouteDecision:"apply_profile_update"}}));
return{ok:true,teamId:body.team_id||resolvedContext.teamId,status:body.status||"joined",diagnosticCode:"",diagnosticMessage:""};
}catch{
