...
return <button type="button" onClick={onClick} className="mode-card" style={{width:"100%",background:`radial-gradient(circle at 12% 10%, ${a.tint} 0%, transparent 55%), ${MODE_CARD_TOKENS.BASE_BG}`,border:baseBorder,borderRadius:24,padding:22,cursor:"pointer",textAlign:"left",position:"relative",minHeight:272,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:baseShadow,transition:"transform .12s ease, border-color .2s ease, box-shadow .2s ease"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=a.glow;e.currentTarget.style.boxShadow=`0 16px 34px rgba(0,0,0,.48), 0 0 0 1px ${a.glow} inset, 0 0 24px ${a.glow}`}} onMouseLeave={e=>{e.currentTarget.style.border=baseBorder;e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}} onMouseDown={e=>{e.currentTarget.style.transform="scale(0.99)";e.currentTarget.style.boxShadow=`0 0 0 2px ${a.glow}, 0 14px 28px rgba(0,0,0,.45)`}} onMouseUp={e=>{e.currentTarget.style.transform="scale(1)"}} onFocus={e=>{e.currentTarget.style.outline="none";e.currentTarget.style.boxShadow=`0 0 0 3px ${MODE_CARD_TOKENS.FOCUS_RING}, 0 14px 28px rgba(0,0,0,.45), 0 0 0 1px ${a.glow} inset`}} onBlur={e=>{e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}}>
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:16}}>
    ...
    <div className="icon-btn-square" style={{boxShadow:`0 0 10px ${a.glow}`}}><svg width="16" height="16" viewBox="0 0 16 16"><path d="M6 3l5 5-5 5" stroke={a.iconStroke} strokeWidth="2.2" fill="none" strokeLinecap="round"/></svg></div>
  </div>
...
return <div>
{/* Mode toggle */}
<div style={{display:"flex",gap:8,background:"#121212",borderRadius:14,padding:6,marginBottom:16,border:"1px solid var(--stroke-1)"}}>
{[{k:"home",l:"AT HOME"},{k:"prog",l:"PROGRAM"}].map(m=>{
const active=mode===m.k;
const thisMode=modeStyles[m.k];
return <button key={m.k} onClick={()=>switchMode(m.k)} style={{flex:1,padding:"10px 0",borderRadius:10,border:`1px solid ${active?"var(--accent)":"#353535"}`,cursor:"pointer",fontFamily:FB,fontSize:13,fontWeight:700,letterSpacing:2,transition:"all 180ms ease",background:active?"var(--surface-2)":"#171717",color:active?"var(--accent)":"var(--text-2)",boxShadow:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:8,textShadow:"none"}}><span aria-hidden="true" style={{fontSize:12,lineHeight:1,opacity:active?1:.65}}>{thisMode.label}</span>{m.l}</button>
})}
</div>
  ...
  <div style={{display:"flex",gap:8,minWidth:"max-content"}}>
    {isHome?
      [{k:"shots",l:"SHOT MAKES"}].map(t=>
        <button key={t.k} onClick={()=>setSub(t.k)} style={{height:32,padding:"0 14px",borderRadius:20,border:sub===t.k?"1px solid var(--accent)":"1px solid #333333",cursor:"pointer",fontFamily:FB,fontSize:11,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap",background:sub===t.k?"var(--surface-2)":"#1E1E1E",color:sub===t.k?"var(--accent)":"var(--text-2)",transition:"all .2s"}}>{t.l}</button>)
    :[{k:"events",l:"ATTENDANCE"},{k:"sc",l:"S&C"},{k:"prog-total",l:"DRILL SCORES"},...programDrills.map(d=>({k:`prog-${d.id}`,l:d.name}))].map(t=>
        <button key={t.k} onClick={()=>setSub(t.k)} style={{height:32,padding:"0 14px",borderRadius:20,border:sub===t.k?"1px solid var(--accent)":"1px solid #333333",cursor:"pointer",fontFamily:FB,fontSize:11,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap",background:sub===t.k?"var(--surface-2)":"#1E1E1E",color:sub===t.k?"var(--accent)":"var(--text-2)",transition:"all .2s",boxShadow:"none"}}>{t.l}</button>)}
  </div>
</div>
...
<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
  ...
  <div style={{fontFamily:FD,color:"var(--text-1)",fontSize:18,letterSpacing:3,flex:1}}>{title} LEADERBOARD</div>
...
{board.map((p,i)=>{
  ...
  if(isLeader) return <div key={p.email} className="podium-glow lbRow listRow" style={{"--pod-c":accentColor,background:"rgba(10, 12, 14, 0.94)",backgroundClip:"padding-box",border:`2px solid ${accentColor}33`,position:"relative",overflow:"hidden"}}>
    ...
    <div className="lbMain listRowText">
      <div className="lbName listRowTitle">{p.name.toUpperCase()}{isMe&&<span style={{fontFamily:FB,fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:4,background:"var(--surface-2)",border:"1px solid var(--stroke-1)",color:"var(--text-1)",marginLeft:6,letterSpacing:1}}>YOU</span>}</div>
  ...
  return <div key={p.email} className="lbRow listRow" style={{background:isMe?"rgba(10, 12, 14, 0.94)":rowBg,backgroundClip:"padding-box",border:isMe?`2px solid ${accentColor}44`:`1px solid ${BORDER_CLR}`,position:"relative",overflow:"hidden"}}>
    ...
    <div className="lbMain listRowText">
      <div className="lbName listRowTitle" style={{fontSize:13,fontWeight:isMe?700:600}}>{p.name.toUpperCase()}{isMe&&<span style={{fontFamily:FB,fontSize:8,fontWeight:700,padding:"1px 5px",borderRadius:4,background:"var(--surface-2)",border:"1px solid var(--stroke-1)",color:"var(--text-1)",marginLeft:6,letterSpacing:1,verticalAlign:"middle"}}>YOU</span>}</div>
...
<div>
<div style={{fontFamily:FD,color:"var(--text-1)",fontSize:16,letterSpacing:3}}>SHOT TRACKER</div>
...
<div style={{background:CARD_BG,borderRadius:18,padding:"20px 22px",border:`1px solid ${BORDER_CLR}`,marginBottom:16,position:"relative",overflow:"hidden"}}>
  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",position:"relative",gap:14}}>
    <div>
      <div style={{fontFamily:FB,color:"var(--text-2)",fontSize:13,letterSpacing:"0.10em",fontWeight:700,textTransform:"uppercase"}}>ATTENDANCE RANK</div>
      <div style={{display:"flex",alignItems:"center",gap:10,marginTop:6}}>
        <svg className={rankFx?"rank-badge-flash":""} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--text-2)" strokeWidth="2"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>
    ...
    :<div style={{marginTop:8}}>
      ...
      <div style={{position:"relative",paddingTop:8}}>
        <div style={{position:"relative"}}>
          ...
          <div style={{height:6,background:"var(--surface-1)",borderRadius:3,position:"relative",overflow:"hidden",border:"1px solid var(--stroke-1)"}}>
            <div
              style={{
                width:`${rankBarPct}%`,
                height:"100%",
...
<button onClick={()=>setShowBoard(!showBoard)} className="ch uiTap" style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",background:`linear-gradient(135deg,${CARD_BG},#141414)`,border:`1px solid ${BORDER_CLR}`,borderRadius:14,padding:"14px 18px",marginBottom:16,cursor:"pointer",textAlign:"left"}}>
  <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:10,background:"var(--surface-2)",border:"1px solid var(--stroke-1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>&#128293;</div><div><div style={{fontFamily:FD,color:LIGHT,fontSize:14,letterSpacing:2}}>LEADERBOARD</div><div style={{fontFamily:FB,color:T.SUB,fontSize:10,marginTop:1}}>Attendance, scores & streaks</div></div></div>
...
{upcoming.map(ev=>{const evR=rsvps.filter(r=>r.eventId===ev.id);const going=evR.some(r=>r.email===user.email);const exp=expanded===ev.id;
  return <div key={ev.id}>
    ...
    {exp&&<div className="fade-up" style={{background:SURFACE,borderRadius:"0 0 16px 16px",padding:"16px 20px",border:`1px solid ${BORDER_CLR}`,borderTop:"none"}}>
      ...
      <button className="btn btn-secondary btn--dense btn-v eventCta" onClick={()=>toggleRsvp(ev.id)} style={{marginBottom:14}}>
      ...
      {evR.length>0&&<div><div style={{fontFamily:FB,color:T.SUB,fontSize:10,letterSpacing:2,marginBottom:8,fontWeight:600}}>WHO'S GOING</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>
        {evR.map((r,i)=>{const tr=getTier(rsvps.filter(rr=>rr.email===r.email).length);return <div key={i} style={{display:"flex",alignItems:"center",gap:6,background:CARD_BG,borderRadius:8,padding:"6px 10px",border:`1px solid ${BORDER_CLR}`}}><Av n={r.name} sz={22} email={r.email}/><span style={{fontFamily:FB,color:LIGHT,fontSize:11,fontWeight:600}}>{r.name}</span>{tr.min>=2&&<span style={{fontFamily:FB,fontSize:7,fontWeight:700,letterSpacing:1,padding:"1px 4px",borderRadius:3,color:"var(--text-1)",background:"var(--surface-2)",border:"1px solid var(--stroke-1)"}}>{tr.name}</span>}</div>})}
      </div></div>}
    </div>}
  </div>})}
</div>
