...
const restoreTeamContext=useCallback(async(profile)=>{const email=normalizeEmail(profile?.email||"");const teamId=String(profile?.teamId||"").trim();if(!email||!teamId)return{ok:false,error:"invalid_request"};const res=await fetch("/v1/teams/restore-context",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,team_id:teamId})});const body=await res.json().catch(()=>({}));if(!res.ok||!body?.team)return{ok:false,error:String(body?.error||"team_context_restore_failed")};const rt=body.team;const restoredTeam={id:String(rt.id||teamId),name:san(rt.name||"Team"),joinCode:normalizeJoin(rt.joinCode||""),school:san(rt.school||""),level:san(rt.level||""),branding:rt.branding||DEFAULT_BRANDING};setTeams(prev=>{const exists=prev.some(t=>t.id===restoredTeam.id);return exists?prev.map(t=>t.id===restoredTeam.id?{...t,...restoredTeam}:t):[...prev,restoredTeam];});return{ok:true,team:restoredTeam};},[]);
...
if(p.teamId){const restored=await restoreTeamContext({email:normalizeEmail(p.email),teamId:p.teamId});if(!restored.ok)setStartupError("Team context could not be restored.");}
...
let t=teams.find(tm=>tm.id===teamId&&tm.ownerCoachId===user.email);
if(!t&&user?.teamId){const restored=await restoreTeamContext(user);if(restored.ok)t=restored.team;}
if(!t)return{ok:false,err:"Team context could not be restored."};
