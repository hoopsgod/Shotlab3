...
const PAGE_ACCENTS={
feed:{accent:"#B8FF00",glow:"rgba(184,255,0,0.35)",bg:"rgba(184,255,0,0.08)"},
drills:{accent:"#38E8FF",glow:"rgba(56,232,255,0.35)",bg:"rgba(56,232,255,0.09)"},
events:{accent:"#FFC400",glow:"rgba(255,196,0,0.34)",bg:"rgba(255,196,0,0.09)"},
sc:{accent:"#5B7CFF",glow:"rgba(91,124,255,0.35)",bg:"rgba(91,124,255,0.1)"},
players:{accent:"#B86CFF",glow:"rgba(184,108,255,0.35)",bg:"rgba(184,108,255,0.1)"},
};
...
HOME_TINT:"rgba(200, 255, 0, 0.15)",
PROGRAM_TINT:"rgba(255, 210, 77, 0.15)",
HOME_GLOW:"rgba(200, 255, 0, 0.24)",
PROGRAM_GLOW:"rgba(255, 210, 77, 0.24)",
...
.pageHeaderTop{display:flex;align-items:center;gap:12px;}
...
.pageHeaderText h1{font-family:${FD};font-size:26px;letter-spacing:.12em;color:#fff;line-height:1;}
.pageHeaderText p{font-family:${FB};font-size:11px;color:rgba(255,255,255,.65);margin-top:4px;text-transform:uppercase;letter-spacing:.06em;}
.pageHeaderRight{margin-left:auto;}
.pageHeaderPill{padding:6px 10px;border-radius:999px;border:1px solid color-mix(in srgb,var(--pageAccent) 50%, transparent);background:color-mix(in srgb,var(--pageAccent) 14%, #1A1A1A);font-family:${FB};font-size:10px;color:var(--pageAccent);font-weight:700;letter-spacing:.08em;text-transform:uppercase;transition:background .15s ease,box-shadow .15s ease,border-color .15s ease,transform .1s ease;}
...
.heroStat{background:#0f0f0f;border:1px solid #2a2a2a;border-radius:10px;padding:8px;text-align:center;}
.heroStatVal{font-family:${FD};color:var(--pageAccent);font-size:20px;line-height:1;}
.heroStatLbl{font-family:${FB};font-size:9px;color:#909090;letter-spacing:.08em;margin-top:2px;}
...
.drillsMetrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-bottom:10px;}
...
const[tab,setTab]=useState(initialTab),[selectedMode,setSelectedMode]=useState("home"),[active,setActive]=useState(null),[input,setInput]=useState(""),[saved,setSaved]=useState(false),[shareData,setShareData]=useState(null),[confetti,setConfetti]=useState(false);
...
const switchTab=(k)=>{setTab(k);if(k==="program")setSelectedMode("program");if(k==="log-drill")setSelectedMode("home");setActive(null);setShowShotStats(false);
...
<div style={{flex:1,padding:"14px 16px 124px",overflowY:"auto",position:"relative",zIndex:1,transform:`translateY(${pullY}px)`,transition:pullY?"none":"transform .3s"}} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
  ...
  {tab==="home"&&!active&&<div className={slideClass} key="home">
    ...
    {(()=>{
      ...
      const attendanceRows=rsvps.filter(r=>r.email===u.email&&(typeof r.verified!=="undefined"||typeof r.attended!=="undefined"));
      const attendanceVerified=attendanceRows.filter(r=>r.verified===true||r.attended===true).length;
      const attendancePct=attendanceRows.length>0?`${Math.round((attendanceVerified/attendanceRows.length)*100)}%`:"—";
      ...
      const programStats=[{label:"UPCOMING",value:upcomingEventsCount,color:VOLT},{label:"ATTENDANCE",value:attendancePct,color:TOKENS.PROGRAM_ACCENT},{label:"NEXT",value:nextEventLabel,color:LIGHT}];
      return <div style={{marginBottom:28}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isNarrow?"1fr":"repeat(2,minmax(0,1fr))",gap:isNarrow?18:16,alignItems:"stretch"}}>
          <ModeCard title="AT HOME" subtitle="Solo drills & shot tracking" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={VOLT} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9.5L12 3l9 6.5"/><path d="M19 13v6a1 1 0 01-1 1H6a1 1 0 01-1-1v-6"/></svg>} stats={homeStats} accent="home" isActive={selectedMode==="home"} onClick={()=>{setSelectedMode("home");setTab("log-drill");}}/>
          <ModeCard title="PROGRAM" subtitle="Team events & verified attendance" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={TOKENS.PROGRAM_ACCENT} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M8 2v4M16 2v4"/><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M3 10h18"/></svg>} stats={programStats} accent="program" isActive={selectedMode==="program"} onClick={()=>{setSelectedMode("program");setTab("program");}}/>
        </div>
      </div>
    })()}
...
return <div style={{background:"linear-gradient(180deg, rgba(255,255,255,0.04), rgba(10,10,10,0.45))",border:`1.5px solid ${BORDER_CLR}`,borderRadius:14,padding:"12px 10px",minHeight:98,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.02)"}}><div style={{fontFamily:FD,color:color||LIGHT,fontSize:24,lineHeight:1.05,wordBreak:"break-word"}}>{value}</div><div style={{fontFamily:FB,color:TOKENS.TEXT_SECONDARY,fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase"}}>{label}</div></div>
}

function ModeCard({title,subtitle,icon,stats,accent="home",isActive,onClick}){
const accentMap={
home:{accent:VOLT,tint:MODE_CARD_TOKENS.HOME_TINT,glow:MODE_CARD_TOKENS.HOME_GLOW},
program:{accent:TOKENS.PROGRAM_ACCENT,tint:MODE_CARD_TOKENS.PROGRAM_TINT,glow:MODE_CARD_TOKENS.PROGRAM_GLOW}
};
const a=accentMap[accent]||accentMap.home;
const cardVars={"--modeAccent":a.accent,"--modeAccentGlow":a.glow};
const baseBorder=isActive?`1.5px solid ${a.glow}`:`1.5px solid ${MODE_CARD_TOKENS.BASE_BORDER}`;
const baseShadow=isActive?`0 14px 30px rgba(0,0,0,.46), 0 0 0 1px ${a.glow} inset, 0 0 22px ${a.glow}`:MODE_CARD_TOKENS.BASE_SHADOW;
return <button type="button" onClick={onClick} className="mode-card" style={{...cardVars,width:"100%",background:`radial-gradient(circle at 12% 8%, ${isActive?a.glow:a.tint} 0%, transparent 50%), linear-gradient(145deg, color-mix(in srgb, var(--modeAccent) 7%, #121212) 0%, #101010 100%)`,border:baseBorder,borderRadius:24,padding:22,cursor:"pointer",textAlign:"left",position:"relative",minHeight:272,display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:baseShadow,transition:"transform .12s ease, border-color .2s ease, box-shadow .2s ease"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=a.glow;e.currentTarget.style.boxShadow=`0 16px 34px rgba(0,0,0,.48), 0 0 0 1px ${a.glow} inset, 0 0 26px ${a.glow}`}} onMouseLeave={e=>{e.currentTarget.style.border=baseBorder;e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}} onMouseDown={e=>{e.currentTarget.style.transform="scale(0.99)";e.currentTarget.style.boxShadow=`0 0 0 2px ${a.glow}, 0 10px 22px rgba(0,0,0,.45)`}} onMouseUp={e=>{e.currentTarget.style.transform="scale(1)"}} onFocus={e=>{e.currentTarget.style.outline="none";e.currentTarget.style.boxShadow=`0 0 0 3px ${MODE_CARD_TOKENS.FOCUS_RING}, 0 14px 28px rgba(0,0,0,.45), 0 0 0 1px ${a.glow} inset`}} onBlur={e=>{e.currentTarget.style.boxShadow=baseShadow;e.currentTarget.style.transform="scale(1)"}}>
  {isActive&&<span style={{position:"absolute",top:10,left:12,padding:"3px 9px",borderRadius:999,fontFamily:FB,fontSize:9,fontWeight:800,letterSpacing:"0.1em",background:`color-mix(in srgb, var(--modeAccent) 20%, #111)`,border:`1px solid var(--modeAccentGlow)`,color:a.accent,textTransform:"uppercase"}}>ACTIVE</span>}
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:16,marginTop:isActive?12:0}}>
    <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}>
      <div style={{width:50,height:50,borderRadius:14,background:MODE_CARD_TOKENS.ICON_INNER,border:`1.5px solid var(--modeAccentGlow)`,boxShadow:`inset 0 0 10px var(--modeAccentGlow)`,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--modeAccent)",flexShrink:0}}>{icon}</div>
    ...
    <div style={{width:38,height:38,borderRadius:10,background:MODE_CARD_TOKENS.CHEVRON_BG,border:`1.5px solid var(--modeAccentGlow)`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 0 10px var(--modeAccentGlow)`}}><svg width="16" height="16" viewBox="0 0 16 16"><path d="M6 3l5 5-5 5" stroke="var(--modeAccent)" strokeWidth="2.2" fill="none" strokeLinecap="round"/></svg></div>
  </div>
...
if(drills.length===0)return <div style={{marginBottom:14}}><SH isCoach={typeof u!=="undefined"&&u?.isCoach} t="PROGRAM DRILLS" s="COACH ADDED"/><Empty t="NO DRILLS YET" action="Your coach can add up to 7 custom shooting drills" cta="CONTACT YOUR COACH" icon={<DrillIcon type="sb" size={48} color="#555555"/>}/></div>;
...
<div style={{flex:1,padding:`${showMiniHeader?"88px":"16px"} 20px 110px`,overflowY:"auto",position:"relative",zIndex:1}}>
  ...
  {/** DRILLS */}
  {tab==="drills"&&!editD&&<div className="page pageShell fade-up" data-accent="drills" id="coach-drills-management" style={shellVars("drills")}><PageHeader title="DRILLS" subtitle="Build and launch shooting work" accent={PAGE_ACCENTS.drills.accent} icon={<DrillIcon type="ft" size={22} color={PAGE_ACCENTS.drills.accent}/>} rightSlot={<button className="pageHeaderPill" onClick={()=>setShowNewDrill(true)}>Add</button>} /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.drills.accent,fontSize:12,letterSpacing:2}}>QUICK START DRILL</div><div style={{fontFamily:FB,color:MUTED,fontSize:10}}>{drills.length} total drills ready to start</div></div><button className="pageHeaderPill pageHeaderPillBrand" onClick={()=>setShowNewDrill(true)}>Start</button></div><div className="drillsMetrics"><div className="heroStat drillsMetricTile"><div className="heroStatVal">{drills.length}</div><div className="heroStatLbl">ACTIVE</div></div><div className="heroStat drillsMetricTile"><div className="heroStatVal">{programDrills.length}</div><div className="heroStatLbl">PROGRAM</div></div></div><button className="pageHeaderPill" onClick={()=>document.getElementById("coach-drills-management")?.scrollIntoView({behavior:"smooth"})}>Manage Drills</button></div>
    <SH isCoach={typeof u!=="undefined"&&u?.isCoach} t="MANAGE DRILLS" s={`${drills.length} ACTIVE`} identity/>
  ...
  {/* EVENTS */}
  {tab==="events"&&<div className="page pageShell fade-up accent-card" data-accent="events" id="coach-events-management" style={shellVars("events")}><PageHeader title="EVENTS" subtitle="Plan attendance and upcoming sessions" accent={PAGE_ACCENTS.events.accent} icon={<EventIcon type="event" size={22} color={PAGE_ACCENTS.events.accent}/>} rightSlot={<button className="pageHeaderPill" onClick={()=>setShowAdd(!showAdd)}>{showAdd?"Close":"Create"}</button>} /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.events.accent,fontSize:12,letterSpacing:2}}>NEXT EVENT</div><div style={{fontFamily:FB,color:MUTED,fontSize:10}}>{nextEvent?`${nextEvent.title} · ${nextEvent.date} ${nextEvent.time}`:"No event scheduled"}</div></div><button className="pageHeaderPill" onClick={()=>setShowAdd(true)}>Create Event</button></div><div style={{marginTop:8}}><button className="pageHeaderPill" onClick={()=>document.getElementById("coach-events-management")?.scrollIntoView({behavior:"smooth"})}>Manage Events</button></div></div>
    <SH isCoach={typeof u!=="undefined"&&u?.isCoach} t="MANAGE EVENTS" s={`${events.length} TOTAL`} identity/>
    ...
    {[...events].sort((a,b)=>a.date.localeCompare(b.date)).map(ev=>{const evR=rsvps.filter(r=>r.eventId===ev.id);const isExp=expEv===ev.id;
      return <div key={ev.id} style={{marginBottom:10}}>
        <button onClick={()=>setExpEv(isExp?null:ev.id)} className="ch" style={{width:"100%",display:"flex",alignItems:"center",gap:12,background:CARD_BG,borderRadius:isExp?"14px 14px 0 0":14,padding:"14px 16px",border:`1px solid ${BORDER_CLR}`,cursor:"pointer",textAlign:"left"}}>
          <span className="eventsDatePill">{new Date(`${ev.date}T00:00:00`).toLocaleDateString(undefined,{month:"short",day:"numeric"})}</span><EventIcon type={ev.type} size={22} color={ev.date>=today?CYAN:MUTED}/><div style={{flex:1,minWidth:0}}><div style={{fontFamily:FD,color:LIGHT,fontSize:14,letterSpacing:2}}>{ev.title}</div><div style={{fontFamily:FB,color:MUTED,fontSize:10,marginTop:2}}>{ev.date} &#183; {ev.time} &#183; <span style={{color:VOLT}}>{evR.length} RSVP</span></div></div>
  ...
  {tab==="players"&&!selP&&<div className="page pageShell" data-accent="players" style={shellVars("players")}><PageHeader title="PLAYERS" subtitle="Roster health and individual progress" accent={PAGE_ACCENTS.players.accent} icon={<UsersIcon size={20} color={PAGE_ACCENTS.players.accent}/>} rightSlot={<button className="pageHeaderPill" onClick={()=>document.getElementById("coach-add-player-form")?.scrollIntoView({behavior:"smooth"})}>Add</button>} /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.players.accent,fontSize:12,letterSpacing:2}}>ROSTER SNAPSHOT</div><div style={{fontFamily:FB,color:MUTED,fontSize:10}}>{ups.length} players on roster</div></div><button className="pageHeaderPill" onClick={()=>document.getElementById("coach-add-player-form")?.scrollIntoView({behavior:"smooth"})}>Add Player</button></div></div>
  ...
  {/* ═════════════ S&C MANAGEMENT ═════════════ */}
  {tab==="sc"&&<div className="page pageShell fade-up" data-accent="sc" style={shellVars("sc")}><PageHeader title="S&C" subtitle="Strength sessions and readiness" accent={PAGE_ACCENTS.sc.accent} icon={<LiftIcon size={22} color={PAGE_ACCENTS.sc.accent}/>} rightSlot={<button className="pageHeaderPill" onClick={()=>setShowAddSC(!showAddSC)}>{showAddSC?"Close":"Add"}</button>} /><div className="heroModule"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><div><div style={{fontFamily:FD,color:PAGE_ACCENTS.sc.accent,fontSize:12,letterSpacing:2}}>TODAY'S LIFT</div><div style={{fontFamily:FB,color:MUTED,fontSize:10}}>{scSessions[0]?`${scSessions[0].sport||scSessions[0].title} · ${scSessions[0].date}`:"No lift scheduled"}</div></div><button className="pageHeaderPill" onClick={()=>setShowAddSC(true)}>Add Session</button></div><div className="heroStats"><div className="heroStat"><div className="heroStatVal">{scSessions.length}</div><div className="heroStatLbl">SESSIONS</div></div><div className="heroStat"><div className="heroStatVal">{scRsvps.length}</div><div className="heroStatLbl">RSVPS</div></div><div className="heroStat"><div className="heroStatVal">{scLogs.length}</div><div className="heroStatLbl">LOGS</div></div></div></div>
...
function SH({t,s,isCoach=false,identity=false}){return <div style={{marginBottom:16,display:"flex",alignItems:"baseline",justifyContent:"space-between"}}><div>{identity?<div className="page-header"><h1 className="page-title" style={{fontFamily:FD,color:LIGHT,fontSize:18,display:"inline-flex",alignItems:"center",gap:6,margin:0}}>{t}{isCoach&&<ShieldIcon size={12} color="#C8FF00" style={{opacity:.5,pointerEvents:"none"}}/>}</h1><div className="page-identity-bar" aria-hidden="true"></div></div>:<div style={{fontFamily:FD,color:LIGHT,fontSize:18,letterSpacing:4,display:"inline-flex",alignItems:"center",gap:6}}>{t}{isCoach&&<ShieldIcon size={12} color="#C8FF00" style={{opacity:.5,pointerEvents:"none"}}/>}</div>}</div>{s&&<div style={{fontFamily:FB,color:T.SUB,fontSize:11,letterSpacing:"0.06em",fontWeight:600,textTransform:"uppercase"}}>{s}</div>}</div>}
...
return <nav className="bottom-nav" role="navigation" aria-label="Main navigation" style={{"--nav-accent":navAccent,position:"fixed",left:0,right:0,bottom:0,display:"flex",justifyContent:"space-evenly",alignItems:"center",height:64,paddingBottom:"env(safe-area-inset-bottom)",background:"#1E1E1E",borderTop:"1px solid #242424",zIndex:20}}>{items.map(t=>{const a=active===t.k;
return <button key={t.k} aria-label={t.l} className={`tab ${a?"is-active active":""}`} onClick={()=>onChange(t.k)} style={{flex:1,minWidth:48,minHeight:48,height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:4,padding:"8px 4px 6px",position:"relative",background:"none",border:"none",cursor:"pointer",transition:"color 150ms ease-out",outlineOffset:2}}>
<div style={{position:"relative",lineHeight:1}}>{t.svg}</div>
<div style={{fontFamily:FB,fontSize:10,fontWeight:500,letterSpacing:"0.05em",textTransform:"uppercase",lineHeight:1.1,color:"currentColor",whiteSpace:"nowrap"}}>{t.l}</div>
</button>})}</nav>}
