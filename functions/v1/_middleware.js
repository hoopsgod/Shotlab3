...
async function findLegacyPlayerProfile(env, { requester, teamId }) {
  for (const teamColumn of LEGACY_PROFILE_TEAM_COLUMNS) {
    try {
      ...
      if (profile) return { ok: true, result: `match:v1/legacy_auth_profiles/${teamColumn}`, profile };
    } catch (error) {
      if (!shouldTreatProfileProbeErrorAsNoMatch(error)) return { ok: false, fatal: error, result: `error:v1/legacy_auth_profiles/${teamColumn}` };
    }
  }
  ...
  for (const teamColumn of LEGACY_PROFILE_TEAM_COLUMNS) {
    try {
      ...
      if (profile) return { ok: true, result: `match:v1/legacy_auth_profiles/${teamColumn}/fallback`, profile };
      teamIds.push(...profiles.map((row) => normalizeTeamId(row[teamColumn])).filter(Boolean));
    } catch (error) {
      if (!shouldTreatProfileProbeErrorAsNoMatch(error)) return { ok: false, fatal: error, result: `error:v1/legacy_auth_profiles/${teamColumn}/fallback` };
    }
  }
...
export async function onRequest(context) {
  ...
  if (request.method !== "POST" || !url.pathname.startsWith("/v1/home-shots/")) {
    return context.next();
  }
