...
async function readRowsByEmail(env, email) {
  const rows = await selectRows(env, "players", `select=${PLAYER_SELECT}&email=eq.${encodeURIComponent(email)}&limit=10`);
  return Array.isArray(rows) ? rows : [];
}

async function readTeamRows(env, teamId) {
  const rows = await selectRows(env, "players", `select=${PLAYER_SELECT}&team_id=eq.${encodeURIComponent(teamId)}&limit=1000`);
  return Array.isArray(rows) ? rows : [];
}
...
async function findCollision(env, row) {
  const byId = await selectRows(env, "players", `select=${PLAYER_SELECT}&id=eq.${encodeURIComponent(row.id)}&limit=1`);
