...
async function hasLegacyProfileAccess(env, email, teamId) {
  const rows = await selectRows(env, 'legacy_auth_profiles', `select=email,team_id&email=eq.${encodeURIComponent(email)}&team_id=eq.${encodeURIComponent(teamId)}&limit=1`).catch(() => []);
...
export async function onRequestPost({ request, env }) {
  ...
  const authorized = (await hasLegacyProfileAccess(env, email, teamId)) || (await hasMembershipAccess(env, email, teamId));
