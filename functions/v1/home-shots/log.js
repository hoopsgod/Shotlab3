...
async function verifyRepairIsAllowed(env, { requester, teamId }, diagnostic) {
  ...
  try {
    const rows = await selectRows(env, "legacy_auth_profiles", `select=email,name,role,team_id&email=eq.${encodeURIComponent(requester)}&limit=1`);
    const profile = Array.isArray(rows) ? rows[0] : null;
    if (!profile) {
      diagnostic.team_binding_repair_account_probe = "0:self_identity_verified";
      return { ok: true, profile: null, reason: "self_identity_verified" };
    }
    const role = normalizeIdentity(profile.role);
    const profileTeamId = String(profile.team_id || profile.teamId || "").trim();
    if (role && role !== "player") {
      diagnostic.team_binding_repair_account_probe = `role:${role}`;
      return { ok: false, reason: "registered_profile_not_player" };
    }
    if (profileTeamId !== teamId) {
      diagnostic.team_binding_repair_account_probe = `team_mismatch:${safeDiagnosticText(profileTeamId, 80)}`;
      return { ok: false, reason: "registered_profile_team_mismatch" };
    }
    diagnostic.team_binding_repair_account_probe = "match:legacy_auth_profiles";
    return { ok: true, profile };
  } catch (error) {
    diagnostic.team_binding_repair_account_probe = `error:${safeErrorMessage(error)}`;
    if (!shouldTreatLookupErrorAsNoMatch(error)) return { ok: false, fatal: error, reason: "registered_profile_probe_failed" };
    return { ok: true, profile: null, reason: "self_identity_verified_profile_probe_unavailable" };
  }
}
...
async function resolveOrRepairHomeShotPlayerBinding(env, { requester, resolvedUserUuid, teamId, name }, diagnostic) {
  ...
  if (!playerRepair.ok || !membershipRepair.ok) return { ok: false, repairFailed: true, repairReason: "repair_upsert_failed", fatal: playerRepair.fatal || membershipRepair.fatal || null };
  ...
  if (repaired.ok) {
    diagnostic.team_binding_repair_result = "repaired";
    return repaired;
  }
  return { ok: false, repairFailed: true, repairReason: "repair_reread_no_binding", fatal: repaired.fatal || null };
}
