...
export async function confirmInviteContext({ callRpc, userId, subjectKey, joinContextToken, clientRequestId = null, attemptedTeamId = "", attemptedRole = "player" }) {
  ...
  try {
    ...
    if (!resolvedUserUuid) {
      return {
        ...
        diagnostic: {
          ...
          resolved_uuid: "",
          attempted_team_id: String(attemptedTeamId || ""),
          attempted_resolved_user_uuid: "",
          attempted_role: String(attemptedRole || "player"),
        },
      };
    }
    ...
    const resolvedCode = mapped.code === "internal_error" ? "consume_membership_insert_failed" : mapped.code;
    ...
    return {
      ...
      diagnostic: {
        ...
        db_detail: String(details?.details || ""),
        db_hint: String(details?.hint || ""),
        table: String(details?.table || ""),
        constraint: String(details?.constraint || ""),
        team_id_type: String(details?.hint || details?.details || ""),
        user_id_value_type: typeof normalizedUserId,
        resolved_uuid: resolvedUserUuid || "",
        attempted_team_id: String(attemptedTeamId || ""),
        attempted_resolved_user_uuid: resolvedUserUuid || "",
        attempted_role: String(attemptedRole || "player"),
      },
    };
  }
