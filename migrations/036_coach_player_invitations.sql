...
create table if not exists public.coach_player_invitations (
  id uuid primary key default gen_random_uuid(),
  team_id text not null references public.teams(id) on update restrict on delete cascade,
  player_profile_id text references public.player_profiles(id) on update restrict on delete set null,
...
create index if not exists coach_player_invitations_team_created_idx
  on public.coach_player_invitations(team_id, created_at desc);

create index if not exists coach_player_invitations_email_idx
  on public.coach_player_invitations(lower(player_email));

create unique index if not exists coach_player_invitations_one_open_per_team_email
  on public.coach_player_invitations(team_id, lower(player_email))
  where status in ('pending','sent');

alter table public.coach_player_invitations enable row level security;
revoke all on table public.coach_player_invitations from public, anon, authenticated;
