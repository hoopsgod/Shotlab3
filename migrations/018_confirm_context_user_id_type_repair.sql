...
declare
  ...
  v_redemption_team_id_type text;
  v_user_uuid uuid;
begin
  ...
  limit 1;
  select data_type into v_redemption_team_id_type
  from information_schema.columns
  where table_schema='public' and table_name='team_invite_redemptions' and column_name='team_id'
  limit 1;
  ...
  if coalesce(v_team_id_type, 'uuid') = 'uuid' and v_user_id_type = 'uuid' then
    ...
    execute 'select id from public.team_memberships where team_id = $1::uuid and user_id = $2::text for update'
      into v_membership_id
      using (v_session.team_id::text)::uuid, trim(p_user_id);
  ...
  if coalesce(v_redemption_team_id_type, 'uuid') = 'uuid' and coalesce(v_redemption_user_id_type, 'uuid') = 'uuid' then
    execute 'insert into public.team_invite_redemptions (invite_id, team_id, user_id, membership_id, client_request_id)
      ...
      using v_session.invite_id, (v_session.team_id::text)::uuid, v_user_uuid, v_membership_id, p_client_request_id;
  elsif coalesce(v_redemption_team_id_type, 'uuid') = 'uuid' then
    execute 'insert into public.team_invite_redemptions (invite_id, team_id, user_id, membership_id, client_request_id)
             values ($1, $2::uuid, $3::text, $4, $5)
             on conflict (client_request_id) do nothing'
      using v_session.invite_id, (v_session.team_id::text)::uuid, trim(p_user_id), v_membership_id, p_client_request_id;
  elsif coalesce(v_redemption_user_id_type, 'uuid') = 'uuid' then
    execute 'insert into public.team_invite_redemptions (invite_id, team_id, user_id, membership_id, client_request_id)
