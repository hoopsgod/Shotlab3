...
begin
  ...
  begin
    select public.resolve_app_user_uuid(v_requester_user_id)::text into v_requester_uuid;
  exception
    when others then
  ...
  if not exists (
    ...
    where coalesce(nullif(to_jsonb(tm)->>'team_id', ''), nullif(to_jsonb(tm)->>'teamId', '')) = v_team_id
      ...
      and (
        ...
        or (
          v_requester_uuid is not null
          and coalesce(nullif(to_jsonb(tm)->>'user_id', ''), nullif(to_jsonb(tm)->>'userId', '')) = v_requester_uuid
        )
      )
  ) then
