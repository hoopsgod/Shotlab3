-- Prefer submitted shot-log names for home-shots leaderboard fallback when profiles are missing.
...
begin
  ...
  latest_shot_names as (
    select distinct on (sl.team_id, sl.player_id)
      sl.team_id,
      sl.player_id,
      nullif(trim(coalesce(sl.name, '')), '') as shot_log_name
    from shot_logs sl
    where sl.team_id = v_team_id
      and nullif(trim(coalesce(sl.player_id, '')), '') is not null
  ...
  enriched as (
    select
      ...
      latest_shot_names.shot_log_name,
