...
--
-- 1) Verify required RPCs exist:
-- select n.nspname as schema_name, p.proname as function_name, pg_get_function_identity_arguments(p.oid) as args
-- from pg_proc p
-- join pg_namespace n on n.oid = p.pronamespace
-- where n.nspname = 'public'
--   and (
--     (p.proname = 'coach_signup_create_team_and_invite' and pg_get_function_identity_arguments(p.oid) = 'text, text, integer, integer') or
--     (p.proname = 'lookup_team_invite_by_code' and pg_get_function_identity_arguments(p.oid) = 'text') or
--     (p.proname = 'resolve_team_invite_context' and pg_get_function_identity_arguments(p.oid) = 'text, text, integer') or
--     (p.proname = 'confirm_team_invite_join_from_context' and pg_get_function_identity_arguments(p.oid) = 'text, text, text, text')
--   )
-- order by function_name;
--
-- 2) Verify team_id column types across invite flow tables:
