-- Return a stable player identity with each authorized aggregate and only rank
-- members who are still active in the requested team. The Cloudflare route is
-- the sole caller of this SECURITY DEFINER function.

drop function if exists public.get_team_home_shots_leaderboard(text, text, integer, text);

create function public.get_team_home_shots_leaderboard(
  p_team_id text,
...
begin
  ...
  return query
  with totals as (
    select t.team_id, t.player_id, t.total_home_shots
  ...
  enriched as (
    select
      ...
      lower(coalesce(
        nullif(profiles.rec->>'rosterStatus', ''),
        nullif(profiles.rec->>'roster_status', ''),
        nullif(profiles.rec->>'status', ''),
        'active'
      )) as roster_status,
      (
        lower(coalesce(profiles.rec->>'hideFromLeaderboards', profiles.rec->>'hide_from_leaderboards', 'false')) = 'true'
        or lower(coalesce(profiles.rec->>'archived', 'false')) = 'true'
        or lower(coalesce(profiles.rec->>'deleted', 'false')) = 'true'
        or lower(coalesce(profiles.rec->>'removed', 'false')) = 'true'
        or lower(coalesce(profiles.rec->>'hidden', 'false')) = 'true'
        or lower(coalesce(profiles.rec->>'teamLocalDataDeleted', profiles.rec->>'team_local_data_deleted', 'false')) = 'true'
      ) as is_inactive,
      coalesce(
        nullif(trim(coalesce(profiles.rec->>'name', '')), ''),
        latest_shot_names.submitted_name,
        nullif(trim(coalesce(profiles.rec->>'email', '')), ''),
        case when strpos(totals.player_id, '@') > 1 then split_part(totals.player_id, '@', 1) end,
        'Player'
    ...
    join profiles
      ...
      and totals.player_id in (
        ...
        coalesce(nullif(profiles.rec->>'userId', ''), '__no_match__')
      )
  ...
  filtered as (
    ...
    where e.is_inactive = false
      and e.roster_status not in ('archived', 'removed', 'team_local_data_deleted', 'deleted', 'hidden')
      and (
        v_scope = 'all'
        or (v_scope = 'players' and e.profile_role <> 'coach')
        or (v_scope = 'coaches' and e.profile_role = 'coach')
      )
  ),
  ranked as (
    select
  ...
  select ranked.rank, ranked.player_id, ranked.player_display_name, ranked.total_home_shots
  from ranked
