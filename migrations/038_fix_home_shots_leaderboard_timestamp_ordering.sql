-- Fix coach/player Home Shots leaderboard RPC failures caused by mixing
-- timestamptz and bigint values in the latest-shot-name ORDER BY expression.
...
begin
  ...
  if v_requester_user_id = '' then raise exception 'REQUESTER_REQUIRED'; end if;
  if v_scope not in ('players', 'coaches', 'all') then raise exception 'SCOPE_INVALID'; end if;
  ...
  with totals as (
    ...
    where t.team_id = v_team_id and t.total_home_shots > 0
  ),
  profiles as (
    select to_jsonb(p) as rec from players p
  ),
  latest_shot_names as (
    ...
    order by
      ...
      coalesce(
        sl.ts,
        case
          when coalesce(sl.date, '') ~ '^\d{4}-\d{2}-\d{2}$' then sl.date::date::timestamptz
          else null
        end,
        sl.created_at,
        to_timestamp(0)
  ...
  enriched as (
    select
      ...
      coalesce(
        ...
        case when strpos(totals.player_id, '@') > 1 then split_part(totals.player_id, '@', 1) else null end,
        'Player'
  ...
  ranked as (
    select
      row_number() over (order by filtered.total_home_shots desc, filtered.player_display_name asc, filtered.player_id asc)::integer as rank,
