...
const authenticatedAuthority = read("../src/styles/AuthenticatedVisualAuthority2026.css");
...
test("the visual foundation loads after the application module", () => {
  ...
  const authorityImportIndex = main.indexOf("await import('./styles/AuthenticatedVisualAuthority2026.css')");
  const foundationImportIndex = authenticatedAuthority.indexOf("./VisualFoundation2026.css");
  assert.ok(appImportIndex >= 0);
  assert.ok(authorityImportIndex > appImportIndex);
  assert.ok(foundationImportIndex >= 0, "authenticated visual authority must include the canonical foundation");
});
