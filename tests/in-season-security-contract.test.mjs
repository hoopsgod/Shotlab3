...
test("route enhancer adds In Season to both roles and preserves a bounded custom drill capacity", () => {
  const enhancer = read("scripts/apply-in-season-performance-hub.mjs");
  const runner = read("scripts/run-route-enhancers.mjs");
  assert.match(enhancer, /in-season-performance-hub-route/);
  assert.match(enhancer, /coach-in-season-performance-hub-route/);
  assert.match(enhancer, /updateProgramDrill/);
  assert.match(enhancer, /countCustomProgramDrills\(programDrills\)>=30/);
  assert.match(runner, /apply-in-season-performance-hub\.mjs/);
});
