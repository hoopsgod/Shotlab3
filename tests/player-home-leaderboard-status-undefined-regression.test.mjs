...
test('player home leaderboard render uses safe derived values and avoids undefined legacy vars', () => {
  ...
  assert.equal(appSource.includes('const playerDashboardHomeLeaderboardRows'), false, 'the dashboard must not fabricate a local fallback for registered rankings');
  assert.equal(appSource.includes('const playerDashboardLeaderboardRows=playerLeaderboardRows;'), true);
  ...
  assert.equal(appSource.includes('playerLeaderboardPlayers=useMemo(()=>playerActiveTeamIdentity.players'), true, 'player leaderboard should use active roster identities');
