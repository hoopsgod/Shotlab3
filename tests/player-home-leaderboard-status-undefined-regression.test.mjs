...
test('player home leaderboard render uses safe derived values and avoids undefined legacy vars', () => {
  ...
  assert.equal(appSource.includes('const playerDashboardHomeLeaderboardRows=useMemo(()=>buildAtHomeLeaderboardRows({scores:teamFacingScores,shotLogs:teamFacingShotLogs,programDrills,players:playerLeaderboardPlayers,limit:3})'), true);
  ...
  assert.equal(appSource.includes('const playerLeaderboardPlayers=useMemo(()=>playerActiveTeamIdentity.players'), true, 'player leaderboard should use active roster identities');
