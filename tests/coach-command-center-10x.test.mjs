...
test("mobile Coach Home remains brand-first, Pulse-first and touch-safe",()=>{
  ...
  assert.match(titleCss,/Phase 6E mobile Coach Home composition authority: one source-owned block/);
  assert.match(mobileTitle,/\.mcHeader\[data-testid="mission-control-team-header"\]\{display:none\}/);
  assert.match(mobileTitle,/\.mcHero\[data-team-identity-stage="coach-mission-control"\]\{[^}]*min-height:334px[^}]*margin-inline:0/);
  ...
  assert.match(mobileTitle,/\.mcRealityStrip button\{[^}]*min-height:54px[^}]*padding:8px 12px/);
  assert.match(mobileTitle,/\.mcPrimary\{[^}]*min-height:46px[^}]*margin-top:11px/);
