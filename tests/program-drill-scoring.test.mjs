...
test('program drills support repeated optional attempts and leaderboards rank by best score', () => {
  ...
  const repeatedAttempts = [
    { id: 'attempt-1', src: 'program', drillId: drillA.id, email: 'active@team.test', player_id: 'player:team_active', name: 'Active Player', score: 20, date: '2026-06-22', ts: 20, teamId: 'team-a' },
    { id: 'attempt-2', src: 'program', drillId: drillA.id, email: 'active@team.test', player_id: 'player:team_active', name: 'Active Player', score: 25, date: '2026-06-22', ts: 25, teamId: 'team-a' },
  ];
...
test('leaderboards hub Program Drills path uses program score selectors while At Home stays on home leaderboard rows', () => {
  ...
  assert.match(compactCardSource, /const scoreValue = entry\.total_home_shots \?\? entry\.score \?\? entry\.total/);
});
