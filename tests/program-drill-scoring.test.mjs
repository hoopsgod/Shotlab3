...
test('leaderboards hub Program Drills path uses program score selectors while At Home stays on home leaderboard rows', () => {
  ...
  assert.match(appSource, /const coachRosterPlayers=useMemo\(\(\)=>players\.filter\(p=>isActiveRosterPlayer\(p,u\?\.teamId\)\)\.map\(p=>\(\{\.\.\.p,name:resolvePlayerDisplayName\(p,playerProfiles\)\}\)\)/);
  ...
  assert.match(hubSource, /buildAtHomeLeaderboardRows\(\{ scores: homeScores, shotLogs, programDrills, limit: 10 \}\)/);
