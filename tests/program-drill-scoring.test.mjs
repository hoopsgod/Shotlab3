...
import { buildProgramDrillLeaderboardRows, buildProgramScoreRow, getAllProgramScoreRows, getProgramDrillBreakdownRows, getProgramLeaderboardRows, getProgramScoresForPlayer, validateProgramDrillScore } from '../src/lib/programDrillScoring.js';
...
test('program score selectors normalize drill_id and player_email aliases', () => {
  ...
  assert.deepEqual(getProgramLeaderboardRows(aliasRows, drill, players).map((row) => [row.rank, row.player_id, row.total]), [[1, 'player:team_active', 25]]);
...
test('program leaderboard and breakdown tolerate missing roster match and drill_name fallback', () => {
  ...
  assert.deepEqual(getProgramLeaderboardRows(savedRows, drill, []).map((row) => [row.rank, row.email, row.total]), [[1, 'active@team.test', 25]]);
...
test('leaderboards hub Program Drills path uses program score selectors while At Home stays on home leaderboard rows', () => {
  ...
  assert.match(hubSource, /const normalizedProgramScores = useMemo\(\(\) => getAllProgramScoreRows\(programScores\)/);
  assert.match(hubSource, /getProgramLeaderboardRows\(normalizedProgramScores, selectedProgramDrill, players, 10\)/);
  assert.match(hubSource, /activeLeaderboardCategory === 'home_shots'[\s\S]*rows=\{leaderboardRows\}/);
