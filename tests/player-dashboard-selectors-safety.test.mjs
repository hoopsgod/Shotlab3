...
test('player dashboard selectors safely handle empty/new player state', () => {
  ...
  assert.equal(deriveMomentumLabel({ weeklyMakes: 0, weeklyGoal: 300 }), 'Building base volume');
  assert.equal(deriveNextFocusLabel({ todaysMakes: 0, dailyGoal: 75 }), 'Close daily make target + confirm attendance');
  ...
  assert.equal(briefing[0].title, 'Consistency chain');
  ...
  assert.equal(identity.commitmentLevel, 'Building base volume');
});
