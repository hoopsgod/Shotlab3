  ...
  try {
    const res = await onRequestPost(
      makeContext({
        ...
        headers: { "x-user-id": "coach@shotlab.app", "x-internal-api-token": "token" },
        env: { ...ENV, INTERNAL_API_TOKEN: "token" },
      }),
    );
...
test("coach signup bootstrap requires internal api token when configured", async () => {
  const res = await onRequestPost(
    makeContext({
      body: { team_name: "Titans" },
...
test("coach signup bootstrap maps missing rpc to missing_rpc diagnostic", async () => {
  ...
  try {
    ...
    assert.equal(res.status, 500);
    assert.deepEqual(await res.json(), {
      error: "missing_function_public_coach_signup_create_team_and_invite",
      diagnostic_code: "missing_function_public_coach_signup_create_team_and_invite",
      diagnostic_message: "Required function public_coach_signup_create_team_and_invite is not available in Supabase.",
    });
  } finally {
    global.fetch = originalFetch;
  }
...
test("coach signup bootstrap maps specific missing table name", async () => {
  const originalFetch = global.fetch;
  global.fetch = async () =>
    new Response(
      JSON.stringify({
        code: "42P01",
        message: "relation \"team_invites\" does not exist",
      }),
      { status: 400, headers: { "Content-Type": "application/json" } },
    );
  ...
  try {
    ...
    assert.equal(res.status, 500);
    assert.deepEqual(await res.json(), {
      error: "table_missing_team_invites",
      diagnostic_code: "table_missing_team_invites",
      diagnostic_message: "Required table team_invites is missing.",
    });
  } finally {
    global.fetch = originalFetch;
  }
...
test("coach signup bootstrap maps schema type mismatch for teams id", async () => {
  const originalFetch = global.fetch;
  global.fetch = async () =>
    new Response(
      JSON.stringify({
        code: "42804",
        message: "foreign key constraint cannot be implemented",
        details: "Key columns \"team_id\" and \"id\" are of incompatible types: uuid and text.",
      }),
      { status: 400, headers: { "Content-Type": "application/json" } },
    );
  ...
  try {
    ...
    assert.deepEqual(await res.json(), {
      ...
      diagnostic_code: "schema_type_mismatch_teams_id",
