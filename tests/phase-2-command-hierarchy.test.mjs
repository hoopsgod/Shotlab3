...
const main=read("../src/main.jsx"),player=read("../src/components/PlayerDailyCommandCenter.jsx"),playerCss=read("../src/components/PlayerDailyCommandCenter.module.css"),hierarchy=read("../src/styles/CommandHierarchy2026.css"),rail=read("../src/components/OperationalInsightRail.jsx"),railCss=read("../src/components/OperationalInsightRail.module.css"),railModel=read("../src/lib/operationalInsightRails.js"),coach=read("../src/components/CoachCommandCenter.jsx"),session=read("../public/shotlab-v15-session-integrity.css");
...
test("Phase 2 control and insight hierarchy survives Phase 4 identity",()=>{
 ...
 assert.match(playerCss,/\.primaryButton[\s\S]*-webkit-text-fill-color: currentColor;/);
 assert.match(playerCss,/\.primaryButton > \*[\s\S]*color: inherit;[\s\S]*-webkit-text-fill-color: currentColor;/);
...
test("Phase 2 copy and coach-primary contracts remain intact",()=>{
 assert.ok(player.includes('Daily work banked.'));
