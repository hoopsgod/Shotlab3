...
test('player and coach compact home-shots cards show Aahna instead of empty while RPC status is still idle', () => {
...
test('leaderboard cards render fallback rows even before RPC status is success', () => {
  const HomeShotsLeaderboardCardSource = fs.readFileSync(new URL('../src/components/HomeShotsLeaderboardCard.jsx', import.meta.url), 'utf8')
  const compactSource = fs.readFileSync(compactCardFileUrl, 'utf8')
  assert.match(HomeShotsLeaderboardCardSource, /rows\.length > 0\s*\? renderRows\(\)/)
  assert.match(compactSource, /const hasRows = previewRows\.length > 0/)
})
