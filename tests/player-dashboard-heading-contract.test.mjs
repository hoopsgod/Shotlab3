...
test('player quick actions render Profile and Logout only', () => {
  ...
  assert.match(playerQuickActions, /aria-label="Profile"[\s\S]*>Profile<\/button>/);
  assert.match(playerQuickActions, /onClick=\{\(\)=>switchTab\("profile"\)\}/);
  assert.match(playerQuickActions, /aria-label="Logout"[\s\S]*>Logout<\/button>/);
...
test('player quick actions do not render Theme or wire setTheme', () => {
  assert.doesNotMatch(playerQuickActions, /Theme|Toggle theme|setTheme|theme=\{theme\}|sun|moon/i);
...
test('player dashboard header keeps the 1135 coach-style hero structure with large brand mark', () => {
  ...
  assert.match(playerHeaderSource, /branding\?\.logoUrl \|\| branding\?\.logoMarkUrl \|\| "\/branding\/titans-exact-logo\.png\.PNG"/);
  assert.match(playerHeaderSource, /<img className=\{styles\.brandMark\} data-player-brand-mark="team-logo"/);
});
...
test('player quick actions live outside the PlayerDashboardHeader hero', () => {
  assert.ok(playerHeaderEnd > playerHeaderStart, 'PlayerDashboardHeader invocation should be present');
  assert.ok(quickActionsStart > playerHeaderEnd, 'Player quick actions should render after the hero closes');
});
