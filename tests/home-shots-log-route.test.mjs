...
test("shot_logs player_id uuid insert error does not retry without client id", async () => {
  const originalFetch = global.fetch;
  ...
  global.fetch = async (url, init = {}) => {
    ...
    if (href.includes("/shot_logs")) {
      ...
      return new Response(JSON.stringify({ code: "22P02", message: "invalid input syntax for type uuid", details: "player_id rejected p@x.com" }), { status: 400 });
    }
    return new Response(JSON.stringify([]), { status: 200 });
  };
  try {
