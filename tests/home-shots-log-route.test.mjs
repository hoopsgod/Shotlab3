...
test("missing durable relationship is repaired, then shot_logs insert succeeds", async () => {
  ...
  global.fetch = async (url, init = {}) => {
    ...
    if (href.includes("/players") && method === "GET") {
      return new Response(JSON.stringify(playersUpserted ? [{ id: "player-repaired", email: "p@x.com", team_id: "team-a", role: "player", status: "active" }] : []), { status: 200 });
    }
    if (href.includes("/shot_logs")) {
      shotInserted = true;
...
test("registered self-identity repair works when legacy profile row is absent", async () => {
  const originalFetch = global.fetch;
  let playersUpserted = false;
  let membershipUpserted = false;
  global.fetch = async (url, init = {}) => {
    const href = String(url);
    const method = String(init.method || "GET");
    if (href.includes("/rpc/resolve_app_user_uuid")) return new Response(JSON.stringify("uuid-self-repair"), { status: 200 });
    if (href.includes("/legacy_auth_profiles")) return new Response(JSON.stringify([]), { status: 200 });
    if (href.includes("/players") && method === "POST") { playersUpserted = true; return new Response(JSON.stringify([JSON.parse(init.body)[0]]), { status: 201 }); }
    if (href.includes("/team_memberships") && method === "POST") { membershipUpserted = true; return new Response(JSON.stringify([JSON.parse(init.body)[0]]), { status: 201 }); }
    if (href.includes("/team_memberships") && method === "GET") return new Response(JSON.stringify(membershipUpserted ? [{ id: "m-self", user_id: "uuid-self-repair", team_id: "team-a", role: "player", status: "active" }] : []), { status: 200 });
    if (href.includes("/players") && method === "GET") return new Response(JSON.stringify(playersUpserted ? [{ id: "p-self", email: "p@x.com", team_id: "team-a", role: "player", status: "active" }] : []), { status: 200 });
    if (href.includes("/shot_logs")) return new Response(JSON.stringify([{ id: "remote-self", ...JSON.parse(init.body)[0] }]), { status: 201 });
    return new Response(JSON.stringify([]), { status: 200 });
  };
  try {
    ...
    assert.equal(body.ok, true);
    assert.equal(body.shot_log.made, 123);
    assert.equal(body.diagnostic.team_binding_repair_account_probe, "0:self_identity_verified");
    assert.equal(body.diagnostic.team_binding_repair_result, "repaired");
    assert.equal(body.diagnostic.shot_logs_insert_success, "yes");
  } finally { global.fetch = originalFetch; }
});
