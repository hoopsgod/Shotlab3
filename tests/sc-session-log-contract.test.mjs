...
test('S&C screen has no dead top Add action and useful add controls focus the coach form', () => {
  const scHeader = source.match(/<PageHeader title="S&C"[^>]*\/>/)?.[0] || '';
  assert.ok(scHeader, 'S&C PageHeader should render');
  assert.doesNotMatch(scHeader, /actionLabel=/);
  assert.doesNotMatch(scHeader, /onAction=/);
