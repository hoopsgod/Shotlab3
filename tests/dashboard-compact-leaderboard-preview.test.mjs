...
test('full leaderboards destination exists and includes all final categories with data-required copy', () => {
  ...
  assert.match(appSource, /No event attendance data yet\. Attendance leaderboards will appear after players check into events\./);
  assert.match(appSource, /No strength and conditioning completions yet\. Leaders will appear after players complete activities\./);
  assert.match(appSource, /No drill leaderboard data yet\. Drill leaders will appear after players log coach-assigned drills\./);
});
