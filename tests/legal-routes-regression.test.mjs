...
test('legal/static routes render before AppInner boot state and do not show startup error copy', () => {
  assert.match(appSource, /const legalRoute=getLegalRoute\(\);\nif\(legalRoute\)return <LegalPage page=\{legalRoute\}\/>;\nif\(appErr\)/);
...
test('legal contact constants are app-store ready placeholders with TODO', () => {
  ...
  assert.match(appSource, /TODO\(App Store\): replace these with final monitored ShotLab inboxes before App Store submission\./);
  assert.match(appSource, /const SUPPORT_EMAIL="support@shotlab\.app"/);
  assert.match(appSource, /const PRIVACY_EMAIL="privacy@shotlab\.app"/);
});
