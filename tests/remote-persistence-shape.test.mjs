...
test('buildRemoteRows("sl:events") never includes owner_coach_id even when ownerCoachId exists', () => {
  const [row] = buildRemoteRows('sl:events', [{
    id: 'event-5',
    teamId: 'team-5',
    title: 'Shooting',
    ownerCoachId: 'coach5@team.com',
  }]);

  assert.equal(row.id, 'event-5');
  assert.equal(row.team_id, 'team-5');
  assert.equal(row.title, 'Shooting');
  assert.equal(Object.hasOwn(row, 'owner_coach_id'), false);
});
