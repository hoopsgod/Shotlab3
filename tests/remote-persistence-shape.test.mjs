...
test('DB.set strict remote empty-payload branch logs safe diagnostics and throws explicit error', () => {
  const source = fs.readFileSync(new URL('../src/App.jsx', import.meta.url), 'utf8');
  assert.match(source, /console\.error\("\[remote-persist\] strict payload empty", \{/);
  assert.match(source, /key: k,/);
  assert.match(source, /table,/);
  assert.match(source, /originalRowCount: Array\.isArray\(v\) \? v\.length : 0,/);
  assert.match(source, /remoteRowCount: remoteRows\.length,/);
  assert.match(source, /throw new Error\("remote_persist_payload_empty"\);/);
});
