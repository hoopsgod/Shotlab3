...
test("event dashboard treats RSVP rows as responses and keeps attendance separate", () => {
  const events = [
  ...
  assert.equal(practice?.rsvpConfirmed, 2);
  assert.equal(practice?.awaitingResponse, 1);
  assert.equal(practice?.confirmed, 2);
  assert.equal(practice?.missing, 1);
  assert.equal(practice?.responseRate, 67);
  assert.equal(practice?.attendanceRecorded, 1);
  ...
  assert.equal(metrics.rsvpConfirmed, 2);
  assert.equal(metrics.awaitingResponse, 1);
  assert.equal(metrics.responseRate, 67);
  assert.equal(metrics.attendanceRecorded, 0);
  assert.equal(Object.prototype.hasOwnProperty.call(metrics, "availabilityRate"), false);
});

test("remaining coach pages receive shared dashboard summaries", () => {
  const summary = buildCoachPageDashboardSummary({
    drills: [{ id: 1 }], programDrills: [{ id: 2 }, { id: 3 }], scSessions: [{ id: 4 }], scRsvps: [{ id: 5 }, { id: 6 }], scLogs: [{ id: 7 }], leaderboardRows: [{ name: "Leader" }], activityRows: [{ id: 8 }, { id: 9 }], seasonArchives: [{ id: 10 }],
  });
