...
test('coach player data management UI is separate from account deletion and labels team-local removal', () => {
  ...
  assert.match(appSource, /const totalPlayers=ups\.length/)
  assert.match(appSource, /<CoachCommandCenter[\s\S]*totalPlayers=\{totalPlayers\}/)
  assert.match(appSource, /const allKnown=useMemo\(\(\)=>ups\.map\(p=>\(\{email:normalizeEmail\(p\.email\),name:p\.name\}\)\),\[ups\]\)/)
  assert.match(appSource, /<CoachRoster players=\{filteredCoachRosterPlayers\}/)
  assert.match(appSource, /const filteredCoachRosterPlayers=useMemo\(\(\)=>filteredCoachPlayerDashboardRows\.map\(row=>row\.player\)/)
  assert.match(appSource, /\{roster\.map\(p=>/)
