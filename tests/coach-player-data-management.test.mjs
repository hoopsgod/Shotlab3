...
test('team-local-deleted player is hidden from player and coach leaderboard rows while other active players remain', () => {
  ...
  const rows = filterActiveTeamLeaderboardRows(surfaceState.leaderboardRows, identity.emailSet, identity.nameSet)
...
test('server-returned leaderboard rows are filtered before player and coach render', () => {
  ...
  assert.deepEqual(
    filterActiveTeamLeaderboardRows(serverRows, identity.emailSet, identity.nameSet).map((row) => row.id),
    ['server-active'],
  )
})
