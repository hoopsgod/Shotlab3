...
test('App archiveSeason passes getCoachRosterPlayers activeRosterPlayers into createSeasonArchive', () => {
  ...
  assert.match(appSource, /catch\(error\)\{return\{ok:false,error:error\?\.message\|\|"Could not archive season\."\};\}/);
});

test('archive creation UI shows progress success and returned errors', () => {
  assert.match(appSource, /setSeasonArchiveMessage\("Review the confirmation below, then tap CONFIRM ARCHIVE to create the frozen snapshot\."\)/);
  assert.match(appSource, /setSeasonArchiveMessage\("Archiving season…"\)/);
  assert.match(appSource, /setSeasonArchiveMessage\(r\.ok\?`Archived \$\{r\.archive\.seasonName\}\.`:r\.error\|\|"Could not archive season\."\)/);
  assert.match(appSource, /catch\(error\)\{setSeasonArchiveMessage\(error\?\.message\|\|"Could not archive season\."\);\}/);
  assert.match(appSource, /await archiveSeason\(seasonArchiveForm\)/);
});
