...
test('App archiveSeason passes getCoachRosterPlayers activeRosterPlayers into createSeasonArchive', () => {
  assert.match(appSource, /const activeRosterPlayers=getCoachRosterPlayers\(\{players,playerProfiles,teamId:user\?\.teamId\}\)/);
  assert.match(appSource, /createSeasonArchive\(\{\.\.\.input,teamId:user\?\.teamId,coach:user,players,playerProfiles,activeRosterPlayers,/);
});
