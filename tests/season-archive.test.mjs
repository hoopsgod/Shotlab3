...
test('archive detail displays summary counts and frozen snapshot sections', () => {
  for (const label of ['Roster', 'Home Scores', 'Program Scores', 'Shot Logs', 'Events', 'Event RSVPs', 'S&C Sessions', 'S&C RSVPs', 'S&C Logs', 'Home Makes', 'Program Score', 'Shot Log Makes']) {
    assert.match(appSource, new RegExp(label.replace('&', '&')));
  }
  for (const section of ['ROSTER SNAPSHOT', 'EVENT SNAPSHOT', 'S&C SNAPSHOT', 'PROGRAM DRILL SNAPSHOT']) {
    assert.match(appSource, new RegExp(section.replace('&', '&')));
  }
});

test('archive detail uses selected archive snapshots instead of live app arrays', () => {
  assert.match(appSource, /const roster=Array\.isArray\(archive\?\.rosterSnapshot\)\?archive\.rosterSnapshot:\[\];/);
  assert.match(appSource, /const events=Array\.isArray\(archive\?\.eventSnapshot\)\?archive\.eventSnapshot:\[\];/);
  assert.match(appSource, /const scSessions=Array\.isArray\(archive\?\.scSessionSnapshot\)\?archive\.scSessionSnapshot:\[\];/);
  assert.match(appSource, /const programDrills=Array\.isArray\(archive\?\.programDrillSnapshot\)\?archive\.programDrillSnapshot:\[\];/);
