...
async function simulateHomeShotAdd({ made = 25, remoteSave, quietContext = {} } = {}) {
  ...
  try {
    ...
    const backendErrorCode = String(normalizedError?.body?.error || normalizedError?.message || 'sync_failed');
    const quietFallback = shouldUseQuietHomeShotFallback({
      status: normalizedError?.status,
      errorCode: backendErrorCode,
      message: String(normalizedError?.body?.diagnostic?.message || ''),
      ...quietContext,
    });
    const syncState = quietFallback ? 'local_pending' : 'failed_sync';
    shotLogs = shotLogs.map((log) => (log.id === localLog.id ? { ...log, syncState, syncSource: 'local', syncError: backendErrorCode } : log));
    statSyncError = quietFallback ? '' : HOME_SHOT_SYNC_ERROR_MESSAGE;
    return { result: { ok: quietFallback, mode: syncState, syncState, error: backendErrorCode }, shotLogs, statSyncError };
  }
}
