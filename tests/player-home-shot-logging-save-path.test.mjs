...
test('manual retry flow hides local-only shots from coach until retry saves remotely', () => {
  ...
  const coachVisibleBeforeRetry = playerShotLogs.filter((log) => !log.syncState || log.syncState === 'remote_saved');
  ...
  const coachVisibleAfterRetry = playerShotLogs.filter((log) => !log.syncState || log.syncState === 'remote_saved');
