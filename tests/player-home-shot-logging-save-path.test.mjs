...
test('home-shot retry panel exposes repair action and debug diagnostics in homeShotDebug mode', async () => {
  const source = await readFile(APP_PATH, 'utf8');
  ...
  assert.match(source, /Your player account is not durably linked to this team yet\./);
  assert.match(source, /REPAIR TEAM LINK & RETRY/);
});
