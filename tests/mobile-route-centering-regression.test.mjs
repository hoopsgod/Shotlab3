...
test('mobile secondary route mastheads resolve to a centered one-column title stage', () => {
  const centered = centerMobileRouteStage(promotedFixture)
  assert.match(centered, /grid-template-columns: minmax\(0, 1fr\);[\s\S]*justify-items: center;[\s\S]*text-align: center;/)
  ...
  assert.match(centered, /\.secondaryPageIntro__actions \{[\s\S]*max-width: 360px;[\s\S]*margin: 1px auto 0;[\s\S]*justify-items: center;/)
