...
async function installSafeRoutes(page) {
  ...
  await page.route("**/v1/home-shots/log", async (route) => {
    const payload = route.request().postDataJSON();
    const shotLog = {
      id: payload.id || "player-phase-one-home-shot",
      team_id: TEAM_ID,
      player_id: PLAYER_EMAIL,
      email: PLAYER_EMAIL,
      name: "Phase One Player",
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({ ok: true, shot_log: shotLog, diagnostic: { shot_logs_insert_success: "yes" } }),
    });
  });
...
test("daily command center resolves urgent commitment then launches one bounded first result", async ({ page }) => {
  ...
  await expect(commandCenter.getByRole("heading", { name: "Set your baseline.", exact: true })).toBeVisible();
  await expect(commandCenter.getByText("Baseline", { exact: true })).toBeVisible();
