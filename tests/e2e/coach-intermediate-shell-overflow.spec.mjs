...
for (const width of [1024, 1280]) {
  test(`Coach Players does not make the ${width}px document horizontally scrollable`, async ({ page, context }) => {
    ...
    console.log(`[coach-${width}-geometry] ${JSON.stringify(geometry)}`);

    expect(geometry.viewport).toBe(width);
    expect(geometry.root).toBeLessThanOrEqual(width + 2);
    expect(geometry.body).toBeLessThanOrEqual(width + 2);
    expect(geometry.shell).toBeLessThanOrEqual(width);
