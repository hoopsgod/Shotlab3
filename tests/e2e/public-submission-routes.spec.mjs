...
const routes = [
  ...
  { path: "/delete-account", heading: "Delete Account & Data", proof: /delete account|personal data/i },
  { path: "/data-request", heading: "Data Request", proof: /access|export|correction|deletion/i },
];
...
for (const route of routes) {
  test(`${route.path} opens directly without authentication`, async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /Back to ShotLab/i })).toBeVisible();
