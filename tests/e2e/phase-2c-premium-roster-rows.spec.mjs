...
async function capture(page, name, locator) {
  ...
  if (locator) await locator.screenshot({ path: path.join(outputDir, `${name}.png`), animations: 'disabled' });
  else await page.screenshot({ path: path.join(outputDir, `${name}.png`), fullPage: true, animations: 'disabled' });
}

async function flatSurface(locator) {
  return locator.evaluate((node) => {
    const style = getComputedStyle(node);
    return { backgroundColor: style.backgroundColor, backgroundImage: style.backgroundImage, boxShadow: style.boxShadow, borderTopWidth: style.borderTopWidth, borderRightWidth: style.borderRightWidth, borderBottomWidth: style.borderBottomWidth, borderLeftWidth: style.borderLeftWidth, borderRadius: style.borderRadius };
  });
}
...
async function verifyRosterGeometry(page, width) {
  ...
  const pseudoGeometry = await profile.evaluate((node) => ({ beforeContent: getComputedStyle(node, '::before').content, afterContent: getComputedStyle(node, '::after').content }));
...
test('Coach Players roster is one flat editorial surface and preserves contextual management', async ({ page }) => {
  ...
  for (const selector of ['.coachRosterCard__details', '.coachRosterCard__identity', '[data-phase1-open-profile="true"]', '.coachRosterCard__metrics']) {
  ...
  const profilePseudoGeometry = await profileButton.evaluate((node) => ({ beforeContent: getComputedStyle(node, '::before').content, afterContent: getComputedStyle(node, '::after').content }));
  ...
  const removeAction = firstRow.getByRole('menuitem', { name: 'Remove from team', exact: true });
  await expect(removeAction).toBeHidden();
  await manageTrigger.focus();
  await page.keyboard.press('Tab');
  await page.keyboard.press('Shift+Tab');
  await expect(manageTrigger).toBeFocused();
  const manageFocus = await manageTrigger.evaluate((node) => { const style = getComputedStyle(node); return { outlineStyle: style.outlineStyle, outlineWidth: parseFloat(style.outlineWidth) }; });
  ...
  page.once('dialog', async (dialog) => { removalDismissed = true; await dialog.dismiss(); });
  ...
  if (options.length > 1) { await sort.selectOption(options[1]); await expect(sort).toHaveValue(options[1]); }
  ...
  const profileFocus = await profileButton.evaluate((node) => { const style = getComputedStyle(node); return { outlineStyle: style.outlineStyle, outlineWidth: parseFloat(style.outlineWidth) }; });
...
test('Coach Players roster remains overflow-safe at all required responsive widths', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await enterCoachPlayers(page);
  for (const width of viewportWidths) await verifyRosterGeometry(page, width);
});
