...
test('expires_at is computed from expires_in when needed', () => {
  return readFile(new URL('../src/lib/supabase.js', import.meta.url), 'utf8').then((src) => {
    assert.match(src, /Math\.floor\(Date\.now\(\) \/ 1000\) \+ expiresIn/);
  });
});
