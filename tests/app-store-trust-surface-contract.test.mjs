...
test('delete account and data request entries are first-class account surface actions', () => {
  ...
  assert.match(accountTrustSource, /busy\?"WORKING\.\.\.":"CONFIRM DELETE ACCOUNT"/)
...
test('delete account primary path is in-app and support email is secondary', () => {
  ...
  const runDeleteIndex = accountTrustSource.indexOf('onClick={runDelete}')
  ...
  assert.ok(runDeleteIndex >= 0 && runDeleteIndex < primaryConfirmIndex, 'primary confirmation button should call deleteAccount through runDelete')
