...
test('coach leaderboard entry is stabilized (no clickable crashing route) and player leaderboards stay enabled', () => {
  ...
  assert.match(appSource, /tab==="leaderboards"&&!active&&renderPremiumLeaderboardsHub\(\{mode:"player"/);
  assert.match(appSource, /tab==="leaderboards"&&renderPremiumLeaderboardsHub\(\{mode:"coach"/);
