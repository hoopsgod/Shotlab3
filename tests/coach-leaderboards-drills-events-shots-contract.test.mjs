...
test('team code lookup normalizes input and uses shared backend invite source', async () => {
  ...
  assert.match(source, /const consumeInFlightRef=useRef\(false\)/);
  assert.match(source, /const joinSubmitInFlightRef=useRef\(false\)/);
  assert.match(source, /if\(consumeInFlightRef\.current\)\{/);
  assert.match(source, /consumeInFlightRef\.current=true/);
  assert.match(source, /finally\{\s*consumeInFlightRef\.current=false;/);
  assert.match(source, /if\(joinSubmitInFlightRef\.current\)return\{ok:false,retryable:true,err:"Join in progress\."\};/);
  assert.match(source, /}finally\{\s*joinSubmitInFlightRef\.current=false;\s*}/);
  ...
  assert.match(source, /const joined=await consumeJoinContext\(user,null,activeContext\)/);
  assert.match(source, /if\(isJoining\)return;/);
  assert.match(source, /setIsJoining\(true\);/);
  assert.match(source, /disabled=\{isJoining\}/);
  assert.match(source, /\{isJoining\?"JOINING\.\.\.":"JOIN TEAM"\}/);
});
