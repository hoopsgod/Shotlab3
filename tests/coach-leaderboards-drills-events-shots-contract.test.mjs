...
test('player-entered stats require durable ids, strict remote persistence, sync error handling, and coach-data refresh', async () => {
  ...
  assert.match(source, /setStatSyncError\(fallback\?\.mode==="supabase"&&!fallback\?\.reason\?"":homeShotDebugMode\?`\$\{baseError\} Error: \$\{String\(e\?\.message\|\|"sync_failed"\)\}`:baseError\)/);
  assert.match(source, /console\.error\("home_shots_save_failed",\{status:"failed"/);
...
test('team code lookup normalizes input and uses shared backend invite source', async () => {
  ...
  assert.match(source, /if\(!ready\|\|view!=="join-team"\|\|!user\|\|user\.role!=="player"\|\|user\.teamId\)return;/);
  assert.match(source, /const saved=normalizeStoredInviteContext\(pendingJoinContext\)\|\|readInviteContextFromStorage\(\)/);
  assert.match(source, /joinTeam\(saved\.inviteCode\|\|""\)\.catch\(\(\)=>\{\}\)\.finally\(\(\)=>\{autoJoinAttemptRef\.current="";\}\);/);
  assert.match(source, /consumeResolvedUserUuid:String\(body\?\.resolved_user_uuid\|\|prev\.join\.consumeResolvedUserUuid\|\|""\)/);
  assert.match(source, /consumeResolvedUserUuid:joined\?\.resolvedUserUuid\|\|prev\.join\.consumeResolvedUserUuid/);
  assert.match(source, /const mapConsumeDiagnostic=\(errorCode,status\)=>\{/);
  assert.match(source, /const \[isJoinConsumeActive,setIsJoinConsumeActive\]=useState\(false\)/);
  assert.match(source, /const joinConsumeFlightRef=useRef\(\{active:false,key:"",startedAt:0,lastClearedAt:0,lastClearedReason:"",promise:null,abortController:null\}\)/);
  assert.match(source, /const consumeInFlightKey=buildConsumeInFlightKey\(\{email:currentUserEmail,inviteCode:resolvedContext\?\.inviteCode\|\|"",joinContextToken:resolvedContext\?\.joinContextToken\|\|"",teamId:resolvedContext\?\.teamId\|\|""\}\)/);
  assert.match(source, /const acquired=acquireConsumeSingleFlight\(joinConsumeFlightRef\.current,\{key:consumeInFlightKey,start:\(\)=>\{/);
  assert.match(source, /if\(acquired\.mode==="joined"\)return acquired\.promise;/);
  assert.match(source, /AbortController\(\)/);
  assert.match(source, /setTimeout\(\(\)=>abortController\.abort\("consume_fetch_timeout"\),15_000\)/);
  assert.match(source, /diagnosticCode:"consume_fetch_timeout"/);
  assert.doesNotMatch(source, /if\(isJoinConsumeActive\)return\{ok:false,retryable:true,err:"Join already in progress\."\};/);
  assert.doesNotMatch(source, /join_in_progress/);
  assert.match(source, /code:"consume_session_lookup_failed"/);
  assert.match(source, /code:"consume_membership_insert_failed"/);
  assert.match(source, /consume_profile_update_failed/);
  assert.match(source, /code:"consume_context_token_missing"/);
  assert.match(source, /code:"consume_subject_mismatch"/);
  assert.match(source, /consumeEndpoint:"\/v1\/team-memberships\/confirm-context"/);
  ...
  assert.match(source, /<div>Consume endpoint: \{dataDebug\.join\.consumeEndpoint\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Consume attempt started: \{dataDebug\.join\.consumeAttemptStarted\|\|"no"\}<\/div>/);
  assert.match(source, /<div>Consume attempt blocked: \{dataDebug\.join\.consumeAttemptBlocked\|\|"no"\}<\/div>/);
  assert.match(source, /<div>Consume blocked reason: \{dataDebug\.join\.consumeAttemptBlockedReason\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Consume timeout ms: \{dataDebug\.join\.consumeTimeoutMs==null\?"n\/a":dataDebug\.join\.consumeTimeoutMs\}<\/div>/);
  assert.match(source, /<div>Consume fetch started: \{dataDebug\.join\.consumeFetchStarted\|\|"no"\}<\/div>/);
  assert.match(source, /<div>Consume fetch finished: \{dataDebug\.join\.consumeFetchFinished\|\|"no"\}<\/div>/);
  assert.match(source, /<div>Consume diagnostic code: \{dataDebug\.join\.consumeDiagnosticCode\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Consume user email: \{dataDebug\.join\.consumeUserEmail\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Consume resolved user UUID: \{dataDebug\.join\.consumeResolvedUserUuid\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Membership insert status: \{dataDebug\.join\.membershipInsertStatus\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Membership insert error: \{dataDebug\.join\.membershipInsertError\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Profile update status: \{dataDebug\.join\.profileUpdateStatus\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Final route decision: \{dataDebug\.join\.finalRouteDecision\|\|"none"\}<\/div>/);
  ...
  assert.match(source, /const joined=await consumeJoinContext\(user,null,activeContext\)/);
  assert.match(source, /if\(joined\?\.retryable\)return\{ok:false,retryable:true,err:joined\.err\|\|"Could not join team\. Please retry\."\};/);
  assert.doesNotMatch(source, /const r=await consumeJoinContext\(user,null,savedInviteContext\)/);
  assert.match(source, /if\(isJoinConsumeActive\)return;/);
  assert.match(source, /disabled=\{isJoinConsumeActive\}/);
  assert.match(source, /type="button"/);
  assert.match(source, /style=\{\{opacity:isJoinConsumeActive\?0\.75:1,cursor:isJoinConsumeActive\?"not-allowed":"pointer"\}\}/);
  assert.match(source, /\{isJoinConsumeActive\?"JOINING\.\.\.":"JOIN TEAM"\}/);
});
