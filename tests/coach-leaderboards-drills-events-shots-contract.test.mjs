...
test('team code lookup normalizes input and uses shared backend invite source', async () => {
  ...
  assert.match(source, /const consumeInFlightRef=useRef\(\{ active:false,key:"",startedAt:0,lastClearedAt:0,lastClearedReason:"" }\)/);
  assert.match(source, /const \[isJoinConsumeActive,setIsJoinConsumeActive\]=useState\(false\)/);
  assert.match(source, /const consumeInFlightKey=buildConsumeInFlightKey\(\{/);
  assert.match(source, /const guardStatus=evaluateConsumeGuard\(consumeInFlightRef\.current,consumeInFlightKey,now\)/);
  assert.match(source, /if\(guardStatus\.blocked\)\{/);
  assert.match(source, /markConsumeGuardStarted\(consumeInFlightRef\.current,consumeInFlightKey,Date\.now\(\)\)/);
  assert.match(source, /clearConsumeGuard\(consumeInFlightRef\.current,Date\.now\(\),"attempt_finished"\)/);
  ...
  assert.match(source, /<div>Consume in-flight key: \{dataDebug\.join\.consumeInFlightKey\|\|"none"\}<\/div>/);
  assert.match(source, /<div>Consume guard cleared: \{dataDebug\.join\.consumeGuardCleared\|\|"no"\}<\/div>/);
