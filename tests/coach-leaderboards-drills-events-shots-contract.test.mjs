...
test('team code join normalizes input and matches coach joinCode field', async () => {
  const source = await appSource();

  assert.match(source, /const normalizedCode=normalizeJoin\(code\)\.replace\(\/\[-\\s\]\+\/g,""\)/);
  assert.match(source, /const localMatches=teams\.filter\(t=>normalizeJoin\(t\?\.joinCode\|\|""\)\.replace\(\/\[-\\s\]\+\/g,""\)===normalizedCode\)/);
  assert.match(source, /resolvedTeamId=localMatches\[0\]\.id/);
});
