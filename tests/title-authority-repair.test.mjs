...
test('Coach Home markup and owned component CSS form one tactical decision-first Mission Control title authority', () => {
  ...
  assert.match(coachTitleCss, /font:\s*760 clamp\(28px,\s*7\.6vw,\s*33px\)\/\.96 var\(--mc-native\)/);
...
test('Coach Home decision, metrics and CTA have one source owner', () => {
  ...
  assert.match(coachTitleCss, /\.mcRealityStrip\s*\{[\s\S]*background:\s*rgba\(2,\s*13,\s*19,\s*\.18\)/);
  assert.match(coachTitleCss, /\.mcRealityStrip strong\s*\{[\s\S]*color:\s*var\(--mc,\s*#c8ff1a\)/);
  assert.match(coachTitleCss, /\.mcRealityStrip small\s*\{[\s\S]*color:\s*#adb9be/);
  assert.doesNotMatch(coachTitleCss, /!important|html\s+body\s+#root/);
