...
test('S&C supporting intelligence remains one mobile disclosure without deleting compliance data', () => {
...
test('S&C route keeps the accepted light native secondary canvas', () => {
...
test('S&C delegates metric presentation to the shared Phase 6 decision stage', () => {
  ...
  assert.doesNotMatch(css, /coach-page-dashboard-strength-metric-strip/);
});

test('S&C operational filters use a flat editorial utility rail', () => {
  assert.match(css, /\[data-testid=\"coach-strength-operational-filters\"\][\s\S]*padding:0!important[\s\S]*border:0!important[\s\S]*background:transparent!important/);
  assert.match(css, /\[data-testid=\"coach-strength-operational-filters\"\] label[\s\S]*min-height:46px!important[\s\S]*background:#fff!important/);
  assert.match(css, /\[data-testid=\"coach-strength-operational-filters\"\] button\[aria-pressed=\"true\"\]/);
  assert.match(css, /\[data-testid=\"coach-strength-operational-filters\"\] button\{[\s\S]*min-height:44px!important/);
});
...
test('desktop preserves expanded S&C intelligence while iPhone defaults it closed', () => {
  ...
  assert.match(css, /@media\s*\(min-width:761px\)[\s\S]*coachStrengthSupportingSummary\{display:none\}/);
  assert.match(css, /@media\s*\(max-width:760px\)/);
  assert.doesNotMatch(enhancer, /open=\{true\}/);
});

test('mobile compliance disclosure uses route-scoped editorial summary authority', () => {
  ...
  assert.match(css, /coachStrengthSupportingSummary>\.coachStrengthSupportingSummaryCopy::before[\s\S]*coachStrengthSupportingSummary>\.coachStrengthSupportingSummaryCopy::after[\s\S]*content:none!important/);
  assert.match(css, /coachStrengthSupportingSummaryCopy>small[\s\S]*box-shadow:none!important[\s\S]*transform:none!important/);
});

test('expanded S&C insight evidence is readable without restoring card chrome', () => {
  assert.match(css, /\[data-testid=\"coach-strength-insight-grid\"\] article[\s\S]*--text-1:#171a18[\s\S]*--text-2:#5d665f/);
  assert.match(css, /\[data-testid=\"coach-strength-insight-grid\"\] article[\s\S]*border:0!important[\s\S]*border-radius:0!important[\s\S]*background:transparent!important[\s\S]*box-shadow:none!important/);
  ...
  assert.match(css, /\[data-testid=\"coach-strength-insight-grid\"\] article button[\s\S]*min-height:44px!important[\s\S]*color:#25321c!important/);
