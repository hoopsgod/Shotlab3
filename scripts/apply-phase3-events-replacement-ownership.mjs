...
if ((app.split(eventAuthority).length - 1) + (app.split(strengthAuthority).length - 1) + (app.split(explicitRsvpStrengthAuthority).length - 1) !== 1) throw new Error('Events replacement authority or its S&C successor must exist exactly once.')
...
if (!app.includes(explicitDelete) && !app.includes(explicitRsvpDelete)) {
...
if ((app.split(explicitDelete).length - 1) + (app.split(explicitRsvpDelete).length - 1) !== 1) throw new Error('Explicit coach event deletion replacement must exist exactly once.')
