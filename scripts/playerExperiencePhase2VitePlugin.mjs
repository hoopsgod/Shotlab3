...
function replaceRequired(source,label,needle,replacement){
  if(!source.includes(needle)){
    throw new Error(`Player Phase 2 transform failed: missing ${label}`);
  }
  return source.replace(needle,replacement);
}

export function applyPlayerExperiencePhase2(source){
  ...
  next=replaceRequired(
    ...
    `{(tab==="log-drill")&&!active&&!showShotStats&&<div className="fade-up">\n    <PlayerAtHomeDashboard\n      today={today}\n      userEmail={u?.email||""}\n      shotLogs={shotLogs}\n      drills={drills}\n      todayScores={todayS}\n      onPrimaryAction={()=>{const nextDrill=drills.find((drill)=>!todayS.some((score)=>String(score?.drillId??score?.drill_id??"")===String(drill?.id??"")));if(nextDrill)setActive(nextDrill);}}\n      onViewStats={()=>setShowShotStats(true)}\n    />`
  );
...
export default function playerExperiencePhase2Plugin(){
  return {
    name:"shotlab-player-experience-phase-2",
    enforce:"pre",
    transform(code,id){
      const cleanId=String(id||"").split("?")[0].replaceAll("\\","/");
      if(!cleanId.endsWith(APP_SUFFIX))return null;
      return {code:applyPlayerExperiencePhase2(code),map:null};
    },
  };
}
