...
function replaceCoachPanelFirstKnown(pairs, label) {
  for (const [, replacement] of pairs) {
    if (coachPanelSource.includes(replacement)) return
  }
  for (const [needle, replacement] of pairs) {
    if (coachPanelSource.includes(needle)) {
      coachPanelSource = coachPanelSource.replace(needle, replacement)
      return
    }
  }
  throw new Error(`Could not locate ${label} parity boundary in any supported CoachDashboardPhase2.jsx state.`)
}
...
// Phase 3L runs before this enhancer in the production build and upgrades the
// leaderboard row geometry. Preserve that newer composition, cap it to the same
// three-row mobile runway, and fill sparse positions with truthful placeholders.
const leaderboardPhase3LBefore = `          {rows.map((row) => (
            <button
              type="button"
...
const leaderboardPhase3LAfter = `          {rows.slice(0, 3).map((row) => (
            <button
              type="button"
              className={styles.operationalRow + " coachLeaderboardRow"}
              data-rank={row.rank}
...
const leaderboardLegacyBefore = `          {rows.map((row) => (
...
const leaderboardLegacyAfter = `          {rows.slice(0,3).map((row) => (
            <button type="button" className={styles.operationalRow} key={row.key} onClick={() => onOpenPlayer?.(row.row)}>
              <div><strong>#{row.rank} {row.name}</strong><span>{row.total} total · {row.weekly} this week · last active {row.lastActivity || "unknown"}</span></div>
              <em className={row.improvement > 0 ? styles.deltaPositive : row.improvement < 0 ? styles.deltaNegative : styles.deltaNeutral}>{formatDelta(row.improvement)}</em>
            </button>
...
replaceCoachPanelFirstKnown([
  [leaderboardPhase3LBefore, leaderboardPhase3LAfter],
  [leaderboardLegacyBefore, leaderboardLegacyAfter],
], 'coach leaderboard minimum ranking rows')

// Phase 2D wraps the activity list in a semantic empty-state branch before this
// enhancer runs. Normalize either that current state or the older raw-list state to
// one six-row mobile runway.
const activityPhase2DBefore = `      {rows.length ? (
...
const activityLegacyBefore = `      <div className={styles.activityList} data-testid="coach-activity-intelligence-results">
          {rows.slice(0, 12).map((row) => (
            <button type="button" className={styles.activityRow} key={row.id} onClick={() => onOpenItem?.(row)}>
              <div><strong>{row.title}</strong><span>{row.type.toUpperCase()} · {row.detail}</span></div>
              <time>{row.date}</time>
...
const activityParity = `      <div className={styles.activityList} data-testid="coach-activity-intelligence-results" data-parity-slot-count="6">
...
replaceCoachPanelFirstKnown([
  [activityPhase2DBefore, activityParity],
  [activityLegacyBefore, activityParity],
...
console.log('Applied stable mobile secondary-page composition for Demo and registered accounts (v2).')
