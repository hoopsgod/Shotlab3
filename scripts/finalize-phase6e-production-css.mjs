...
function pruneSupersededCoachMobile(css) {
  ...
  let output = rewriteMediaBlocks(css, MOBILE_700_MEDIA, (whole, selector, declarations) => {
  ...
  output = rewriteMediaBlocks(output, MOBILE_350_MEDIA, (whole, selector, declarations) => {
    if (selector.trim().startsWith("@")) return whole;
    const arms = selector.split(",").map((arm) => arm.trim()).filter(Boolean);
