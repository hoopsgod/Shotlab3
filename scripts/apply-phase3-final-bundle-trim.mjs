...
const replacement = `function ProgramPulsePanel({ model }) {
  ...
  return <article className="mcSection mcTeamHealth" aria-labelledby="mc-program-pulse-heading" data-testid="coach-program-pulse"><div className="mcSectionHead"><span><h2 id="mc-program-pulse-heading">Program Pulse</h2></span><strong className="mcHealthScore">{value == null ? "—" : \`\${value}%\`}</strong></div>{value == null ? <small>No weekly goal data</small> : null}</article>;
}`;
source = source.slice(0, start) + replacement + source.slice(end);
source = source.replace("  const pulsePanel = <ProgramPulsePanel model={programPulse} onOpen={onPlayersClick} />;", "  const pulsePanel = <ProgramPulsePanel model={programPulse} />;");
...
tests = tests
  ...
  .replace('assert.match(source,/model\\?\\.value == null \\? "No weekly goal data"/);', 'assert.match(source,/value == null \\? <small>No weekly goal data/);');
