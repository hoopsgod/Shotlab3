...
const mobileAnchor='  getPlayerNavItem("team-store",{mobileLabel:"Team Store",description:"Official team apparel and fan gear"}),',mobileItem='  getPlayerNavItem("profile",{k:"personalization",l:"Personalization",description:"Profile photo",group:"team"}),';
...
const profileComment='  {/* ═════════════ PROFILE — Offseason Resume ═════════════ */}',route=`{tab==="personalization"&&<SecondaryPageShell testId="player-personalization-workspace"><SecondaryPageDecision eyebrow="Player identity" title="Personalization" detail="Profile photo shown across ShotLab." testId="player-personalization-photo" icon="profile">${photoSurface}</SecondaryPageDecision></SecondaryPageShell>}\n\n`;
...
const coachAnchor='<Av n={profile.identity.name} sz={64} email={profile.identity.email}/>',coachPhoto='{player?.photoUrl||player?.photo_url?<img data-testid="coach-player-profile-photo" src={player.photoUrl||player.photo_url} alt={`${profile.identity.name} profile`} width="64" height="64" style={{width:64,height:64,borderRadius:"50%",objectFit:"cover",flexShrink:0}}/>:<Av n={profile.identity.name} sz={64} email={profile.identity.email}/>}';
...
for(const marker of[photoImport,'personalization:"/personalization"','k:"personalization"','testId="player-personalization-workspace"',photoSurface,'className="coachRosterCard__photo"','data-testid="coach-player-profile-photo"'])if(!source.includes(marker))throw Error(`Phase 7E marker missing after transform: ${marker}`);
