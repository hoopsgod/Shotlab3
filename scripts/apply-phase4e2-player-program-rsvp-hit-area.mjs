...
if (!source.includes(marker)) {
  ...
  if (firstIndex < 0 || source.indexOf(anchor, firstIndex + anchor.length) >= 0) {
    throw new Error('Phase 4E.2 expected exactly one Player Program inline quick-RSVP button template.');
  }
  ...
  console.log('Phase 4E.2 Player Program RSVP component hook already applied.');
}

const authorityMarker = 'Phase 4E.2 Player Program RSVP physical target';
if (!authority.includes(authorityMarker)) {
  authority += `\n\n/* ${authorityMarker}. The Phase 4E readiness simulation proved this family\n * can reach the mobile touch baseline without overlap, overflow, or hierarchy damage. */\n.performance-shell[data-workspace-tab="program"] button[data-player-program-rsvp-action] {\n  min-height: 44px !important;\n  box-sizing: border-box !important;\n  touch-action: manipulation !important;\n}\n`;
  writeFileSync(authorityPath, authority);
} else {
  console.log('Phase 4E.2 final Player Program RSVP authority already applied.');
}

console.log('Applied Phase 4E.2 Player Program RSVP hit-area correction.');
