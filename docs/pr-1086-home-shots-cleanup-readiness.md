# PR 1086 Home-Shots Cleanup Readiness

## Preview validation

Cloudflare Pages preview registered-player testing checklist for PR 1086 cleanup:
...
- Before merging PR 1086, confirm GitHub shows it as ready for review and mergeable, and confirm the PR 1086 Cloudflare Pages preview deploy is green.
...
- A standalone Cloudflare Workers check is non-blocking for the PR 1086 cleanup because Cloudflare Pages is the supported deployment target; production merge should use the Cloudflare Pages preview/deploy status as the source of truth unless CI explicitly maps that check to the Pages project.
