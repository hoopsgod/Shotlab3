...
- Full npm test currently fails because of unrelated pre-existing contract/e2e failures.
- Targeted Supabase leaderboard persistence test must be run directly:
  node --test tests/supabase-leaderboard-persistence-verification.test.mjs
- npm run verify:leaderboards:supabase skipped because Supabase env vars are missing.
- Live Supabase was not verified in this environment.
- This PR adds the live Supabase verification harness but does not prove live Supabase until run with real credentials.
