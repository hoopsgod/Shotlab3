...
- `TEST_TEAM_ID`
- `TEST_PLAYER_A_ID` (or `TEST_PLAYER_ID`)
- `TEST_PLAYER_B_ID`

If any are missing, the live smoke test is **skipped** (not failed), so CI stays green.
...
- the player with higher saved total ranks above the lower total player
